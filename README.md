...
- 可选 Node-RED 服务提供 HTTP、WebSocket、MQTT、MySQL、PostgreSQL、Oracle、TDengine、OPC UA、Modbus TCP、BACnet 和 FlowFuse Dashboard；Studio 统一为一个 ECharts + GridStack 轻量看板入口，内置演示数据、即点即加组件和只读浏览模式，布局可保存到场景并通过同一 WebSocket 实时更新；统一场景消息只允许映射显隐、颜色、位置和标签
