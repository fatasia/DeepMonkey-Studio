...
| Web 静态包 | 2026-09-18 更新：loader EOF 取消竞态已修（b3b6bc3）+包根纵深防御；生成端与消费端整组入库（a3fd714）；r6 端到端重生成后两轮真实浏览器 15/15（根/子目录、双主题、980 窄屏、全屏、篡改/缺资源 fail-closed、取消恢复，be879a7）。 | API 侧 web 下载格式注册未接；生成端尚未复用 ScenePublication 状态机（当前为独立脚本，收拢时不得建第二套发布状态机）。 |
...
> 2026-09-18 进度注记：P0-04 放行门禁（7e8c60c）、P0-06 五组件中文交付验收（ebd51dd）与三项缺陷修复（标题 letterbox 2cb395d、测量满框 72b52f5、轴/图例 9f804f8）、P1-13 受控 HTTP+泵（0ab976e/9146d7f）、P1-18 TS 字形合同（5732f23）已收拢；G04 切片 B 宿主模块就绪（618b6d3，部署装配待接）。证据见 docs/specs/dashboard-* 与 deep2d-* 2026-09-17/18 报告。
