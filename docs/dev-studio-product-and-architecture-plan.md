...
从 M4 开始增加一条不可降级的产品门槛：**在项目锁定的 Three.js 版本中，能够通过 Three.js 文档化公开 API 在目标浏览器实现的能力，必须能够在 Dev Studio 中通过 `SceneCapabilitySDK`、行为脚本或 Scene Extension 插件简单实现，并且不得修改编辑器核心代码。** 常见能力必须优先提供可发现、可撤销、可预览的编辑器操作；长尾和底层能力允许进入可信扩展，但扩展仍使用版本化公开上下文，不得导入 `ViewerEngine`、React 状态或其他私有模块。
...
| 典型迁移 | 同目录保存 Three.js 原始样例、Dev Studio 实现、迁移说明和预期截图/数据 | 至少覆盖对象批量创建与层级、相机飞行与射线拾取、自定义 Mesh/材质、GLTF 动画、实例化、后处理/TSL、第一人称控制、AGV/机器人时间驱动八类样例 | 增加自定义 Loader/转换结果、GeoAnchor/地图叠加、拓扑与三维联动样例 | 抽取为可安装示例插件和 SDK 回归夹具；全部不改核心即可启停 |
...
- 浏览器永远只访问 Dev Studio 同源 `ConnectorGateway`：HTTP 由服务器代理并返回统一错误信封，外部 WebSocket 由服务器建立、限流和重连，再复用一个应用级同源 WebSocket channel 向多个 2D/3D 订阅者分发。发布文档不保存密码、Token 或私有 Header；网关从服务器凭据引用解析。管理员可配置出站域名/IP/端口与私网访问策略，并防止 DNS 重绑定，避免“解决 CORS”变成无约束服务器代理。
...
验收：继续通过 4.3.3 门禁中与加载器、转换结果、坐标、地图材质、嵌入视口和跨编辑器交互相关的矩阵项；转换任务异常不影响编辑器且产物可复用；`.x_t/.x_b` 是 M5 硬门槛，必须选定可私有部署且可自动化的 Parasolid 转换 Provider，并使用固定黄金样本验证版本探测、单位、坐标、装配树、零件选择、属性和失败诊断，不得把“等待转换器”冒充为已支持；JT 不作为 M5 硬门槛，只有获得合规 SDK 与黄金样本后才升级支持状态；底图离线可降级；坐标黄金样本双向定位一致；1000 节点、2000 边的拓扑基准满足质量预算。
...
边界与依赖：M10 依赖 M3 的数据契约、M4 的行为脚本与 Scene SDK、M6 的插件/权限/版本体系、M8 的仿真证据和 M9 的版本文档；不做开放域问答、完整语义中台、自动训练平台、通用 Agent 编排、任意 Shell/文件/网络操作、无人确认发布、真实设备自动控制或以 AI 输出替代确定性仿真。没有稳定公开命令/API 的编辑能力不得靠 AI 绕过架构补洞。IoT-NB 与 FATHOM 均为同一用户开发，可直接复用代码；接入时仍按当前公共契约提取小模块、保留来源与测试，不把其 Tauri/Rust/Edge、完整 Python 语义平台、Dify 镜像或独立 UI 整包并入 Dev Studio。
