...
验收：典型 1920×1080 工业看板不写代码可完成；同一应用可创建 3840×1080 双联屏、3840×2160 4K 和合法自定义分辨率页面，保存刷新后尺寸与适配策略不丢失；四种适配策略在目标屏幕比例下行为符合名称，越界组件可定位且修改分辨率可撤销；300 个普通组件编辑可用；关键操作均支持撤销；复制粘贴不破坏数据和交互引用。
...
边界与依赖：M10 依赖 M3 的数据契约、M4 的行为脚本与 Scene SDK、M6 的插件/权限/版本体系、M8 的仿真证据和 M9 的版本文档；不做开放域问答、自动训练平台、通用 Agent 编排、任意 Shell/文件/网络操作、无人确认发布、真实设备自动控制或以 AI 输出替代确定性仿真。没有稳定公开命令/API 的编辑能力不得靠 AI 绕过架构补洞。

规划交付物：候选场景价值/风险排序、`AiStudioPlugin` 与 `AiProvider` 草案、数据出域和权限模型、成本估算、失败降级方案，以及未来可冻结的 50 个任务评测集设计。实施立项门槛建议保持为：锁定 Provider/模型版本后至少 85% 的任务能生成通过 schema、类型、权限和引用检查的可预览结果；零任务可绕过确认直接发布或访问未授权上下文；代表性任务必须支持“提出意图 → 查看计划/依据 → 预览差异 → 局部接受/拒绝 → 撤销”。这些是未来 go/no-go 条件，不代表当前已实现或已进入开发。
