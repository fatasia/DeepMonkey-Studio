...
| 三维场景深度 | ThingJS、飞渡 DTS、51WDP | 保留当前 BIM/CAD、构件、空间、剖切、碰撞、动画和工业视觉优势，补齐 GIS 与超大场景资源管线 |
| 二三维编排 | FineVis、51WDP | 统一事件图和变量作用域，二维与三维双向联动、可视化调试 |
| 低代码易用性 | 山海鲸 | 自动联动、条件样式、向导、模板、实时预览、错误可定位 |
| 专业扩展 | ThingJS、Unity、51WDP | 公式、脚本沙箱、组件 SDK、动作 SDK、数据连接器 SDK、模板变体与可检查的实例覆盖 |
| 本地交付 | 山海鲸、ThingStudio | Tauri 本地工作区、一键发布、私有化服务器、断点续传和回滚 |
| 大场景性能 | 飞渡 DTS、51WORLD、Unity | BIM/GIS 结构索引、转换与轻量化、分块/LOD、地址化按需资源和性能预算 |
...
| ThingStudio | 将数据接入、园区/城市场景、大屏、拓扑和发布组成工具链；资源与插件可复用 | 不把二维、三维、拓扑做成孤岛：统一资产目录、数据集、变量、事件、组件注册表和发布包；拓扑作为可引用的页面/组件类型 |
| 飞渡 DTS | 数据转换与轻量化、Explorer 场景编辑、Cloud 云渲染、SDK 和 Charts 分层 | 建立“导入/转换 → 资产编目 → 场景搭建 → 应用编排 → 构建发布”的资源管线；WebGL 本地渲染为默认，云渲染通过后端接口按项目选配 |
...
| Unity Prefab | 模板、变体、实例覆盖、Apply/Revert 和隔离编辑模式 | 组件套件、业务模板、三维设备模板统一采用“定义 → 变体 → 实例覆盖”；覆盖值有醒目标识，可逐项应用或还原；双击进入隔离编辑并用面包屑返回原上下文 |
| Unity Scene/Game、Timeline | 创作视图与最终运行视图分离，Play 模式验证行为；复杂序列使用时间线 | 二维和三维均提供“设计/预览运行”状态，运行态使用明显边框且不静默写回；动画、镜头、告警演练共用多轨时间线和事件标记 |
| Unity Build Profiles/Addressables | 不同构建目标拥有独立场景列表与配置；资源按逻辑地址和依赖异步定位，可区分本地/远端 | 提供本地预览、服务器 Web、后续云渲染等发布配置；资产以稳定地址、内容哈希和依赖图管理，缺失资源可在发布前定位 |
| 51WDP | 场景、面板、交付分阶段；蓝图节点编排场景与画布联动；场景支持版本增量 | 项目管理中明确“页面、场景、数据、发布”；第二阶段增加可视化交互图，发布采用不可变版本和增量资源 |

调研后的取舍：

- 学 ThingJS 的完整生产链和插件化，但 Dev Studio 的二维页面、三维场景、拓扑、数据不能分别保存和发布，必须由一个 `ApplicationDocument` 原子交付。
- 学飞渡的大场景数据治理和编辑/渲染分层，但首期不把云渲染设为必需依赖；普通项目应离线可编辑并由浏览器/Tauri 使用同一 WebGL 运行时。
- 学 Unity 的专业编辑器语法，但不照搬游戏开发复杂度；默认界面服务低代码用户，高级的组件、覆盖、时间线和诊断按需展开。
...
- [Unity Prefab 实例 Inspector](https://docs.unity3d.com/ja/current/Manual/prefab-instance-inspector-reference.html)
...
- [Unity Addressables](https://docs.unity3d.com/ja/6000.0/Manual/com.unity.addressables.html)
...
- 组件套件支持定义、变体和实例覆盖；被覆盖的属性有明确标记，可逐项“应用到模板”或“恢复模板值”。
...
场景树沿用和二维一致的选择语法：可见、可选取、锁定分别控制；模型、设备、材质、镜头和交互可保存为模板或变体。双击模板实例进入隔离编辑，顶部显示“项目 / 场景 / 模板”面包屑和返回来源，实例差异可检查、应用或还原。
...
├── definitions[]: ComponentDefinition / Variant
├── data
│   ├── connections[]
│   ├── datasets[]
│   ├── transforms[]
...
`ComponentDefinition` 同时覆盖二维业务组件套件和三维设备/效果模板。实例只保存与定义不同的 override，定义升级时可预览影响、逐项接受或保持本地覆盖。`AssetManifestEntry` 至少包含逻辑地址、内容哈希、类型、依赖、转换产物、本地/远端位置和兼容版本；页面、场景和脚本只引用稳定资产 ID，不直接写易变的绝对路径或服务器 URL。
...
源文件/数据服务
  ...
  → 坐标、格式转换与轻量化
  → 生成 LOD/切片/缩略图/结构索引
  → 以稳定地址、内容哈希和依赖图进入资产目录
  → 被二维、三维、拓扑和发布配置复用
```

小资源可由浏览器上传后交给服务器转换；大 BIM/GIS/视频资源优先由 Tauri 本地 sidecar 预处理并断点上传产物。转换器必须是可替换任务，不把具体格式逻辑耦合进编辑器 UI。
...
项目提供可命名的 `PublicationProfile`，首批为“本地预览”和“服务器 Web”；接口预留“服务器云渲染”。Profile 只保存目标、入口页面、资源策略、环境变量引用和性能/兼容选项，不复制业务文档。开发、测试、生产可以共享内容但使用不同数据连接和资源地址，凭据仍只在服务器解析。
...
发布诊断必须列出入口页面/场景、资源依赖图、缺失或过期转换产物、未使用资源、包体积、预计首屏资源和运行时能力要求；日志能从界面中的问题定位到具体对象、资产或转换任务。
...
- 主题 token、组件定义/变体/实例覆盖、业务套件、隔离编辑、设计态五种数据状态。
...
- 地址化资产目录、依赖图与本地/服务器 Web 发布配置；支持大资源转换任务、内容去重和缺失依赖诊断。
...
- 三维对象/设备模板、变体和实例覆盖；隔离编辑与影响范围预览。
- 数据标签、三维图表、热力、流线、路径、告警效果和状态绑定。
- BIM/GIS 导入转换、3D Tiles/分块/LOD/按需加载、GPU/内存预算、后台 Worker。
- BIM 版本对比、问题/视点、碰撞报告和空间关系继续深化。
- 抽象渲染会话接口，验证超大场景使用服务器云渲染的可插拔方案，但不影响默认本地 WebGL 路径。
...
- 模板市场/私有组件仓库、依赖锁定和签名。
- 可保存/切换的工作区布局、扩展面板、资源转换器插件和拓扑编辑能力。
...
- 发布前必须解析完整资产依赖图；缺失资源、绝对本地路径、过期转换产物和不兼容运行时均能定位到具体对象。
...
10. `EPIC-010` 地址化资产目录、转换任务、依赖图和发布配置。
11. `EPIC-011` 组件/设备定义、变体、实例覆盖与隔离编辑。

实施顺序固定为 001 → 002 → 003/004 → 005/011 → 006/009 → 007/010 → 008。二维组件扩容和三维特效扩容必须建立在统一模型之后，避免继续向当前 overlay 和 `App.tsx` 堆功能。

## 14. 立即执行的下一步

先完成 M0 的技术设计，不直接大改 UI：

1. 定稿 `ApplicationDocument`、对象 ID、模板/实例覆盖、资产引用、迁移和发布边界。
