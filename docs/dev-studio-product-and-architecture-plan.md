...
- 工业 CAD 格式范围聚焦 RVT、IFC、STEP/STP、DWG 与 DXF；2026-08-26 决定移除 Parasolid x_t/x_b 与 JT，不保留未验证入口。
...
- 增加可独立启停的 `AskDataPlugin`。它参考本地 FATHOM 的受控问数契约，但不复制其 Python 语义平台：自然语言先编译为闭合的 `AskDataPlan`，只允许引用当前项目中已授权的指标、对象、维度、过滤、时间范围和比较方式，再由固定模板及参数绑定查询 M3 Data Hub。结果必须包含答案、结构化数据、图表建议、Evidence、Trace ID、语义版本、数据新鲜度、质量警告和推荐追问；未知指标、越权对象或缺少事实时澄清或阻断，禁止模型生成或执行任意 SQL。首版以 TypeScript 最小 Provider 为主，未来可选 `FathomHttpProvider` 通过 `/query/ask` 接入完整服务。
...
边界与依赖：M10 依赖 M3 的数据契约、M4 的行为脚本与 Scene SDK、M6 的插件/权限/版本体系、M8 的仿真证据和 M9 的版本文档；不做开放域问答、完整语义中台、自动训练平台、通用 Agent 编排、任意 Shell/文件/网络操作、无人确认发布、真实设备自动控制或以 AI 输出替代确定性仿真。没有稳定公开命令/API 的编辑能力不得靠 AI 绕过架构补洞。IoT-NB 与 FATHOM 均为同一用户开发，可直接复用代码；接入时仍按当前公共契约提取小模块、保留来源与测试，不把其 Tauri/Rust/Edge、完整 Python 语义平台、Dify 镜像或独立 UI 整包并入 iTwin Studio。
