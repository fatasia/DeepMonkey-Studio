...
Industrial Studio 应定位为“一套创作内核、两种使用方式、一个发布运行时”的 BIM 数字孪生应用开发平台：
...
Industrial Studio 不是重型地理空间平台，也不是复刻 Unity 的游戏引擎。当前明确保留：
...
| 能力轴 | 主要对标 | Industrial Studio 取胜方式 |
| --- | --- | --- |
...
- 学 ThingJS 的连续生产链、场景 API 和插件化，但 Industrial Studio 的二维页面、三维场景和数据必须由一个 `ApplicationDocument` 原子交付。
...
| 能力面 | Industrial Studio 必须达到的公开能力 | 验收方式 |
| --- | --- | --- |
...
从 M4 开始增加一条不可降级的产品门槛：**在项目锁定的 Three.js 版本中，能够通过 Three.js 文档化公开 API 在目标浏览器实现的能力，必须能够在 Industrial Studio 中通过 `SceneCapabilitySDK`、行为脚本或 Scene Extension 插件简单实现，并且不得修改编辑器核心代码。** 常见能力必须优先提供可发现、可撤销、可预览的编辑器操作；长尾和底层能力允许进入可信扩展，但扩展仍使用版本化公开上下文，不得导入 `ViewerEngine`、React 状态或其他私有模块。
...
| 典型迁移 | 同目录保存 Three.js 原始样例、Industrial Studio 实现、迁移说明和预期截图/数据 | 至少覆盖对象批量创建与层级、相机飞行与射线拾取、自定义 Mesh/材质、GLTF 动画、实例化、后处理/TSL、第一人称控制、AGV/机器人时间驱动八类样例 | 增加自定义 Loader/转换结果、GeoAnchor/地图叠加、拓扑与三维联动样例 | 抽取为可安装示例插件和 SDK 回归夹具；全部不改核心即可启停 |
...
外部身份只负责证明“是谁”，项目、场景、数据和发布权限仍由 Industrial Studio 内部用户体系决定。建议新增：
...
- 使用一次性 `state` 防 CSRF；支持时启用 PKCE 和 `nonce`；OAuth 回调后签发 Industrial Studio 自己的短期会话。
...
- 浏览器永远只访问 Industrial Studio 同源 `ConnectorGateway`：HTTP 由服务器代理并返回统一错误信封，外部 WebSocket 由服务器建立、限流和重连，再复用一个应用级同源 WebSocket channel 向多个 2D/3D 订阅者分发。发布文档不保存密码、Token 或私有 Header；网关从服务器凭据引用解析。管理员可配置出站域名/IP/端口与私网访问策略，并防止 DNS 重绑定，避免“解决 CORS”变成无约束服务器代理。
