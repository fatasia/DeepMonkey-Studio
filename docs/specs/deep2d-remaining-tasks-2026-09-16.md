...
| P1-03 | ~~**混合值列式分块**~~ **已完成(2026-09-17 接线收尾)**:存储层(2026-09-16)+ 数据通道接线——`ChartRuntime` 增每数据集分块窗口镜像 `data_windows`(runtime 态,不动 ChartIR JSON 合同),`update_data` AppendWindow 全部走 `ChunkedRows::append_window`(追加=行块 Arc 入列/淘汰=整块回收/跨块边界=仅首块写时复制),镜像 get-clone 局部推进、提交才落地,校验/几何失败路径零污染并有测试锁定;Replace 清镜像、全量 `replace()` 作废全部镜像。与未分块参考实现逐值一致的全链测试(跨块、双整块淘汰+边界收缩、空批缩窗、replace 重建、同批混合多数据集)+ 克隆引用计数共享证明(20_000 行 3 块镜像,克隆后每块 count+1,滚动后保留块 count≥2、分离/新增块 count=1)。lib 341 通过/chart 家族 215 通过/clippy -D warnings --all-targets --all-features/fmt 全绿;性能对比按任务边界留给 P1-04 后续。 |
...
| ID | 剩余任务、已有部分与完成条件 |
| --- | --- |
| P1-12 | ~~**sim 普通窗口与失败恢复**~~ **已完成(2026-09-16)**:`pump()` 唯一推进实现(渲染 gate 在 prepare 前,失败不碰游标)+`wake_delay()` 纯调度数学;`--smoke-chart-sim` 升级为 8 阶段注入(正常视口/空闲/到期/积压追赶/失败注入/恢复续跑/取消),5 项固定时钟生命周期测试+真实 GPU 冒烟。**剩余已完成(2026-09-18)**:P0-05 Web真实GPU矩阵(`scripts/verify-p05-web-matrix*`,6轮60断言)即真实换包端到端GPU注入——连续换包hash独立/旧帧release+dispose/迟到候选基线像素diff=0保持可见/取消0分配,证据`test-output/p05-web-matrix-20260918/`;Native侧换包通路既有证据(候选抢占/热更新恢复,ledger native-full-retry-reuse行)。[台账](active-task-recovery-ledger.md) |
| P1-13 | **宿主数据源状态机**——第一批已完成(2026-09-16):`chart/data_source.rs` 离线优先状态机(Disconnected/Connecting/Connected/Retrying/Closed,注入时钟+Transport trait 零真实 IO),版本治理(迟到/重复/串源拒绝)、有界环形离线缓存+水位续传对接、凭据类型上不出现;CAS 经 payload→ChartDataMessage 适配对接真 ChartRuntime。11 项测试。**第二批已完成(2026-09-16 接手会话)**:①**缓存字节预算**——`cache_byte_budget` 与条数上限共同生效(谁先触顶谁淘汰),单条超预算不入缓存但照常交付(`cache_oversized` 计数),`cache_bytes()` 可观测;②**宿主静默窗口**——`idle_deadline_ms` + `last_activity` + `poll_idle_deadline`,覆盖同步 Transport 下「连接建立后无活动」这一唯一可观察的死链形态(握手驻留要等真实异步绑定),收发事件刷新窗口、`on_receive_at` 带时间戳重载保持既有调用方零改动。6 项新测试(共 17 项)。**修订说明**:任务原写「宿主侧 deadline」,实测同步 Transport 下 `connect()` 立即返回、不存在可观察的在途窗口,故口径改为静默窗口并如实记录。**第三批已完成(2026-09-18)**:受控 HTTP/1.1 Transport(std::net零新依赖,`http_transport.rs`,chunked/重定向/非2xx/预算fail-closed)+宿主泵(`http_pump.rs` poll_http_once 自动重连/send失败消化为退避/响应经版本治理on_receive_at泵入),8+4项真回环socket测试。[spec](deep2d-http-transport-2026-09-18.md) **剩余**:TLS与订阅式明示后续。 |
...
| P1-22 | ~~**统一固定步回放**~~ **已完成(2026-09-17)**:`src/replay.rs`+`replay_tests.rs`——固定时钟扩展到行为/动画/随机种子(SplitMix64)与输入事件,命令序列/状态轨迹/display hash对拍,网络副作用仅显式replay fixture,过期命令拒绝不改旧epoch。[台账](active-task-recovery-ledger.md) |
| P1-23 | ~~**N1版本化适配器**~~ **已完成(2026-09-18)**:Native adapter_n1(Rust)+TS侧消费(`packages/deep-engine/src/adapterN1`,9文件)——wire合同逐字段对齐schema.rs,canonicalJson复刻serde_json+ryu-pretty,SVG白名单禁曲线圆弧;四类fixture digest黄金对拍全中Native固化值,58项测试+fail-closed 15+路径。[台账](active-task-recovery-ledger.md) **边界**:渲染仍在Native,范围外元素blocked;TS计数上限收紧多拒方向安全。 |

## 本轮待办：P2 有淘汰条件的兼容实验

| ID | 剩余任务与完成条件 |
| --- | --- |
| P2-01 | **兼容开关和隔离边界**——**无脚本首切片已完成(2026-09-17，[spec](deep2d-p2-01-x-compat-isolation-2026-09-17.md))**：`compat_x` 明确 N0/X lane 与关闭开关；封闭调用树只读宿主注入的资源/时钟/种子化随机/事件，无 DOM/GPU/文件/网络对象；CPU work units、内存、调用深度、消息条数/字节、wall-clock 均硬拒绝；evaluate→publish 两阶段重查取消/迟到/旧 epoch。**剩余**：独立受限进程、进程崩溃/终止隔离与正式 IPC；本切片不运行真实 JS、不宣称 ZRender/ECharts 兼容。**三要素收口已完成(2026-09-17)**：独立受限进程=单次 JSON IPC+XSF1 持续会话+零 capability LPAC+Job 内存/CPU/进程数限额+portable 包固定 worker；崩溃/终止隔离补真实测试 `worker_self_termination_reaps_live_descendant_tree`（worker 非零码自终止、子/孙持继承管道存活，宿主收 Crashed 后逐 PID 核验整树回收）；正式 IPC 补崩溃重启用例 `crashed_session_is_rebuilt_with_new_pid_and_old_epoch_stays_rejected`（TerminateProcess 后会话中毒回收，重建新 PID 会话输出与本地求值一致、旧 epoch 发布被拒）。门禁：lib 362/bin 128、compat_x_process 7/windows_job 5/lpac_session 6/lpac 4/scheduler_cli 1/续期 1 全过，clippy `-D warnings --all-targets --all-features` 与 fmt 通过。**修正**：先前记录的 compat_x_lpac 4 项本机 ACCESS_DENIED 非环境限制——普通 examples 构建以动态 CRT 覆盖静态探针致 `0xc0000022`（DLL ACL），按复跑清单重编静态 CRT 三件套后 4/4 通过。仍不运行真实 JS、不宣称 ZRender/ECharts 兼容；LPAC 文件/网络负向权限矩阵、产品发布/编辑器接线、签名与更新撤销继续待办。 |
| P2-02 | **ZRender Painter 批量命令实验——受限动态 bar 切片已完成(2026-09-17，[spec](deep2d-p2-02-zrender-painter-batch-experiment-2026-09-17.md))**：真实 ECharts/ZRender 6.1.0 无 DOM SVG SSR，同一实例输出 stable-id rect upsert/remove；输入与完整输出快照 SHA-256 已冻结并 10/10 双跑一致；预算/错误保持上一 epoch/hash。formatter、图片、tooltip、动画、非 rect displayable 继续明确 blocked，未认证完整 Painter/ECharts 生态；独立受限进程仍归 P2-01 剩余。**ChartIR→期望 rect 映射认证切片已完成(2026-09-18，[spec](deep2d-p2-02-zrender-chartir-rect-certification-2026-09-18.md))**：期望 bar rect 集合由 chart-web-geometry golden 同一参照（chartPlot/cartesian/zoomDomain）从 ChartIR 确定性推导，真实 ECharts 6.1.0 SSR 实例逐值对拍（容差同 golden 1e-9 相对口径），动态通道（同形状数据 replace/类目对齐 x 窗口/y 窗口插值）4 帧全 matched、跨实例双跑 hash 流一致；实测发现 ECharts dataZoom 类目窗口语义（(count-1) 取整、端点包含）与 Native 行带公式不一致 → 窗口语义由 lane 重投影承担，非对齐窗口与值越界帧整体拒绝；formatter/image/tooltip/animation/非 rect 系列/legend/类目增删九项 blocked 全部有测试证据；证据落 test-output/p02-zrender-20260918/；认证版本锁定 ECharts/ZRender 6.1.0，升级即失效须重认证。 |
...
| P2-04 | **首版认证与淘汰报告已完成（2026-09-17，[report](deep2d-p2-certification-report-2026-09-17.md)）**：冻结 X v1、ZRender 6.1.0 动态 bar、富文本/IME 三条窄通道的版本、hash、预算、timeout/epoch 保持、准入与淘汰条件；HTML rich text、ECharts-GL、任意扩展继续 blocked。报告完成不关闭 P2-01～03 的独立进程、完整 Painter 与真实 IME/像素证据。 |
...
| ID | 剩余任务与完成条件 |
| --- | --- |
| P3-01 | **对象级内容寻址delta**：复用现有包diff/watch/LKG，接document/data/layout/resource增量、资源依赖闭包、下载/校验/预热/present后提交；迟到delta、丢包、schema不兼容保留旧版并给诊断。 **标准包 watch 已完成（2026-09-18，[报告](deep2d-delta-watch-closure-2026-09-18.md)）**：真实文件监听→生产事件→scene/Deep2D GPU present→完整 LKG；16 项合同、坏 hash/schema/截断/缺操作、迟到基线 CAS、未呈现不提交与 delta 源跨重启恢复通过。实验 X v6 保持专用入口，不扩称任意格式 delta。 |
...
| P3-04 | **Windows设备/驱动能力矩阵**：覆盖已声明Windows目标和WebGPU/Native后端、字体/DPI/显存限制、设备丢失和恢复。新增后端的lowering/宿主/验证三部分合同要明确；不因此重启非Windows实现。 **当前单机切片已完成（2026-09-18，[报告](deep2d-delta-watch-closure-2026-09-18.md)）**：RTX 4060 Laptop Vulkan/DX12 6 后端/尺寸格+4 字体格，12 份 RGBA 与原证据 SHA-256 一致；隔离 device.destroy→重建两后端像素一致。harness 主入口 956→188 行，阈值未变；多设备与真实驱动 TDR 未扩大声明。 |
...
| V-04 | 短时稳定性、内存/资源峰值、设备丢失、失败注入、取消和回滚，端到端性能无回退；8小时soak已取消。 **当前版本短时/恢复切片已完成（2026-09-18，[报告](deep2d-v04-short-stability-2026-09-18.md)）**：20 轮七场景 4,900 帧/157.6 秒、内存/资源原始采样、产品失败恢复 5/5、Vulkan/DX12 实际 destroy→重建像素零差异。**本轮待办**：同机成对测量确认 CPU 成本增长，继续 profile 与等价缓存收敛；性能无回退尚未通过。实际驱动 TDR 未由隔离 device.destroy 代替。 |
