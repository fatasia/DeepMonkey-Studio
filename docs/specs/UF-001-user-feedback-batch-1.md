...
a) **[查]** 3D 编辑器窗口缩小后，滚动会**反复跳**：2026-09-05 夜间深测（`u19a-3d-body-scroll.mjs`）在 900–1440px 全梯度未复现——body 滚动恒 0、无页面溢出、canvas 尺寸平滑跟随、零控制台错误。疑似已由 Codex `df9a14f` 的仿真浮窗约束修复覆盖（当时浮窗可被拖出工作区引发跳动）。**标记：当前版本未复现，保留用户复验；若再现在 escalation 单独定位。**
