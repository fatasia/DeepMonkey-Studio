...
发布冻结在 `Uint8Array.from` 之前校验类型、单资源64MiB和总资源256MiB预算；此前超大资源会先被完整复制再拒绝。冻结后SHA只计算一次，供部署catalog与清单复用。没有改变预算值、内容身份或读后隔离。

新增测试断言超额/空/非字节数据不调用复制、1MiB backing buffer上的3字节视图仅冻结3字节、改原字节不影响候选、catalog不匹配拒绝以及存储返回时取消不复制。冻结/生产资源/候选/归档4文件54项与API类型检查通过。未将此局部优化等同于整个发布链峰值RSS测量。

按engineering-taste执行同族检查与前置预算验证，十维自评均9；无UI、依赖、权限或存储拓扑变化。
