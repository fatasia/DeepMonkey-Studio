...
### 8.3b R6-2 · 引擎层 CPU 帧准备时间优化（2026-09-19 用户追加立卡；同日整合另一会话的实测归因与 A–D 组方案）

**实测归因约束（已核实的既有数字）**：纯文字变更 7.9ms（文字管线地板≈8ms）；单系列更新 12.2ms（文字 8 + 局部几何 ~4）；全量数据替换 35.7ms（几何+数据重建≈28ms）；缓存命中 0.25ms。两个关键信号：**过度失效**（单系列更新 vs 缓存命中 = 50:1，改一条系列重建了远超必要的东西——脏区粒度细化本身就是大牌）；**GPU 99.6% 空闲**（0.07ms/16.6ms）。

**三杠杆**：GPU 近全闲、我们拥有编译链（编译期知道全部内容）、我们拥有逐位确定性门禁（R2/R3）。开创性方案全部从杠杆长出，不磨 CPU 劣势（vs 原生栅格化 300:1），而是把推导驱逐到 GPU/编译期：

- **C3 摊销重建 + 脏区细化**（第一优先,1–2 天）：全量替换按每帧 4ms 预算逐 widget 换入（deck swap 粒度从整 deck 细化到单 widget）,35.7ms 尖刺变 9 帧平滑;P95 直接受益,正是 A 组对标口径。纯调度,不过视觉门禁。
- **A1 MSDF 文字 + 编译期 CJK 字形子集烘焙**（1–2 周）：运行时 CPU 栅格化归零（发布时扫描文档字符集,只烘焙实际用到的字形——工业仪表盘通常几百汉字）;文字 8ms→~0.05ms GPU。Three/Babylon 抄不了——他们没有编译链。视觉基线变化走 R2/R3 门禁重定基线。
- **C2 并行 encoder 农场**（3–5 天）：widget 级无依赖,按 widget 切 job 多线程 encode、主线程按序 submit;帧摘要必须逐位不变（顺序敏感）。
- **A2/A3 图表几何与筛选/聚合 compute 化**（2–4 周,DCIR v1 buffer I/O 先行）：数据更新=裸行写字进 storage buffer,compute 做三角化/聚合（整数/定序归约已在 DCIR v0 白名单）;几何 28ms→<1ms。
- **C1 投机帧准备**（独有玩法）：数据按节拍到达时 worker 线程提前用预测输入准备下一帧,digest 比对即全部验证,失败只浪费一次离线计算——确定性门禁就是"预测器冲刷"。首个实现必须带命中率统计。
- **B1/B2 帧计划 IR + 静态场景编译到骨头**（大,归 R4/R8 战役）："这一帧画什么"的决策序列化成重放脚本（内容哈希键）,0.25ms 缓存路径覆盖面从无变更扩到结构性变更;PVS/HLOD/meshlet 预分区/间接缓冲预生成。
- **D 组数据通路**（顺手做）：uniform 环形竞技场、热路径 JSON→二进制、staging ring 三缓冲——随 R4/B05 落地。

**排序即上表顺序;诚实边界**：A/B 组收益预估建立在场景差归因的强假设上,**细分计时落地前是假设**;C1 命中率无数据,首个实现带统计;所有动渲染语义的(A1/A2/A3)必须过逐位帧摘要门禁后重定基线,不许静默改。第一步 = 半天的 prepare 细分计时(决定 A1/A2 谁先上)。
...
> **2026-09-19 R6-0 裁决(另一窗口车道完成,证据 test-output/r6-0-wgpu-overhead-20260919/)**:wgpu 30.0.1 features 干净、生产形态 1 encoder+1 submit、提交成本 0.207ms ≈ 帧预算 **3%(<10% 档)**——**判定:引擎层消化即可,不动底座;R6-1 热点档/白盒档封存**(数据不支持;R4 万灯落地后按同口径复测)。encode 内部大头是 `encoder.finish()`(63-66%),非 validation;唯一纯 flag 项(关 VALIDATION_INDIRECT_CALL,0.07-0.09ms/帧)暂缓留作 R4 后验证。零成本档审计:无可实施净收益项(profile 已最优)。→ 用户"未验证禁改 wgpu"指令自然满足:不需要 fork。62% 早前数字是 cpu-submit 通道口径(含 present 等),细分后修正为 3%。
>
> **2026-09-19 用户硬性指令**:wgpu 底层代码在未验证前**禁止修改**。已核实当前状态：wgpu 30.0.1 直接取自 crates.io（Cargo.lock source=registry,无 [patch]/无 fork/无本地路径），本项目对 wgpu 零改动。热点档/白盒档启动的先决条件 = ①R6-0 细分测量完成并给出占比判定，②确定性门禁全绿，③用户明确批准 fork 行为。三条件缺一不可。
