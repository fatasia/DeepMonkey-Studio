...
### 8.3b R6-2 · 引擎层 CPU 帧准备时间优化（2026-09-19 用户追加立卡）

定义：GPU 开跑前的全部 CPU 工作——场景图遍历、剔除决策（frustum/HiZ 结果消费）、绘制命令编码（encoder/record）、资源绑定与上传准备、动画/变换更新。R6-0 实测的 CPU encode+submit 占比即本卡的度量主体。

基线方法：帧 CPU 时间细分（遍历/剔除/编码/提交四段 Instant 计时，样本≥5 取中位数），六类负载逐档记账；与 R4 GPU-driven 剔除联动（剔除下推 GPU 后 CPU 段应显著缩短，缩短量即 R4 的 CPU 收益证据）。

优化动作（按收益/成本排序，全部过确定性门禁）：
1. 遍历与剔除：脏域增量遍历（复用 B02 语义，只走变更子树）、frustum 预筛选在 CPU 端保留最小集（可见性最终裁决仍在 GPU HiZ）；
2. 编码：render bundle/间接绘制预编码（静态子图一次编码多次重放）、编码器复用池；
3. 绑定与上传：bind group 缓存按（材质,布局）指纹、上传暂存区环形复用、双缓冲映射消等待；
4. 多线程准备（视 wgpu 线性提交限制 #5525 实测结果决定）：按 pass 并行准备命令、单点提交。

验收：四段细分基线→目标（对位 Unity 同场景 CPU 帧时间）→季度重测；BIM 大场景（万级对象）CPU 准备时间进入 R6 性能账本；未测不写达标。
...
### R6-1 · wgpu 白盒优化卡（2026-09-19 立卡，DeepSeek 式打法）

前提：wgpu 是自有 Rust 代码（MIT/Apache），可 fork/patch/上游；用户的 DeepSeek-CUDA 类比成立——把别人当黑盒的层打开当白盒改。四档推进，**每档必须通过 R2/R3 逐位帧摘要门禁后才可合入**（确定性合同=补丁安全网，这是独有优势）：

1. 零成本档：release 关校验、power preference、子分配参数；
2. 热点档：R6-0 实测定位（队列串行化 #5525 / barrier 跟踪 / validation / Naga 编译）→ fork 定点补丁，负载封闭（工业场景）使风险可控；
3. 编译器档：Naga 代码生成调优 + 热点 kernel SPIR-V 后处理（spirv-opt/手工 patch = 我们的 PTX 层），尽量上游回 wgpu 社区；
4. 白盒档：wgpu-hal 直下最热提交通路（与既有逃生舱合并）。

纪律：测量先行（R6-0 占比数据决定档位顺序）；补丁全走 fork 并尽量上游，降低维护税；渲染语义零变化是合入硬门。
