...
## 9. 2026-09-05 场景物流最小闭环回填

本节覆盖 §8 中对应的“本轮待办”，不表示全部 SIM-1a 或 Siemens 对标完成。

- 已完成：新增 `flowNode` 直接复用 PlantLiteNode 合同，支持对象绑定来料源、队列、工位和产出汇；已有 `flowLink` 显式连接，角色参数与连线沿原 scene snapshot/历史/导入重绑链路保存。空图、断链、重复对象角色和非法图报错，不自动绑定默认对象或自动猜流程。
- 已完成：`scenePlantModel` 只作合同适配，使用原图校验与原 DES；可选 `model.sceneBinding` 保存实际场景节点/对象/三维坐标快照，加入原 Study 指纹和复现。旧无绑定模型兼容；绑定元数据亦做完整性校验，不改算法、不增结果仓。
- 已完成：场景内调用原 run/reproduce API，固定三个统计重复、可调时长/种子，结果显示原 Study id/KPI；快速编辑不覆盖原高级运营工作台（班次、故障、资源池等仍可进入）。运行完成自动折叠作者窗，留出视口。
- 已完成：原 `PlantLitePlayback` 单时钟输出帧，时间线增加仿真轨道，播放/暂停/倍速/定位沿现有适配器；原场景动画单独编辑，不写模型变换。临时 helper 使用固定 18 点缓冲区显示真实 DES 事件位置，关闭时间线/面板、切场景或引擎即清理；HUD 明确标“轨迹样本”。**瞬时连线没有被包装成物理 AGV 路径，事件点位不是空间热力。**
- 已完成：重复请求/卸载取消保护，陈旧全图替换冲突阻止 dirty；浏览器停止等待明确提示服务端若已完成仍可能有 Study。配置与运行状态分离，不把瞬态覆盖层保存。

验证：Web 聚焦 6 文件 24 项（桥接、失效锚点、重复角色、旧模型、原回放选择器、overlay 固定缓冲区释放、导入重绑、陈旧编辑、面板尺寸）；API 实际 DES 与模型/轨迹/统计/指纹复现 2 项；Web typecheck 通过。全仓类型/构建由主代理统一记录。

最终真实浏览器证据：`test-output/codex-2026-09-05/scene-plant-flow-2R3HHn/`，dark/light × 1440/980 四组退出 0；绑定四对象/三边→队列参数→保存刷新→正式 Study→单时钟暂停→原快照复现→关闭清理；确认原对象变换与持久化内容无 helper 污染。新建模文字对比度最低 dark 8.218/light 8.259，仿真时间线文字最低 dark **7.569**/light **7.525**，零意外控制台错误，仅 dark1440 一组既有显卡编译精度 warning 单列记录。已目视浅 980 参数展开/回放及深 1440 回放截图，确认浅色时间线不再继承旧硬编码暗色、标题按钮不挤在一起，运行后作者窗折叠留出视口。旧 `dwVwxQ` 仅保留为主题修复前功能基线，不能替代最终图。

剩余分类：

- 本轮待办：停靠/autohide、关闭后历史回放恢复、更完整运输资源/连续 AGV 轨迹/空间热力、跨仿真域的时间线轨道；高级运营工作台原独立回放尚未整体迁入统一轨道，当前单播放器结论只涵盖场景快速物流路径。SIM-1b/c/d 未在本批扩大。
- 项目级后验收：真实客户场景/大型网络与复杂参数组合、异常网络下服务端最终保存状态核对；固定四对象验证不扩大承诺。
- 明确排除：认证动力学、OLP、厂商控制器矩阵、新算法、新结果存储。

完整文件/恢复入口：`docs/SIM-1a-in-progress-checkpoint-2026-09-05.md`。浏览器未读用户 token、未访问原业务数据；无 push，无账号/环境/存储拓扑变更。

## 10. 2026-09-06 停靠工作台增量

本批不重做 §9 已有场景物流闭环，只补布局能力。左右停靠 / 浮动 / 44px 收起条、键盘停靠和缩放、Esc 保留表单并收起已实现；真实 `studio-scene-surface` 按面板宽度让位，面板与表单始终保留在原宿主节点，不通过换 key / Portal 重挂载。

布局偏好 v2 兼容 v1 浮动矩形，仅保存尺寸 / 位置 / 方向，不另存场景仿真输入、临时运行或 Study。收起仅当前会话，关闭仍沿原卸载清理；再次打开默认展开并保留停靠方向。窄窗停靠临时让场景树与检查器收起，恢复浮动 / 关闭后恢复原偏好。**这不等于关闭后历史回放恢复，也不是悬停式自动展开。**

r5 两轮四组 `simulation-docking-pl7SSK` 行为通过，截图亲审发现的工具坞裁切、分隔条遮挡、菜单锚点和极窄标准视图遮挡已同族修补。**最终 r8 `simulation-docking-l0h7w8` 两轮 dark1440 / light980 四组 exit0**：真实 canvas 让位 / 恢复、同一 canvas / input DOM、原表单保留、菜单真实切换、6 个方向按钮命中、拖动 +20px、布局阶段零 API 写入、完整保存基线后无恢复误提示。全部最终截图已亲审；最低正文对比度 dark6.00 / light5.00。十维自评和严格边界见 `../simulation-docking-verification-2026-09-06.md`，不外推全站 / 全套仿真能力。

相邻真实仿真 r5 `scene-plant-flow-ScU6Mu` 与 r7 `scene-plant-flow-ApwMDa` 各 dark/light × 1440/980 四组通过：保存刷新→正式 DES Study→单时间线→原快照复现→关闭清理仍成立，固定对象变换及持久化内容无临时 helper 污染。r8 最后仅补标准视图布局，不把既有 Study 产物改称 r8。

恢复兼容新增发现：完整保存快照因对象键序而误判恢复已修（先红测后递归规范对象键、数组 / 实际数值不改）；稀疏旧快照缺省默认字段和相机浮点尾差仍为本轮兼容待办，不把输入保持或键序修复扩大为任意旧场景无恢复提示。

本轮待办保留：关闭后历史回放恢复、连续 AGV / 空间热力、跨域时间线及 SIM-1b/c/d 后续深化；不以停靠替代仿真能力。认证动力学 / OLP / 控制器矩阵 / 新算法等排除项不变，用户暂停的 AskData / 语义公开治理和专项性能 / 全仓长文件治理不重开。
