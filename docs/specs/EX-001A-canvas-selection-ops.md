...
2026-09-05 Codex 回填：已提交的 Alt 拖拽/右键选层保持，本轮不重复实现。修正框选读取滚动容器而非 artboard 的坐标偏差；底部页签去除左侧重复入口。`u112b-selection-verify.mjs` 用独立内存夹具跑 1440/980 × dark/light，均实测框选 9、Shift 加选 2、Ctrl 切换 1，页面增删复制/最后一页保护通过；`u116-ctrlwheel.mjs` 同四组合 10%→11%、60 帧零漂移。Alt 拖拽撤销条数与遮挡选层的专项浏览器证据仍为本轮待办，不能由框选门禁代替。
