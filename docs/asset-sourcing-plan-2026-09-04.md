...
## 1. 数量与质量目标（行业主流口径，2026-09-04 用户明确：素材=二维组件+模型+模板全口径，参照山海鲸/ThingJS/帆软）

三条建设线，目标与进度：

| 线 | 类别 | 本轮前 | 当前（2026-09-04 扩容后） | 目标 | 建设方式 |
|---|---|---:|---:|---:|---|
| 下载线 | HDRI 环境 | 12 | **55** | 40+ ✅ 已超 | Poly Haven CC0（运营态工厂/机房/仓储/维修/基础设施/夜景/阴天精选） |
| 下载线 | PBR 材质 | 16 | **72** | 60+ ✅ 已超 | Poly Haven CC0（金属/瓦楞板/锈蚀/混凝土/沥青/砖/木地板/瓷砖/屋面/砾石） |
| 下载线 | 3D 开放包 | 14 包 | **16 包**（+家具 140 件/站体 150 件） | 按行业缺口持续补 | Kenney CC0 镜像；后续 Poly Pizza/Sketchfab-CC0（S3-A4/A5） |
| 下载线 | 3D 工业模型 GLB | 1,482 | 1,482 | 3,000+ | source-b 通道（Poly Pizza/Sketchfab），夜间批次 |
| 代码线 | 2D 组件预设 | 156 | 156 | **300+** | S3-B 规格：ECharts 变体（雷达/旭日/仪表变体/极坐标/散点簇/热力）+ 工业图元（管道/阀门/设备符号/状态灯/进度/监控框）SVG 装饰；不新增依赖 |
| 内容线 | 看板模板 | 120（12 行业×10） | 120 | **300+** | S3-C 规格：行业扩容（电力/水务/化工/烟草/汽车/物流/食品/制药/矿业/港口…每行业 5-10 页可编辑、可绑语义模型） |

质量底线不变（AGENTS.md §3.4）：只有来源、许可、结构、缩略图和质量门禁全过才进公共包；下载量不冒充商业质量。

### 2026-09-04 首轮执行结果（已验证）

- `pnpm assets:sync:environment-materials`：477 文件 / 1.55 GiB 新增，catalog 127 资产**全部 published**（必需贴图齐备、哈希完整），总量 1.96 GiB。
- `pnpm assets:sync:open-packs`：16 包（新增 furniture 4.9MiB、space-station 6.4MiB，均 CC0）。
- `pnpm assets:audit:test` 10/10 通过。API `assetLibraryRoutes` 直读 dataDir 目录，资源中心即刻可见。
