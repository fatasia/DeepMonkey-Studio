...
BIM Studio 当前大量路由、项目状态、场景运行时和 UI 集中在 `apps/web/src/App.tsx`，二维看板又依附于三维场景快照。继续直接增加二维编辑、桌面 Host、脚本和高级三维能力，会扩大修改影响面并形成循环依赖。
...
**一次性重写**：拒绝。回归面太大，旧项目兼容和持续交付风险不可控。
