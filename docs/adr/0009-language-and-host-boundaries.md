...
Deep Monkey Studio 同时包含 Web 编辑器、Node.js 服务器、Tauri 客户端、Revit 接入、插件、模型转换和可选仿真。若为每类能力随意引入语言和常驻服务，跨语言 Schema、错误协议、构建、部署、调试和升级成本会快速增加。项目要求高内聚、低耦合、代码可读并尽量减少代码量。
