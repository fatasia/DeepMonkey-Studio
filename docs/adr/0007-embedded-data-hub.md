...
Dev Studio 的主体是连贯的一体化 2D/3D 编辑器，但工业项目还需要接入数据库、HTTP、WebSocket、MQTT、OPC 等数据，进行过滤、映射、计算和状态逻辑，并向外提供 REST API 或实时 WebSocket。若二维、三维、拓扑和地图各自实现数据源与绑定，数据查询、公式、权限和故障语义会迅速分叉。
