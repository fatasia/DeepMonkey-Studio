...
Deep Monkey Studio 需要同时扩展实时渲染能力、工业数据接入和低代码看板。把 Node-RED 运行时、数据库驱动和流程编辑器直接打进 React/Three.js 前端会显著增加首屏体积、权限暴露和故障耦合；把每种协议直接写入查看器又会让场景代码无法维护。
...
- Node-RED 编辑器与 Dashboard 独立加载，使用与 Deep Monkey Studio 一致的深色主题；主应用仅提供入口、连接状态和场景数据桥。
