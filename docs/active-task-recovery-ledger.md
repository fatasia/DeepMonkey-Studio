...
- 2026-09-22 B5 多选 select 最小切片(工作树完成、提交待基链落库):`DashboardFrozenFilter` 新增 `multiSelect` 可选字段(serde default,旧包缺字段按单选解释、缺省序列化省略,读写双兼容);`DashboardRuntime` 新增 `selected_options: BTreeSet<usize>` 与 `toggle_filter_option`(已选→取消/未选→添加,每次以所有已选项 updates 按数据集求行交集重放,空选集恢复包内原始行,文本绑定目标沿用 `apply_input_value` 交集;事务保证失败不发布),多选列表点击经 `toggle_filter_pointer` 单事务同步悬停/焦点,单选 `selected_filter` 指针与全部单选路径逐位不变(单选包 toggle fail-closed)。新增 `filter_multi.rs`/`filter_multi_tests.rs`(单选兼容、多选交集与恢复、非法索引 fail-closed、指针一致性 4 项)随既有 filter 链 19/19、dashboard 同族 38 项、runtime_package 43 项全绿(`cargo test --lib`)。提交阻断:B5 叠加在并行会话已登记未提交的 Deep2D select-v1/文本输入混合链(约 40 个未提交/未跟踪文件,含 input_profiles 依赖)之上,B5-only hunk 无法独立编译;按"不整批纳入并行会话文件"纪律不代为整体提交,待该基链落库后按文件清单单独提交本切片,未 push。
- 2026-09-22 B3-c 物理可观测性收口：碰撞开始/结束事件经 Rapier `EventQueue`（autoDrain 关闭，固定步进追赶循环后统一 drain）解析为 `collisionStart`/`collisionEnd` 交互脚本触发，`payload.other` 携带对端对象 id（地面/不可解析为 null），脚本经 `ctx.event.payload.other` 读取；contract（`SceneInteractionTrigger`/校验白名单）、交互 UI 下拉与标签、脚本补全 BUILTIN_EVENTS、viewer 派发接线（`ActiveEvents.COLLISION_EVENTS`、句柄归属表、释放清理）全链贯通；新增纯逻辑模块 `rapierPhysicsCollisionEvents.ts` 及 5 项测试，contracts 定向 96 项、Web 物理与交互定向 21 项通过，contracts 纯净快照 tsc 与 Web tsc 中本切片文件零错误。代码内容已随并行会话提交 `025495c` 一并落地（该提交消息为 A4 粒子证据且混入 B2-a playbackRange 内容），按 B3-b 先例不重写历史、不补空提交，B3-c 归属以本条目为准；物理调试可视化（缺口 5）与 per-body 参数（缺口 4）仍为后续切片，未 push。
...
- **P0-1 精确盘点完成（2026-09-21 主线程）**：实例链 GPU-driven 基本完整——①非 LOD batch：GPU compute frustum+Hi-Z 时间性遮挡剔除+实例 compactor+indirect（≥64 实例，<64 直绘是已论证的成本权衡）；②LOD batch：独立 GPU LOD 链（PacketLodResources/PacketShadowLodResources/encodeIndependentLod）；③meshlet 层间接命令生成；④实例缓冲独立更新（OPT-01/03）。**真实增量收缩为**：a) 万级实例同配置端到端 CPU/GPU P50/P95/P99 证据（归批次 F）；b) preflightCullingBounds 动态信封的 CPU 逐实例成本确认（若超标则 GPU 化该信封计算）；c) deep-engine dist 重建已修复 validateLightIes 陈旧导出（sceneClientPackagePreparedNative 12 项恢复全过）。波次3 主体让位给 P0-2 可见性 buffer。
- **修复与批次 E 启动（至 034dc62，共 56 个本地提交）**：`e57f6c1` linear-depth COPY_SRC 漂移修复（renderTargets 实际创建+pbrOpaquePass/pbrPostProcessChain claims 与计划合同三处同步——plan↔actual fail-closed 对拍抓住，18 项过）；`b93ce6c` bindless 级 1 纹理数组（anchored WGSL 双轨+MaterialArrayIndices 独立 uniform+三层 fail-closed，默认关逐字节一致）；`034dc62` 批次 E 验收核对清单（8 项历史失败重放规格）。**在途子代理**：RayBackend TLAS WGSL 扩展、批次 E 双客户端打包执行（A1–A8 历史失败重放）。
- **A4 产物级复验 PASS（至 e0c8fc2+，批次 E A 组全闭环）**：工作台 EXE 内嵌 CSS 已加载让位规则（padding 56px）、overlap=0、三点全命中内容组件、「返回总览」真实点击成功；1200×800 回归 PASS。关键产物事实：three-webview EXE 编译期剪除 studio 入口（不受 A4 影响）；行为级载体=工作台 EXE。证据 batch-e-20260921-r2/a4-product-verify*。
- **deep-engine 基线失败全清（至 b9cf713，共 91 个本地提交）**：`b9cf713` 四项修复——①transient 池分桶键加 resourceId（OIT 三目标 usage 统一后同桶身份互换真回归，资源身份语义修复）②pbrTransparencyPass claims/acquire 与 linear-depth 合同同步 ③environment v7 fog 必填校验补齐（solidEnvironment 6/6）④authorGrid power-of-two 恢复——webgpu 全域 166 文件 1305 测试全绿（authorGrid/pbrTransparency/GridSession 全恢复）；`fe36bc4` lab:isolation 门禁更新（生产导出面审计 196 处引用通过）。**批次 E 四组（A/B/C/D）全闭环**。剩余批次 F 阻塞：App.tsx 805/r2_harness 996 行（并行 WIP 编辑中待收尾）。
- **批次 F 门禁 runner 就绪并验证（至 0a31331+，共 83 个本地提交）**：`scripts/batchFGateRun.mjs`（十一项序列逐条串行+JSON 汇总+--quick 模式；quick 真机验证 6 pass/3 failed——3 失败全为已知并行域）。剩余失败归属：App.tsx 805/r2_harness 996 行（并行 WIP 编辑中）、deep-p0 内 deep-engine 3 基线失败、lab:isolation 长期项（待拍板）。api.ts 拆分已修（696 行≤800）。
- **App.tsx 达标（798 行）+ 批次 F 正式门禁启动（95+ 提交）**：`2ffbaf4` 绑定行压缩零语义；source-size 仅剩 r2_harness 996 行（并行 WIP 编辑中，拆分待其收尾防冲突）。
- **批次 F 全序列门禁启动（92+ 提交）**：App.tsx 拆 sortScenesByTime（803 行，余 3 行为主壳解构无独立块，并行 +6 行 WIP 归位后自然达标）；全量 tsc 干净（软光栅接线收敛）；正式门禁经 `scripts/batchFGateRun.mjs` 全序列后台执行中（日志 test-output/batch-f-gate-*/）。**并行域遗留**：GridSession 尺寸校验半成品（M，未越界）、r2_harness 996 行（M，编辑中）。
- **批次 F 预检 B 类修复（至 98aa75a，共 80 个本地提交）**：web 22 失败已修（三测试文件 fake backend 补 setProbeClipmapEnabled——生产 backend 已有完整实现，纯 mock 漂移）；viewer 全量剩 4 失败全归并行 GridSession 在途域（尺寸校验半成品）。**批次 F 剩余待修**：api.ts 811 行超限（AI 切片增长需拆分）、deep-engine 3 失败+App.tsx/r2_harness 行数（并行域）、lab:isolation（长期）。
- **批次 F 预检结果入库 + clippy 债清理（至 690d6c7，共 78 个本地提交）**：批次 F 第一轮预检（test-output/final-gate-preflight-batchF-r1-20260920/，01-20 号日志）：repository/public-brand/build/scene-client/product-browser/webgpu/production-artifact/runtimePurity **PASS 无回归**；失败归因三类——A 既有基线（300 行 87 个、LPAC 12、lab:isolation 长期未入表）、**B 我方新增**（web 22 失败=setProbeClipmapEnabled mock 漂移+setProbeClipmap 尺寸文案、clippy ray_backend 8 错、api.ts 811 行超限）、C 并行在途（deep-engine 3 失败/solidEnvironment v7 测试先行/r2_harness 996 行）。**已修**：clippy 我方文件域清零（690d6c7，lib 462 过）；`229a02c`/`b830708` 已修 A5/A4。**待修队列**：web 22 mock 漂移（加 setProbeClipmapEnabled 或改 mock）、api.ts 拆分、deep-engine 3 失败（并行域）、App.tsx+r2_harness 行数（并行域）。
- **批次 E D 组三端性能对拍完成**：启动矩阵 Web load 中位 161ms（冷 202/热 149-184，evidence 5/5）/ WebView 渲染就绪中位 1709ms / Native 2434ms（RTX 4060 同机）；Web 三维场景页 rAF 2×10s **avgFps=144（vsync 锁定）、帧间隔 P50 6.9/P95 7.1/P99 7.2ms、长帧=0**；口径差异已写入 result JSON。证据 batch-e-20260921-r2/d-group/。
- **批次 E C 组三端对拍完成**：同场景 1200×800 三端截图 SSIM 矩阵——Native↔WebView 0.7983 / WebView↔Web 0.6434 / Native↔Web 0.5669，**全部如实分级 diverged**（未放宽）；根因=覆盖层有无/宿主布局/引擎差异/DPR 1.25 vs 1（非语义分歧）；二级视口对齐诊断 WebView canvas↔Web canvas **0.8754**（材质/光照/几何语义一致，残差=纵横比 3.4% 拉伸+DPR 光栅化）。证据 batch-e-20260921-r2/c-group/。结论：三端语义一致；像素级收敛路线=对齐 DPR 与视口裁剪后重对拍（后续批次）。
- **批次 E B 组闭环（75+ 提交）**：22 项操作矩阵 **17 pass / 4 by-design / 1 observed / 0 fail**（真实鼠标键盘+像素差分：旋转 58%/缩放/平移/拾取轮廓/显隐近全还原/图表 tooltip/Tab 焦点闭环；含新增三维场景页 revision 3 发布）；全程 0 console 错误。证据 batch-e-20260921-r2/b-group-result.json（24 截图）。**在途子代理**：批次 F 第一轮全量门禁预检、批次 E C 组双端一致性（待派/在途）。
- **软阴影消费者落地 + 批次 F 预检启动（至 0605d68，共 75 个本地提交）**：`0605d68` 软阴影射线扩展（RayBackend 第三消费者：半影带遮挡射线 PCSS 校正/替代双档、三道闸门、16 用例含真实 TLAS 两级端到端）；`62d6fbb`/`777f490` 启动证据组装器+CDP 入口；`3a587de` 冷/热启动采集器（真机 3 轮实测）。**在途子代理**：批次 E B 组功能操作矩阵、批次 F 第一轮全量门禁预检（六项序列摸底）。
- **A4 复验 PASS + 双新切片派发（至 0a31331，共 71 个本地提交）**：A4 遮挡修复真机复验 PASS（三点探测 insideHeader 全 false，56px 让位生效）；软光栅真机成果收尾（`309d77d` 8 案例逐位一致+Nanite 式 atomicMin）；`62d6fbb`+`777f490` 启动证据组装器+CDP 采集入口；`b830708` 遮挡直修。**在途子代理**：软阴影射线消费者（RayBackend 第二消费者）、批次 E B 组功能操作矩阵。
- **两级追踪真机全通 + 批次 E 执行中（至 c92cbed，共 65 个本地提交）**：`4f3c42a` 软光栅 CPU 参考（visibility 编码同空间+独立重心参考对拍）；`e57f6c1` linear-depth COPY_SRC 三处同步修复；`b93ce6c` bindless 级 1（anchored WGSL 双轨）；`c92cbed` **两级 TLAS 真机 PASSED**（9 案例：平移/缩放/mask/近远；单级 sha256 钉死零回退；真机抓到并修复 WGSL 保留字/buildTlas 盒退化/共边平局三缺陷）；`d48a5a3` 系 Native 镜像。**在途子代理**：批次 E 双客户端打包执行（A1–A8 历史失败重放）。**批次 F 就绪项**：latencyStats 统一口径、启动打点、ray-trace/cluster-lod 真机 runner。
- **批次 E A5 实锤与修复（至 229a02c，共 67 个本地提交）**：批次 E 前置执行完成（88 截图+42 JSON 证据，batch-e-20260921-r1）：A1–A8 中 **7 pass + A5 fail**——Three WebView 弹 1199×616 控制台（Tauri main.rs 缺 windows_subsystem，PE 子系统=CONSOLE，历史失败复发实锤）。`229a02c` 一行修复（release 声明 windows_subsystem="windows"）。附带发现：1440×900 页顶组件被发布头栏遮挡（待修）。**在途子代理**：A5 修复复验（PE 字段+窗口差分+回归抽查）、RayBackend TLAS WGSL 扩展。
- **波次5 主体落地（至 5ba72c8，共 53 个本地提交）**：`5ba72c8` **Native 真多线程级联阴影编码**（render_graph.rs scoped-threads executor：层间有序层内并行对应 R6-3 合同；CPU 1/2/4/7 线程逐位一致+GPU 真机 RTX 4060 Vulkan 深度回读 6700 万字节逐位相等=确定性硬证据；CPU executor 3.39-3.71x，GPU 小场景 0.74-0.81x 如实记录开销交叉点；零新增依赖）。**在途子代理**：RayBackend TLAS WGSL 扩展（两级追踪）。
- **LOD 真机接线 + bindless 启动（至 a90f4bc，共 49 个本地提交）**：`a90f4bc` cluster LOD 真机 6 案例 PASSED（GPU 读回喂入 planClusterLodIndirect 再对拍；间接计划 20B 步长交叉断言）+ `a489da3` 编辑器启动并行化（三路径 styles+bundle Promise.all）；`f5ac2c2` 纹理数组打包合同（bindless 级 1 地基）+ 任务卡 specs/bindless-texture-virtualization-task-2026-09-21.md。**在途子代理**：Native 多线程 encoder、bindless 级 1 纹理数组实施。
- **消费者落地 + 双线推进（至 57d7284，共 42 个本地提交）**：`342c41a` SSR 屏外反射射线（RayBackend 第一消费者，真机 PASSED 63 射线 24 命中，默认关+预算双闸门+失败回退逐位一致）；`57d7284` cluster LOD GPU 屏幕误差选层（Nanite 选层件：前沿规则叶子恰一次覆盖，20 用例+全域 91 项零回退）。**在途子代理**：Native 多线程 command encoding（真并行+加速比证据）、cluster LOD 选层真机化+间接命令接线。
- **流送接线合同 + bindless 级 2 派发（至 1a27781，共 69 个本地提交）**：`1a27781` 世界分区流送三合同（翻译/增量/节流，18 新用例+23 回归；同族修复 worldChunkBridge urgency 恒 0 违反自身合同的真缺陷）。**在途子代理**：A5 修复复验、bindless 级 2 Native 索引合同镜像（identityGolden）。
- **TLAS 双端落地（至 6a6fb14，共 38 个本地提交）**：`1cf50f4` TS TLAS 实例层（盒 BVH+逆仿射+方向缩放 t 修正+mask，4 用例）；`6a6fb14` Native TLAS 镜像（7 测试过）——两级追踪（TLAS→BLAS）双端就绪，RayBackend 消费者与开放世界多实例可直接组合。**在途子代理**：SSR 屏外反射消费者接线。
- **MCP 诊断观察闭环 + 消费者接线在途（至 b8122f1，共 36 个本地提交）**：`7e360e9`/`a1cf04f`/`33dea4d` Native 三镜像（trace fan 对拍/world partition/world chunk bridge）；`228585c` **TS↔Rust identityGolden**（BVH f32 bits 级对拍，抓住并修复两处移植分歧：分割条件混入非轴分量、质心 corner 偏移加错维度——249/189 节点 → 63 对齐）；`d72f7ed` **P0-2 可见性 buffer**（opt-in，154 用例）；`e77f8c5` **RayBackend WGSL 执行器真机 PASSED**；`6933237`→`b8122f1` **MCP 诊断观察闭环三件套**（目录/拉取桥/工具）。**在途子代理**：SSR 屏外反射消费者接线（RayBackend 第一消费者）。
- **波次3/4 主体落地（至 e77f8c5，共 34 个本地提交）**：`d72f7ed` **P0-2 可见性 buffer**（rg32uint 编码 slot+triangleLocalIndex、opt-in features.visibilityBuffer 默认关、全屏材质还原+哨兵透传零回归、154 用例过；阴影/clustered/IBL/真机证据如实 deferred）；`e77f8c5` **RayBackend WGSL 软件执行器真机 PASSED**（headless Chrome+NVIDIA，48B 节点布局+栈溢出 atomic 哨兵 fail-closed，fan 射线 maxRelT≈1.7e-7 f32 量化级，真机抓到并修复 uniform usage 缺陷；TLAS/native wgpu RT 腿 deferred）；`228585c` **TS↔Rust identityGolden**（BVH 节点布局 f32 bits 级比对，抓到并修复两处移植分歧：分割条件混入非轴分量、质心 corner 偏移加错维度）。**超越路线进度**：软件光追执行器（追平 Lumen 的射线层）真机可用、可见性 buffer（Nanite 三件套之一）opt-in 落地、开放世界基础合同双侧就绪。**下一步**：RayBackend 消费者接线（SSR 屏外反射/软阴影/probe 遮挡）→ TLAS 实例层 → cluster LOD DAG GPU 选层 → 软光栅后备 → Native 渲染线程 → 批次 E+F。
- **Native 镜像推进 + 拉取桥（至 33dea4d，共 31 个本地提交）**：`7e360e9` Native ray_backend trace 镜像（栈遍历+80 射线 fan 对拍，5 测试过）；`a1cf04f` Native world_partition 镜像（4 测试过）；`33dea4d` Native world_chunk_bridge 镜像（lod 键需求展开，3 测试过）——TS/Rust 双侧合同逐语义对齐，消费者接线（波次4）两侧均可用；`4740291`+`062f0a4` MCP 字节拉取桥+driver 闭环（预算/租约/幂等，白名单强制）。**在途子代理**：P0-2 可见性 buffer、RayBackend WGSL 软件执行器。
- **MCP 诊断闭环成形 + 波次4 前奏（至 062f0a4，共 27 个本地提交）**：`bdbdb97` linear-depth 深度快照链（copy-src+白名单第三项+灰度导出）；`6933237`/`6762977` 诊断快照元数据目录（presence 镜像，白名单强制）；`4740291`+`062f0a4` 字节按需拉取桥+driver 闭环（16→96MB base64 预算、64MB 原始预算、租约匹配、幂等缓存、分块编码）；`bf30f3a` GI relocation 接线收尾提交（子代理成果：storage ABI v1 通道激活+防抖+时域安全，37 项过）；`d48a5a3` Native ray_backend 镜像（BVH 构建+Möller–Trumbore，cargo 3 测试+fmt/check 过）；`593be36` BVH rightChild 修复+CPU trace（fan 对拍）；`012f130` 世界分区流送桥。**在途子代理**：P0-2 可见性 buffer、RayBackend WGSL 软件执行器。**已知在途红**：shader/canonical 与 visibilityBuffer 相关 tsc 错误属 P0-2 子代理中间态，待其收尾。
- **波次4 前奏落地（至 012f130，共 22 个本地提交）**：`593be36` BVH rightChild 修复+CPU 参考 trace（80 射线 fan 对拍逐位一致；修复两个真 bug：递归构建右子索引不相邻、slab 零分量 NaN 剪枝）；`012f130` 世界分区→chunk 流送需求桥（world|cx|cz|lodN 键空间+紧迫度+往返解析）。下一步队列：Rust 侧 RayBackend 镜像合同（identityGolden 模式）→ RayBackend WGSL 软件执行器 → 消费者接线。
- **后续新提交（至 6762977，共 19 个本地提交）**：`79a75f8` AI 助手升级（failover/模型目录/思考深度/遥测/上下文预算/设置中心 UI，api 167 测试）；`b258cea` 波次1 三合同+CPU 参考 BVH；`de558eb` cluster LOD DAG CPU 参考 bake（确定性顶点聚类）；`8974eb6` probe relocation 求解器（GI 收口更新侧核心）；`bdbdb97` linear-depth 深度快照（copy-src+白名单+灰度导出）；`6933237`+`6762977` MCP 诊断快照目录（presence 元数据镜像+白名单强制）；`a250d8d` DCIR subgroup ops+HiZ 变体（CPU 层 13 尺寸逐位一致；Chrome 153 Tint 拒 requires subgroups 如实记 feature-unavailable 待重跑）；`dd78691` LOD 权威修复。**在途子代理**：GI relocation 接线（任务卡 specs/gi-relocation-closure-task-2026-09-21.md）、P0-2 可见性 buffer（opt-in features.visibilityBuffer）。
...
- 缺陷③修复 `9f804f8`：present_chart presenter层新增轴/图例——轴线+外向刻度+轴标签quad（axis_ticks.rs纯推导+axis_render.rs绘制），刻度语义与map_cartesian同源（zoom同步、category取ChartIR原始单元格），图例系列色chip，文字复用TextRasterizer通道+.notdef fail-closed；render_chart几何零改动故TS golden自动一致。合成4项+golden+GPU读回（生产包13标签quad逐个in-window）全过，2轮视觉闭环修复标签对比度；真实窗口华东/华北/华南与0-90刻度可见。边界：该生产包legend.visible=false按语义不出图例；time轴暂毫秒数值。[报告](specs/chart-axes-legend-window-2026-09-18.md)
...
- P1-12交叉判定关闭 `47c1c84`：P0-05 Web真实GPU矩阵即其剩余点"真实换包端到端GPU注入"（包切换通路已随C1宿主+候选控制机就绪），Native侧换包通路另有候选抢占/热更新恢复既有证据；任务表已标已完成。P1真正剩余收敛为：P1-01 resize小切片、P1-07 Arc化（跨禁区立项）、P1-19再分发权（用户决策）、P1-20 bidi（依赖shaping库决策）、P1-18合流（尺寸决策）。

- P1-01收尾 `da71239`：NativeApp::resize新增last_resize去重（同尺寸ScaleFactor回环不bump），真实变化时content.active_mut().epoch.bump_layout()，溢出fail-closed走failed；产品resize路径两处全经该入口。resize_epoch_tests单测锁定单调/去重/非resize不bump（Windows winit EventLoop进程内不可重建故单函数合并断言）。全量723 passed。GPU resize失败注入沿用诚实口径未覆盖。
...
- 双交付范围：Web发布链与原生离线链分别验收；现有原生ZIP不等于静态Web部署包。独立Web包先盘点复用能力，尚未计作完成；体积优化及DE26继续后置。
...
- 打包读取边界：EXE 改为同一文件句柄查大小、固定容量分块读取，拒绝中途截断/增长/修改，分块间响应取消；不再由 `readFile` 在文件增长时扩容。13 项打包/读取测试和 API 类型通过，真实发布→ZIP/DMDA/EXE→无参数呈现复跑通过，证据目录 `test-output/dashboard-bounded-exe-20260917-r1/`。
- 3DM `d755fc8`：四条零源容差共边通过源控制网恒等证明统一参数链，保持原顶点，新增192顶点/三角；跨面问题69→65，7条自缝继续核验。27项回归及类型/治理通过，仍保持部分预览。[证据](specs/industrial-3dm-isocurve-sync-2026-09-17.md)

- 单 EXE 版本切换：基于真实发布图表派生1.0.1测试版本（37/91→91/37），同一路径实际执行旧版→新版→坏新版拒绝→旧版回退，检查点/载荷逐项匹配，坏新版保留全部恢复文件。安装替换由测试脚本驱动，不计自动更新或断电事务完成。[证据](specs/dashboard-release-standalone-2026-09-17.md)
...
- Deep2D 多页编译：共享一次文档快照和合同校验，取消逐页复制整个应用；布局与单页编译逐项一致，第二页作为发布入口保持正确。27 项回归通过，新增入口断言后二文件 19 项通过；Web 类型检查、正式编译器构建、5 项集成与仓库门禁通过。本片未重跑 Native 字体项（环境变量未设置），此前真机证据不扩大为本片视觉验收。
...
### 最新执行范围：主流程优先，DE26 后置待决定（2026-09-17 用户更新）

- 本轮待办：加速 Deep2D、Deep Engine、工业模型主流程，优先打通编辑/导入→预览→保存→发布→离线打开→更新与失败恢复。三条主线继续并行。
- 明确排除（当前主流程阶段）：主动推进 DE26 三引擎对标及独立扩展卡；放到主流程收敛后，由用户决定是否继续，不自动恢复。下方历史“DE26 全部必做”及“一天全部完成”不再作为当前执行要求。
- 已完成的 DE26 代码和证据保留，不回退；主流程确实依赖的已有性能/资源管理能力正常复用，不以 DE26 名义扩大任务。进度与估时不再混入未决定执行的 DE26 卡。
- 自研/开源、内置离线、工业精度、测试与视觉验收要求保持；延期项不得计作完成。
...
- 同族修复：RuntimePackage/RenderPacket 首轮必须读内容，源缺失不退出，修复初始加载与监听启动间改写漏检。bin 119 passed/39 ignored，clippy/build/fmt、真实 ECharts GPU 两路径与 repository gate 通过。
- 本轮待办：完整视觉、动态适配器及 P3 其余项。[证据](specs/deep2d-p3-x-live-package-2026-09-17.md)
...
- 实证：1025 次真实请求跨两个 PID，前 1024 次复用，旧新进程都已回收；重复提交、取消、幂等关闭、坏 hash 后自行终止均通过，窗口 GPU 回归 3/3。
- 本轮待办：动态显示适配器、真实设备交互与完整视觉验收。本条覆盖此前“会话续期待办”。[证据](specs/deep2d-p2-01-x-session-renewal-2026-09-17.md)
...
- 实证：真实窗口主文件与恢复启动各 3 ticks，同 PID，退出无残留进程；JSON-safe 种子边界拒绝仍保留首帧恢复检查点。聚焦 3/3，bin 110/35 ignored，clippy 通过。
- 本轮待办：真实输入队列、1024 次会话后的持续续期、动态显示适配器和完整窗口视觉；本片保留累计 CPU 与请求预算。[证据](specs/deep2d-p2-01-x-window-ticks-2026-09-17.md)
...
- 工业格式永久边界(2026-09-17 用户重申):以 `specs/industrial-3d-format-work-plan-2026-09-16.md` 为唯一计划入口；格式名统一写 `X_T`，路线为自研/开源、本地离线，**不依赖外部商业 SDK、商业转换器或许可证服务器**，也不将其列为阻塞或回退。允许联网下载开放样本，并优先盘点 `D:/Download`。本次只读盘点确认顶层有 3 份 STEP、2 份 RVT；ZIP 内未发现目标工业格式；`IFCmodel.rar` 含 3 份 IFC，`V型口外槽轮式排肥盒_爱给网_aigei_com.rar` 含 1 份 STEP；顶层及已索引压缩包未发现 X_T。
...
- 已完成：API 按显式部署 JSON 组合真实内容编译器、窗口验证器、发布资源 closure 与 ZIP 下载，并在 index 注册；新增 [启动方式](specs/dashboard-native-startup-2026-09-16.md)。字体目录绑定项目/应用 revision、许可、face 与实际对象 hash，C3 读字节时再查防替换。主线 41 项 API、类型、窗口过程与证据绑定 28 项通过；真实文字编译→新 EXE 正常窗口→实际节点/字体回执的 6 项测试通过。独立真实候选 prepare→ZIP 整链正在复验，尚未交付完整可测验收版本。
...
### 2026-09-17 DE26/C03 第一切片：透明与双面状态保真（Codex 接手收尾）

- 已完成：冻结并贯通 straight/premultiplied 与 front/double 支持矩阵。Three 桥接受常见双面玻璃并将 two-pass 声明折叠为 weighted OIT 单 pass；packet/runtime JSON、batch key、材质变体与实例 bit128 保真；Web OIT 预乘分支不二次乘 alpha；Native 增加 premultiplied blend，forward 管线 12→18，RGB 因子 `One`，两类 BLEND 均不写深度。OPAQUE/MASK 上任何显式 `premultipliedAlpha` 声明均 fail-closed，BLEND 显式 `false` 等价 straight。
...
### 2026-09-17 PLAN-06 GLB 几何审计补强（Codex）
...
### 2026-09-17 X_T：并行修复与 109 件全量预览复跑（Codex）

- 修复旧 BODY 字段错位、跨版本属性字节计数、缺失/尾随终止记录诊断、狭长三角形距离 NaN 与面索引溢出；最小研究补丁入库，不引入商业或产品运行依赖。
- 109/109 实体流读尽；合并构建后 109/109 生成有效 GLB，报告面/三角计数一致且无 stderr。AS-2940 源 FACE 为 75、AD5008 为 38，与新输出一致；旧 24/25 分母不是源真值，不能据此称原源文件丢面。
- parser 20 项、cad-xt 9 项、cad-tess 34 项通过；主线程复跑全 corpus、产品 GLB 审计及独立 typed IR 审计。后者仍有 shell/fin 语义诊断，生产认证保持 0。完整因果对照、独立保留集、几何精度、单位/source map、授权归档与产品接线继续本轮待办。[最新证据](specs/industrial-format-plan01-02-lock-2026-09-16.md#6-2026-09-17-修复后全语料复跑)

### 2026-09-17 X_T 源分母独立核验（Codex）
...
### 2026-09-17 Deep2D P2-01 X display batch→真实 painter（Codex）

- X 封闭 ABI 新增每 epoch 最多一个、自包含的 `Deep2dDisplayList` layer；TypeScript 冻结、Native worker、IPC receipt 与 scheduler/LKG 提升前均复用正式合同校验。逐 draw-call IPC、多个 display list、坏引用和预算超限全部 fail-closed。
...
- 显式 headless CLI 支持 1～1024 次真实 LPAC→IPC→Painter，所有 tick 成功后才更新包级 LKG。3 次实跑 epoch 9→11、请求 hash 变化、静态输出 hash 相同、末次 1 fill triangle；scheduler 7 项与产品 CLI 纵向通过，clippy/fmt 通过。重绑定前后先执行深度/IPC 字节预算，防止哈希前无界 JSON 分配。
...
### 2026-09-17 Deep2D P2-01 持续 LPAC worker（Codex）

- XSF1 长度前缀 IPC 复用单个零 capability LPAC worker；每帧 4 MiB、每会话 1024 次、容量 1 通道，Job 累计 CPU 不清零。失败关闭会话，旧已发布输出保留；成功路径清理后才提升包级 LKG。整段产品回执也限制为 4 MiB。
...
### 2026-09-22 Shader S2：作者 Graph → 既有 WGSL IR lowering（ZCode）
...
### 2026-09-22 B2-a 切片：时间轴倒放 UI + 区间播放（ZCode）
...
### 2026-09-23 主线收口会话：千文件工作树清零 + F3 三线性 + 四端门禁全绿

- **工作树收口**：接手时工作树积压 715 文件(14k 行,多波次并行成果+若干会话遗留),全部按主题分批验证落库,现仅剩 6 个 `.tmp-*`/`r10-compat-check` 草稿不入库。今日提交链(时序)：815428a(B6 baker hint)→a012ce6+025495c(A4 三预设真机像素 gate:alarm 300/ring 2/flow 6)→e94b939(B5 native 台账)→a85ce0b(B3-c 碰撞事件收口记录)→4e9a95a+ff917d4(B2 播放区间/B5 多选 web)→826a46b+e78393f(Deep2D select-v1/文本输入/视频/UIA + F5 筛选多选 + 原生渲染核心波次)→b58622c0(B2 倒放 UI/区间设置收口+合同去重)→a3c7665(F3 网格头+三线性)→d9e28b6/ff85e7e/a89c800(packages/web/docs+scripts 收口)→4791513(api 修复)。
- **F3 Native GI 三线性切片**(a3c7665)：`probe_gi_grid.rs` 网格头合同(record 0=origin/spacing/gridSize/maxPosition/probeCount,CPU 对拍参考逐式对齐 Web sampleIrradianceProbeClipmap 单层:三线性×validity×Chebyshev×法线权重 bias=3,半球用原始着色点,0.2 格偏移只进可见性);`native_mesh_v1.wgsl` 开关通道 `frame.lightDirection.w` 三态(0 关=逐位不变/1 最近探针/≥1.5 网格三线性 8-tap),头/记录数/世界位置任一非法 fail-closed 返零;网格模式 positionOffset=重定位增量(与 Web storage 路径一致),最近模式仍为预烘焙世界位置,两模式语义差异已写明。门禁:probe_gi 族 15/15、lib 524/0、bin 251/0(含真机 GPU family 编译)。剩余:GPU 像素消费证据、producer 接线、多层级联、设备恢复。
- **继承破损修复 8 处**(全部是并行波次咬合遗留,非本轮引入)：①web studioDeepEnvironment 衰减合同升级(distance=0=无截止+decay 透传)后测试未更新;②playbackLifecycle/SceneBehaviorDebug 的 ESM 行映射期望停在 1 行 prelude 时代(现 2 行,映射-2 正确);③docsCatalog 断链(engine-benchmarks 引用 gitignored test-output,改纯文本路径);④RendererDiagnosticsPanel 硬件光追行中文漏英译(补静态+前缀式模板英译);⑤prefabThumbnail 预制体 120→121 且 kind 断言改为与构建器表自维护对账;⑥api system.ts+routes.ts 重复注册缓存路由致 Fastify already declared(system.ts 侧移除,routes.ts 被多测试依赖保留);⑦ZIP LICENSE 断言对齐 DMCSL-1.0 法定名(产品显示名改 DeepMonkey 不改许可证名,AGENTS.md 钉死);⑧AI 超大上下文语义矛盾(裁决:预算 fail-closed 拒绝为合同,目录测试夹具 130k→60k 保预算内语义)。
- **四端门禁终态(串行复跑)**：web 721 文件 4255/0+tsc 0 错误;api 235 文件 1538/0(PG 真库并发超时放宽 60s);deep-engine 3891/0;contracts 340/0;native lib 524/0+bin 251/0。已知抖动:同机并行跑多套件时 PG/长时用例会超时,串行或放宽超时后稳定全绿。
- **诚实边界**：web 套件在高并发负载下存在超时抖动(两次取样一次 138 失败/串行全绿);F3 三线性尚无 GPU 像素级消费证据;B2-b(发布侧 playbackRange 下译/发布查看器 transport)、F2 设备恢复与 BLEND 回退、F3 producer/设备恢复、F5 IME/触控/音频全链、V1-V5 全量验收未做。未 push。

### 2026-09-23 F2 设备恢复与 BLEND/无 RT 回退证据切片（ZCode）

- **切片 1 设备恢复**：`renderer/rt_residency.rs` 把 frame.rs 的 opaque RT/栅格逐帧裁决抽成纯函数 `rt_opaque_ready`（行为不变，frame.rs 改为消费它），回退条件可被单测钉死。新增 `renderer/rt_recovery_gpu_tests.rs`（真机 ray-query 设备）：初建驻留经真实 Ray Query 探针命中 → 整体 drop 旧驻留（模拟 app/recovery.rs 整体重建 Renderer）→ 全新 `RtSceneResidency::build` 自足（探针仍命中、计划逐槽与初建一致、BLAS 缓存数一致——重建是 (device, scene) 纯函数，不复用旧实例）→ 按 reestablish_rt_residency 生产路径重挂 frame RT 槽 + 重装管线族后裁决从回退(None)恢复到 RT 就绪(Some)；全程 uncaptured error 干净。CPU 合同（rt_residency_tests）：计划核跨构建纯度（A→B→A 第三次逐字段等于第一次，槽位派生无历史状态）+ 空驻留槽必回退。
- **切片 2 BLEND/无 RT 回退**：新增 `renderer/rt_fallback_gpu_tests.rs`。①同适配器创建**不带 ray-query 特征**的 device（与真实无 RT 设备同 `device.features()` 判定路径）：frame_rt layout 不创建、`RtSceneResidency::build` 返回 MissingFeature、栅格全链（级联阴影+opaque）validation/uncaptured 干净，128×128 像素与 RT 设备同路径基线**16384/16384 逐位一致（100.0000%，mean|Δlum|=0，max|Δchan|=0）**；②全 BLEND 场景真机驻留构建 EmptyScene 拒绝（与 classify_rt_batches CPU 合同同口径，BLEND 整族由既有 transparent 栅格 pass 承载）；③`rt_opaque_ready` 在真实驻留/槽位/管线族对象上的边界（管线族缺失回退、槽未挂回退、custom shader 回退、全就绪走 RT）。lib 侧 `hardware_ray_query`：`ray_query_device_ready` 双缺失为 false + 无特征设备上 BLAS/TLAS 构建在触碰几何前 fail-closed MissingFeature。
...
### 2026-09-23 F6 仓外独立消费示例（Node 校验器 + 浏览器 bundle 两例真实运行）

- **已交付**：`packages/deep-engine/examples/{node-standalone.mjs,browser-standalone.mjs,browser-standalone.html}` 两个第三方可复制的最小示例（中文注释）+ 驱动 `packages/deep-engine/scripts/standaloneConsumerExamples.mjs`。驱动在系统临时目录模拟第三方消费者走完整链：重建 dist → pnpm pack 引擎与 @webgpu/types（复用 gate:deep-engine-consumer 的 pack lib，tarball 私有性/导出/sha256 全断言）→ `npm init` + `file:` .tgz 依赖 + overrides 钉住传递依赖 → `npm install --offline --ignore-scripts`（注册表指向 127.0.0.1:9 断网探针，added 2 packages 零错误）→ Node 示例真实运行 → esbuild bundle 浏览器示例 → headless Chrome 153（playwright-core，参照 gpuParticleRenderTest.mjs 启动模式）加载断言 + 截图。
...
### 2026-09-22 F3 多层探针网格运行包载荷管道（子智能体切片，级联最后一环）

- **Web 合同 v2（双形态按 levels 键分派，同一 schema `deep-engine.probe-grid` v1）**：`environmentTypes.ts` 将 `RuntimeIrradianceProbeGrid` 拆为 `RuntimeIrradianceProbeGridSingle`（旧单层形状逐字段不变）∪ `RuntimeIrradianceProbeGridCascade`（`levels` 细→粗 1..=4 层，每层 `RuntimeIrradianceProbeGridLevel` 与单层字段一致）；顶层单层字段与 levels 互斥（并存由 `fields()` 未知字段拒绝）。`environment.ts` 的 `validateIrradianceProbes` 重构为"字段闭合 + `validateProbeGridGeometry`（既有单层规则原样）"，级联新增：层数 1..=4（引 lighting 打包器 `NATIVE_PROBE_GRID_MAX_LEVELS`，常量唯一来源不另写数字）、粗层 spacing 严格递增、粗层范围逐轴包含细层、布局头+Σ(层头+探针) 总数 ≤ `NATIVE_PROBE_GRID_MAX_PROBES`——与 Rust `decode_probe_grid_cascade`/Web `packNativeProbeGridLevels` 同一套 fail-closed 合同。schema 身份字段只在顶层校验，层不带。
- **编译器衔接（apps/web compileSceneRuntimePackage）**：`SceneIrradianceProbeBake` 变联合（`SceneIrradianceProbeGridBake` 单层 ∪ `SceneIrradianceProbeCascadeBake.levels`）；`compileSceneIrradianceProbes` 按 `"levels" in bake` 分派：每层 origin 独立 `worldToLocal` 局部化（纯平移，包含关系不变）后写 v1 级联载荷，并用 `packNativeProbeGridLevels` 先行校验对账（输出字节数恰 = (1+Σ(1+count))×96B）。**选择保持 JSON 载荷、打包器只做校验（字节即弃）**：既有链路是 JSON→buildDeepRuntimePackage→validateRuntimeEnvironment 二道把关、记录流由 Native 装载端重建，切二进制载荷需改包格式且破坏旧包逐位不变；单层分支原样（含错误信息前缀）。
- **Native 解析分派（solid_environment.rs）**：`decode_probe_grid` 在 schema 校验后按 `levels` 键分派——新 `decode_probe_grid_cascade_payload` 组装 v2 记录流（`ProbeGiGridLayoutHeader::encode` + 每层 `encode_with_base(游标+1)`），最终过 `decode_probe_grid_cascade` 全合同复核；互斥合同（levels+旧字段并存拒绝）、层数 [1,4]、层号定位错误信息。单层路径重构进 `parse_grid_level`（`ProbeGridLevelContext` 前缀使既有逐字错误信息不变，base=1 走 `encode_with_base` 与 `encode()` 同字节）。renderer init 的级联消费上一切片已验收，无需改动。
...
### 2026-09-23 I3 缺口收口切片：线性预制体贴地投影 + 坡度边界（四缺口之二落地）

- **贴地投影（确定性管道层）**：`linearPrefabPath.ts` 新增 `projectLinearPrefabPathPoints(points, snapToGround, groundY?)` 纯函数 + `linearPrefabSegments(path, {groundY}?)` 投影参数——仅当路径启用贴地且调用方提供有限基准面时把全部控制点 y 归到基准面（投影在样条采样前、作用于控制点，与作者 raycast 投影同语义）；无地形数据（groundY 缺省）或贴地关闭时原样返回作者 y，不虚构地形高度。**发布编译不传 groundY（无场景地形上下文）**：消费作者态已投影保存的点，与预览同走 `linearPrefabSegments` 保持同源。作者态既有 `viewerEngineInteraction.snapPrefabPlacementPath`（raycast→写回保存点，无命中保持作者 y）不改动。场景地面语义沿用现状：y=0 平面（指针兜底平面/Deep 固定地面网格/Rapier 内置静态地面一致）。
- **坡度边界（fail-closed）**：`assertLinearPrefabPathSlope` 在投影后的控制点上校验相邻点坡角 `atan2(|dy|, hypot(dx,dz))`，超上限抛中文可读 RangeError（含段号/实际坡度/上限/两个可操作动作）；闭合路径收尾段（末→首）同受约束；垂直段（无水平投影）按 90° 拒绝；重合点留给分段阶段零长过滤；判定容差 1e-9 弧度吸收"恰好等于阈值"浮点毛刺（恰好等于放行、真正超过才拒）。阈值 `maxSlopeAngleDegrees?: number` 进合同（`packages/contracts` 类型+校验 0..89 对齐 `normalizeNavigationSettings` 口径），缺省 `DEFAULT_LINEAR_PREFAB_MAX_SLOPE_ANGLE_DEGREES = DEFAULT_NAVIGATION_SETTINGS.maxSlopeAngle`（50°，同一来源不漂移）。作者预览（proxy→buildLinearPrefabGeometry）与发布编译（compileLinearPrefabRenderPacket→同函数）天然同走、同样拒绝。
- **作者态落点**：`IndustrialPrefabInspector.save` 捕获 `setIndustrialPrefabState` 抛错 → `role="alert"` 错误条（`--danger`，state 不落库=失败保留原状）；铺设路径区新增"坡度上限"NumberField（0..89，缺省展示导航同源 50）。校验错误落 UI 而非炸组件树。
- **测试**：linearPrefabPath 3→8（缺省对齐钉住防漂移、投影确定性/基准面可换、无地形保持作者 y、恰好等于 45° 放行 vs 超过拒绝、缺省 50° 两态、垂直段、越界阈值、闭合收尾段 vs 开放路径放行）；linearPrefabGeometry 1→2（预览 fail-closed + 阈值可配置放行）；sceneSnapshotRenderPacket +1（发布拒绝超坡度、缺省内平缓正常出包）；contracts industrialPrefabValidation 5→6（新字段 50/89 放行、-1/90/NaN 拒绝）；Inspector 断言坡度上限字段缺省 50/max 89。
...
### 2026-09-23 30 分钟自检（第十六轮）：I3 贴地/坡度验收 + junction 补位
...
### 2026-09-23 a01x 进程内存+长稳采集接线（V4 补位子代理）

- **交付**（链路：`apps/web/scripts/gate-a01x-babylon-pairing.mjs` 驱动 → lab `competitiveBenchmarkRunner.ts` → pass-N.json）：①新 `scripts/benchmarks/run-windows-process-metrics.ps1` 的姊妹采集器 `scripts/benchmarks/run-windows-process-tree-metrics.ps1`——attach 型进程树采样（以 Chrome 主进程为根逐样本聚合整树 WorkingSet/私有内存 peak+mean，stop-file 干净收口，GPU Process Memory 计数器按树内 pid 实例尽力求和）；②驱动每 pass 渲染窗口并行起采样器，产出 `records[].processMetrics`（peakHostBytes/meanHostBytes 等，camelCase 与 record 风格一致）；③`--long-run-minutes [N]`（省略值 5，正式口径 30，硬上限 60，未知参数 fail loud）：页内 runner 新增长稳相位（配对轮次后 candidate→reference 串行各渲染 N 分钟墙钟，帧间隔含让步口径对齐 bevy frame-interval，600 帧一圈轨迹循环回放），`longRunFrameP99Ms` 只注入收尾轮双侧 summary（非收尾轮不回填防覆盖度虚标）+ 报告级 `longRun` 元数据；④适配层 METRIC_MAP 增 `longRunFrameP99Ms→long-run-frame-p99-ms`，`processMetrics/longRun` 记录级透传（缺采样不带键）；⑤paired-summary 增进程内存/长稳标注行（bevy 输入无这些字段输出零变化，合成样本验证 has-process/longrun/new-row 全 false）。
- **短跑实测**（`--long-run-minutes 1`，A01X_WARMUP=10/CPU_SAMPLES=30/GPU_SAMPLES=0，5 轮单 pass，2026-09-22 23:11）：进程内存 **peak-host 1083375616 B（1033.2 MB）· mean-host 1000093838.9 B（953.8 MB）**（Chrome 树 301 样本 @200ms，采样窗口 100s，stop-file 干净收口；peak-gpu 如实 null——本机 GPU Process Memory 计数器类别存在但树内无 pid_* 实例，宿主树总量+标注交付）；长稳 **long-run-frame-p99 Deep 4.90ms（68723 样本/60.0s）· Babylon 4.70ms（86132 样本/60.0s）**，帧间隔含让步口径。适配层+聚合器实跑：报告新行 `long-run-frame-p99-ms | 4.900 (1轮) | 4.700 (1轮)` 与进程内存/长稳标注行齐备；独立重算 peak/mean/p99 与报告全一致；样本存 `test-output/a01x-babylon-pairing-20260920-r1/process-metrics-sample.json`（gitignore）。门禁：deep-engine lab/src tsc 0 错误；competitiveBenchmarkRunner 5 测+src 合同 5 测全过；buildLab 零错误；node --check 三脚本全过。
...
### 2026-09-23 F4 逐字段对拍矩阵落地 + 3 处静默丢失修复（F4 补位子代理收口）

- **交付**：`docs/specs/f4-field-parity-matrix-2026-09-23.md`——属性面板→快照→运行包→Web/Native 渲染逐字段对拍证据矩阵，6 域（灯光 15 行、后处理+色彩分级 9 行、雾/天气 5 行、环境 7 行、材质槽/对象外观 15 行、烘焙/GI 2 行），每格带证据指针（文件:行/测试名）与状态格（✓/🔶降级声明/⛔fail-closed/–无语义）。口径：静默丢失=编译侧声明能力或接受字段、消费端不消费、且门禁无 degraded/blocked。
- **对拍发现并修复 3 处静默丢失（修复后计数 0）**：①**作者色彩分级六通道**——TS 编译端从不生成 colorGrading（合同最高 v8），而 Native v9 decode/渲染端早已就绪，链路断在"快照→运行包"，且门禁 reason 还错误声称"Deep Native 尚未实现消费"（与事实相反）；修复=非中性分级随 v9 档（`native-aces-grading-v9`，recipe v14）进运行包，TS 合同+Web 校验器+编译器+evidence（`deep.scene.author-grading.v1`）+门禁（部分编译语义：分级能力条+域内其余 degraded 条，过时文本删除）+Native 档位门扩展 no-ibl kind（studio/no-ibl 双 kind，三档之外仍拒）全链贯通；六通道范围三方一致（面板 hue±180/余±1 = `AuthorGrading::new` = Web 校验器）。②**spot shadowSoftness**——载荷携带+Native 解码合法+CPU 写 ABI 行 120 但 wgsl 无消费（硬 PCF），已编译场景无任何标记；修复=门禁 compiled 态输出 degraded 条（`hasAuthoredSpotSoftness`），自动进 manifest degradedCapabilities；真消费（PCSS 采样）留后续切片。③**environmentIntensity（HDR 链）**——校验 [0,64] 后把值丢弃（v6 载荷无强度字段）；修复=非 1 一律 fail-closed 拒绝 HDR 编译→deferred→门禁 blocked；真强度进载荷需新档位，列边界。
...
### 2026-09-23 F3 探针网格 GPU 烘焙编排服务落地（feat 子代理，2907cb3f）

- **交付**（闭环"捕获→读回→网格聚合→编译器输入"生产者链，全复用既有能力零重建）：①`packages/deep-engine/src/rayTracing/probeGridBakeMath.ts`（新，纯函数层，脱离 GPU 可测）——`planProbeGridCapture`（origin/spacing/gridSize→捕获计划，校验口径与 Native 打包器 validateLevel 同源：每轴 2..64/体积预算/spacing/origin 范围，GPU 请求前 fail-closed）、`probeGridReadbackLayout`+`decodeProbeGridCapture`（rgba16float 读回布局 256B 对齐 + half 流解码成样本表，NaN/Inf fail loud）、`aggregateProbeGridBake`（样本表→网格条目：每 cell 恰一条、irradiance 透传、validity=捕获覆盖 alpha、缺样本补 validity 0 零辐射不虚构、越界/重复样本 fail loud、查表映射与供给顺序无关=确定性）；②`probeGridBakeService.ts`（新，GPU 编排层，162 行）——内部复用 `ProbeSceneRadianceProducer`（encodeSourceRadiance 一次 dispatch 全网格）+ generation 递增支持真实重捕获，bake 收尾用 `packNativeProbeGridRecords`（编译器写包前同一校验器）做编译器输入路径 fail-closed 对账，溢出哨兵非 0 拒绝发布；输出与 apps/web `compileSceneRuntimePackage` options.irradianceProbes 单层 `SceneIrradianceProbeGridBake` 结构逐字段兼容；③单测 15 例（计划映射/非法参数/读回步进/half 解码/覆盖/空场景全 miss/溢出/越界/重复/确定性/打包器对账/构造合同含 stub GPUDevice 全局常量）；④真机取证链 `lab/probeGridBakeGpuProbe.ts` + `scripts/probeGridBakeGpuTest.mjs`（package.json 新增 `test:probe-grid-bake-gpu`）。
- **真机证据**（headless Chrome 153.0.8010.53 + 真 WebGPU，2×2×2 单层，地面+悬空盒 fixture）：**gate TRUE 一次通过**——8/8 探针全覆盖（validity 全 1，两次独立 bake）；逐 cell GPU/CPU 对拍（同一着色公式 cpuReferenceFor）最差绝对差 **2.33e-4**（f16 容差 1e-3，8 cell×2 次 16 组全过）；两次独立 bake 产物 **JSON 逐位一致**（真机确定性）；遮挡 cell 亮度 < 开阔 cell 语义断言过；溢出哨兵 0；打包器（编译器输入路径）接受。证据：`test-output/probe-grid-bake-20260923/{report.json,bake.json,probeSceneRadiance.wgsl}`（gitignore 不入库）。
...
### 2026-09-23 V1 三端统一对拍 runner 骨架落地 + 首轮三端取证（V1 补位子代理）

- **交付**（2117d033）：`scripts/verify-v1-tri-endpoint-matrix.mts`（309 行统一 runner，中文注释）+ `apps/web/scripts/v1-tri-endpoint.html`/`v1-tri-endpoint-page.ts`（浏览器侧宿主，headless Chrome 与 WebView2 共用同一页面，真实 ViewerEngine 消费冻结运行包）。复用骨架：gi-crossend-matrix 的 vite+playwright 取证口径、verify-scene-native-window 的 Native 哈希链验证、captureNativePlayerWindow 实窗截帧、V2 build-minimal-fixture 的夹具编译链、batch-e a4 探针的 WebView2 CDP 启动/taskkill 树杀口径。用法：`npx tsx scripts/verify-v1-tri-endpoint-matrix.mts [--output <dir>] [--frames N]`。
- **冻结 fixture**（`test-output/v1-tri-endpoint-20260923/fixture/`，gitignore 不入库）：以 V2 最小 box 场景为基础（1 box #60a5fa scale2 + orbit 相机 + none 环境），compileSceneRuntimePackage 编译 packageId=`scene.v1-tri-endpoint`、packageHash=`125fb6b7e6819f0cabe0a30f736bd542183f87472d28b6f8629598f5e20b2064`、2664 字节、3 resources、camera schema 3；**两轮复跑运行包 SHA-256 逐字节一致**（固定 createdAt/updatedAt，冻结确定性成立）。三端消费同一 runtime-package.json 字节。
- **首轮三端取证（3/3 端全绿，门禁 ≥2 端真实帧/截图证据 PASS）**：
  - **Web**：headless Chrome 153，webgpu 与 webgl 双后端各 **8/8 帧** + 画布截图（`web/web-webgpu.png`、`web/web-webgl.png`），parse 包 hash 校验一致，GPU 错误干净（口径=无未捕获异常且无非资源类 console error；favicon 404 资源噪声单列不计入 GPU 错误）。
  - **Three WebView**：真实 Tauri EXE（batch-e a4 的 `three-webview-new.exe`，SHA-256 cb54558f…）经 `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--remote-debugging-port` 开 CDP：实窗启动截图（`webview/launch.png`，内嵌 publication="批次E验收场景"，历史构建内容如实记录）+ **同一 WebView2 真实进程内**加载同一冻结包，webgl **8/8 帧**（`webview/webview-webgl.png`）+ webgpu **8/8 帧**（`webview/webview-webgpu.png`），hash 一致、GPU 错误干净。
  - **Deep Native**：release EXE（V2 构建的 87a4cc50…）走 verify-scene-native-window 全哈希链：**1200×800 真窗、Vulkan/RTX 4060、8/8 帧、gpuErrorsClean=true、报告 hash 与夹具逐字节一致**（`native/native-verify.json`）；另以 captureNativePlayerWindow PrintWindow 实窗截帧 `native/native.png`（960×540）。
...
### 2026-09-23 V3 Unity WebGL 本机导出 + 托管端到端验证：五项 5/5 PASS（补位子代理）

- **交付**（46cf3726）：`tools/unity/v3-webgl-e2e/`（Unity 最小验证工程：V3ProbeBuilder.cs 场景=相机+平行光+Unlit 红立方体+no-code 探针 telemetry/focus/device-click，引用仓内 com.bim-studio.bridge 包；run-v3-webgl-build.ps1 驱动：Hub 会话凭据提取→batchmode 构建→bridge 注入/manifest/无压缩产物/ZIP 四重校验→日志脱敏）+ `apps/web/scripts/v3-unity-e2e.html`/`v3-unity-e2e-page.ts`（Deep Web 宿主，复用产品 unityBridge.ts 真实合同 parseUnityBuildManifest/unityHostMessage/resolveUnityDataLayers）+ `scripts/verify-v3-unity-webgl-e2e.mts`（runner：ZIP→解压逐字节对拍、静态托管 .wasm MIME+CORS、vite 宿主、headless Chrome 五项断言）。
- **构建实测**：Unity 6000.0.52f1（WebGL 模块 D:\Soft\Unity\6000.0.52f1\Editor\Data\PlaybackEngines\WebGLSupport 在位；2021.3.10f1c2/2022.3.62f1 亦带，6000.3.11f1 不带），batchmode 真实构建 90.8s，产物 19 文件（loader/framework/wasm 13,038,161B/data 平铺无压缩），ZIP 5,337,644B SHA-256=930bea1d9e380019…；index.html 已注入 unity-bridge.js+createTrackedInstance+register，bim-studio.manifest.json schemaVersion=1/bridgeVersion=1/unityVersion=6000.0.52f1，资源清单 scenes=[V3Probe]/events=[device-click]/actions=[focus]/dataLayers=[telemetry]/objects=[BridgeNoCodeProbe] 全部由 no-code 绑定自动发现。
- **端到端五项 5/5 PASS**（Chrome 153.0.8010.53 headless，宿主 vite 127.0.0.1，托管独立 origin 跨源 iframe）：①加载=进度序列[0,0.9,1]真实单调+ready+iframe 内 canvas 960x600；②材质=Unlit 红立方体真实光栅化（红像素 6.93%/背景 93.07%，双色确定性输出，截图为证）；③输入=init/ping/action 三消息全部被 Unity 主线程 ack，health fps=50.000019/scene=V3Probe（非 mock 整数，真实帧间隔推算）；④资源=manifest 资源清单解析+telemetry 数据层经 init 下发被 ack；⑤发布=ZIP 解压 19 文件与构建目录 SHA-256 逐字节一致+playerUrl HTTP 200。证据：`test-output/v3-unity-webgl-20260923/`（build/ 产物+ZIP、logs/unity-build.log、screenshots/01-host-iframe-player.png+02-unity-canvas.png、build-summary.json、e2e-result.json、capability-matrix.md）。`pnpm test:unity-bridge` 既有 3/3 复跑通过无回归。
- **诚实边界**：①输入验证为 bridge 消息链真实往返（Unity C# ApplyStudioMessage→jslib→宿主），未含鼠标/键盘像素级交互轨迹；②发布验证为 ZIP→托管→playerUrl 可达的产物合同，产品控制台 ZIP 上传与发布记录落库链路未覆盖；③渲染经 SwiftShader，GPU 硬件光栅化表现未声明；④构建显式取 Uncompressed（Unity 6 API PlayerSettings.WebGL.compressionFormat=Disabled），brotli/gzip 托管需 Content-Encoding 协商未覆盖；⑤个人许可强制闪屏（MADE WITH Unity），场景帧在 ready+6s 淡出后取样；⑥迭代记录：首跑包相对路径错（file: 需相对 Packages/ 两级）、次跑编译错（Environment 缺 using System+Unity 6 压缩 API 变更）、材质判定阈值曾误设色彩桶>8（Unlit 双色即证据，已修正），三次构建两次 e2e 全部留日志。

### 2026-09-23 30 分钟自检（第二十七轮）：V3 端到端验收 + V5 预检补位

- **V3 Unity WebGL 本机导出+托管端到端验收通过**（46cf3726+1f07cb81）：Unity 6000.0.52f1 真实 batchmode 构建（90.8s、19 文件、无压缩 ZIP SHA 钉死）、构建脚本自动注入 unity-bridge 并四重校验、no-code 绑定自动发现；**五项 5/5 PASS**（加载=真实进度序列+canvas、材质=Unlit 红立方体真实光栅化 6.93% 红像素、输入=init/ping/action 三消息 Unity 主线程 ack + fps 50.000019 非整数值、资源=manifest+数据层 ack、发布=ZIP 19 文件逐字节一致+playerUrl 200）。复跑 test:unity-bridge 3/3 无回归。边界：输入为 bridge 消息链往返非像素级轨迹、发布为产物合同（控制台上传链未覆盖）、SwiftShader 渲染、三次构建三次 e2e 迭代日志全留。
- **V 系列状态**：V1 runner✓（3/3 端）、V2 ✓、V3 ✓（5/5）、V4 适配层+采集✓（正式跑待）、V5 预检启动。
- **满载补位**：空闲代理派 V5 全量门禁预检（清单化映射具体命令→逐项实跑→PASS/FAIL/SKIP 报告，真实缺陷同族修复）；F3 烘焙 UI 穿透代理在跑。

### 2026-09-23 F3 探针烘焙 UI 与发布穿透：编辑器入口→发布会话态→包内 irradianceProbes 端到端接线（子代理）

- **交付**：UI 入口 `apps/web/src/components/SceneProbeGridBakePanel.tsx`（GI 行下方烘焙区块：origin/spacing/gridSize 参数输入+阶段进度+探针数/覆盖率摘要，客户端预检 validateProbeGridInputs，服务侧 fail-closed 仍兜底）；执行编排 `apps/web/src/delivery/probeGridBakeRunner.ts`（compileSceneRenderPacket 纯 CPU 编译→独立 requestDevice→deep-engine ProbeGridBakeService.bake→存会话态，结束 destroy）；承载点 `apps/web/src/delivery/probeGridBakePublicationSession.ts`（页面内存 Map，键=编译器严格源投影哈希再剔除发布决策三元组 publicationMode/publicationPerformance/publicationToolbarVisible，上限 4 条 LRU 淘汰）；穿透 `sceneClientPackage.ts` 新增 `resolveProbeGridPayloadInput`（显式透传优先→显式 null 强制不带→会话态查找→null，传给 prepareNativeSceneClientPayload 第 4 参）；接线 SceneLightingEditor（可选 props，其他使用方不受影响）→SceneEnvironmentPanel→AppStudioViewport（ownerKey ref 防陈旧回执，场景/项目切换回 idle）；deep-engine 主索引导出 ProbeGridBakeService/ProbeGridBakeEvidence/ProbeGridBakeGrid/ProbeRadianceLighting。
- **方向语义取证**：compileSceneLighting 的 direction 在 Native 端被 gpu_resources.rs shadow_ray_direction（frame[11]）直接当 surface_to_light 消费，与烘焙服务 surfaceToLightWorld 同名同义，故原样传递不取反；radiance 已预乘强度故 intensity 恒 1。
...
### 2026-09-23 自检（第二十九轮）附：F/I 快速收口启动 + I2/I6 结论

- **I6**：SDK 文档补独立消费示例可运行命令与证据路径引用（8/8 过）。**I6 判定收口：已完成面=SDK API/插件兼容矩阵/Native 能力矩阵/可运行样例引用；I6 收口**。
- **I2**：UI 覆盖度按第十五轮语义判定收口——clipping 按快照能力显隐（f87fd49b）+ degradedCapabilities 进 manifest（payload 层有测试）+ 工具坞缺省兼容；测量/爆炸维度不伪造判定。**I2 判定收口（覆盖度结论=能力驱动显隐已实现，剩余维度均有明确语义归属）**。
- **满载补位**：子代理①派 F6 收口（统一宿主入口 facade + viewerEngineInteraction 动画控制域拆分，公开 API 兼容+前后回归对比）；子代理②派 F5 收口（IME 组合/触控一致性/音频三子项，每项最小可验证）；主线程 I2/I6 结论 + 文档切片。
- F/I 剩余（代理在跑）：F6 两切片、F5 三子项；完成后 F≈97%/I≈98%。

### 2026-09-23 F6 收口两切片：统一宿主入口 + viewerEngineInteraction 动画控制域拆分（子代理①）

- **切片 1 统一宿主入口**：新增 `packages/deep-engine/src/host.ts`（约 131 行，纯 re-export + 入口 JSDoc，不搬实现）——面向宿主（Studio/第三方）的稳定 facade，按五主题分组：①渲染后端创建与切换（PbrRenderer/DeviceSession/BackendSwitchCoordinator/BackendPreferenceController）；②探针 GI/粒子/烘焙服务（ProbeGridBakeService/ProbeClipmapPbrController/GpuParticleRuntime 及类型常量）；③runtime-package 构建/校验/序列化/哈希（validateDeepRuntimePackage/validateDynamicSceneRuntime/serialize/parse/runtimePackageSha256/buildDeepRuntimePackage）；④能力探测与宿主合同（resolveRayTracingCapability/resolveRayTracingDecision/validateHostCapabilities/SessionAuthorityCoordinator）；⑤渲染包准备合同（prepareRenderPacket/prepareInstanceUpdate）。`package.json` 新增 `./host` 子入口（development→src、发布→dist，与既有子入口同模式）；`src/host.test.ts` smoke 3 例：预期值导出逐个非 undefined + `Object.keys` 断言导出面与清单一一对应（防漂移双向）+ 经入口真实调用运行包校验 fail-closed 双向。deep-engine build 后 dist/host.js 与 dist/host.d.ts 已生成，发布态子入口路径有效。
- **切片 2 动画控制域拆分**：`viewerEngineInteraction.ts` 695→666 行，10 个模型动画控制方法（hasAnimation/listAnimationClips/getAnimationPlayback/getModelAnimationPlaybackState/setModelAnimationPlaybackState/applyModelAnimationLoopPolicy/restartModelAnimationActions/updateCompletedModelAnimations/controlAnimation/transitionAnimationClip）迁入新文件 `viewerEngineAnimationControl.ts`（44 行，`ViewerEngineAnimationControl extends ViewerEngineCore` 抽象类，方法体逐字保留、继续委托 `viewerEngineAnimation.ts` 纯函数）；interaction 改为 `extends ViewerEngineAnimationControl` 继承混入，公开/保护签名与拆分前逐字一致，ViewerEngine 叶子类 API 零变化；import 链无环（AnimationControl→Core→Interaction）。
...
- **F5 Deep2D/GUI parity 收口三子项（本条目）**：①IME 组合输入——现状判定 Native 链已完整（`WinitImeAdapter`/`input_ime` 处理 Preedit/Commit/Cancel、`input_key` 组合闸、preedit 于 `input_content()` caret 处渲染占位、`select_ime_rect`/`input_caret_rect` 候选窗与 IME 定位），Web 文本框为浏览器原生 `<input>`（浏览器自带 IME）无需自定义 composition；补 runtime 层缺失测试段：组合期间物理键盘编辑被抑制（文档/数据/修订号零变化）、组合串更新不触发数据提交、Commit 后上屏、preedit 占位推动 caret 且内容层重新光栅化（`input_text_tests` 5/5 绿）。②触控一致性——现状判定 `WindowEvent::Touch` 全仓无处理（落 `_`），触屏点击/滑动完全无交互；新增 `app/touch_pointer.rs` 单指触点跟踪状态机（只跟踪首触点、第二指与乱序事件忽略、Cancel 释放不点击；先比对再释放防非主触点误清主触点），`window_events.rs` 提取 `pointer_move/press/release` 共用函数，鼠标 `CursorMoved`/`MouseInput` 与触控 `Touch` 消费同一批调用链（视频控件→图表→拾取→文本手势全同链），状态机单测 4/4。③音频——现状判定 Web `StreamVideo` 无条件 `video.muted=true` 覆盖作者静音意愿、Native MF `SetStreamSelection` 显式只选视频流且非静音在编译合同层 fail-closed（`native-video-audio-unavailable`/`[audio-output]`）；Web 实现 `audioTrackPlan`（作者静音→全程静音确定性不解锁；未静音+自动播放→静音起播+手势恢复有声；未静音+非自动→直接有声）与 `applyVideoAudioState`（音量夹取 [0,1]、非有限回退 1）纯函数并接线 `beginPlayback`/`retryPlayback`/autoplay 提示文案（dashboardMedia 15/15 绿）；Native 新增 `DashboardVideoAudioTrackProbe` 音轨探测（采样率），ffmpeg 8.0 本地合成 H.264+AAC 样本（6668B，SHA-256 与档案见 `tests/fixtures/README.md`）验证探测管道 2/2 绿（对照 line-loop.mp4 报告无音轨）。门禁：`cargo check --lib --bins` 0 error、lib 542/0、bin 275/0（首次全量 2 个 GPU/线程测试因并行会话构建负载环境性失败，单独与重跑均绿）、`tsc --noEmit` 0、vitest dashboardMedia 15/15 + delivery 777/777。诚实边界：真实音频出声（Native WASAPI/MF 音频流输出与音画同步）仍是登记 blocker，本切片只做 Web 音轨控制与 Native 探测显式化；触控真实硬件端到端未做（winit Touch 映射逻辑已测、物理触屏设备未验）；IME 真实候选窗人工视觉验证未做（逻辑层全覆盖）；jsdom 无法真实解码音频，Web 音轨出声未在无头测试中断言。

### 2026-09-23 自检（第三十一轮）附：author-grading 运行证据（解码接受级）
...
### 2026-09-25 终态快照(29 提交,基线 5c09e1af→439033b2,未 push,门禁双绿,dev 服务已关停)
