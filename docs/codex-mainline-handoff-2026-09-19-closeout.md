...
继续把 ⑥ D24–D28、⑦工业 S1–S6、⑧ DE26 readiness 的真实缺口补齐；②动态场景运行包已于 2026-09-19 晚间转 passed。已通过的 ①③④⑤ 只做必要回归。同时 R1–R5 超越波次已开工（见下）。

## 超越波次（2026-09-19 用户拍板，与收尾线并行）

依据 `docs/specs/de26-high-value-scope-analysis-2026-09-19.md` 第 7/8 节与 A01 政策增补。总目标：性能、效果、核心能力全面超越 Unity/UE/Three/Babylon 未来版本。**拍板记录：G1/G2/G3、G5、G6、G7、G8 批准；G9 多人协同、G4 地理坐标深度排除；G10 XR 留待。新增项不计入当前完成率。**

- **Wave 1（进行中）**：R2 跨后端着色 IR+Compute 基座（审计→设计→最小确定性 kernel 切片，首个切片因子代理并发上限推迟到 ⑧ 车道释放后重发）；R3 确定性产品化（剖切/选择/数据修订进 canonical 合同，跨端 digest 一致，子代理进行中）；G5/G6 政策已写入 A01 文档（WebGPU-first 主通道 + 季度重冻结）。
- **Wave 2（依赖就绪后）**：R4 万灯+GPU-driven 组合拳（先决 R2 compute 基座）；R5 AI 可校验编译链（先决 H09 事务卡收口）；G7 体积介质升级 E06。
- **持续**：R1 跨端实时 GI 生产化与 E03 P0 同线推进；季度重冻结按 G6 政策执行。
- **量化三轴（2026-09-19 用户追加，已入 DE26 文档 8.3）**：R6 性能跃升、R7 效果跃升、R8 体积瘦身——每项"基线→目标→季度重测"三段记账，未测不写达标。
- **工期口径（AI 高速并行不间断，2-3 车道 24h 轮转）**：第 1 周收尾线（⑥⑦⑧+480min soak+Native 对手）+ Wave 1；第 2 周 R2 收口、R4 启动、R1/E03 攻坚、R8 建账；第 3 周 R4 万灯 256→4096 实测对拍 Unity、R7 效果基线包逐格验收、R8 体积门禁生效；第 4 周全量对测与量化超越报告；第 5-6 周长跑稳定性与冻结。**约 4-6 周达到"对位能力全齐+对位场景实测数字领先"的可验收状态**；此后转入季度重测保持机制。
- 执行纪律：每季度用 A 组固定基准重测对位场景，以实测数字进报告，未测不写超越。经验教训：**长稳 soak 只能在零子代理运行时执行**——子代理的解析/构建负载会把个别样本 P95 打到 175ms+，触发严格门禁失败（已发生 3 次）。
