...
### 2. 素材线推进（十四批视觉审核，166 项全量清零，批准 7→89）

- 复用既有审核管线（`gate-source-b-review-batch.mjs` 采集 → 人工逐项三视角亲审 → `apply-source-b-visual-review.mjs` 校验应用），**14 批共 159 个模型**全部完成真实浏览器双主题两轮×三视角采集（每批 48/48 组、采集阶段零渲染异常；加上首轮已审 7 项，**166 项 catalog 全部审核完毕，零遗留**）：
  - 批次 2-14（`source-b-review-93D4Ut` 至 `source-b-review-soLKka`）：每批 12 项（末批 7 项），批准数 7/6/5/7/6/9/4/8/6/8/7/3，合计**新增批准 82 项**——涵盖阀门与管件、输送与物流、自动化与机器人、电气设备、暖通与工艺设备、动力设备、机械传动、工业建筑、车辆与载具、仪表与传感、产线与工艺、场景道具等 12 个业务类目。
- 素材库可用资产 **7 → 89**（166 项全部审核完毕）；**77 项保持待审**且每一项都有明确问题记录：白模材质族（约 20 项）、品牌/商标风险（Shell 涂装、蝙蝠侠标志、CLARKSON 印字、SG90 厂商标签、Fire-Chief 历史品牌、Kbahn 手柄印字、YouTube 奖牌等约 10 项）、集合/分解散排包（约 10 项）、用途不符的游戏/IP 模型（约 10 项）、取景或渲染不可辨（约 8 项）、X4122 驱动警告同族（6 项）等。
- 审核决定逐项留档：`docs/source-b-visual-review-batch{2..14}-2026-09-0{8,9}.json`（含逐项理由、缩略图 hash、署名许可核对）。
- 每批应用前检/后检：166 项目录 hash/bytes 校验、许可缺口 0、待审项不进公共包（`audit-source-b-preflight.mjs` 后检 issues 仅含 `visual-review-and-thumbnail-required: 77`）。
...
1. **素材 77 项保持待审是有记录的明确问题，不是未审核**（166 项已全部目视完毕）。待审项的解决路径各不相同：白模族需材质增强管线、品牌风险项需中性涂装衍生版本、集合包需拆分入库——均不能靠重拍或放宽门槛绕过，不改原 GLB。
