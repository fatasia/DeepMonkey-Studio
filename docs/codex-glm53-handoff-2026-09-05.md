...
20:22门槛补查：发布运行本地提交`817cb69`后，新增公开控制区真实文字对比度断言，复现亮色黄色选中字体仅2.16:1。已让运行控制的选中文字采用现有主题派生强调深色，不改品牌主色与作者画布。`published-application-fVTC0t`四组合/剪贴板拒绝/导航全通过，抽样控制文字均≥4.5:1，当前权威截图取此目录；最终Web1205再次通过。此修正不等同全站可访问性已验收。

## 19. 模型补充下载前置修复（ASSET-001增量）

- **已完成**：source-b同步CLI从业务逻辑拆至独立原子下载/许可与格式政策/同步服务；保留旧文件、严格实际字节、截断拒绝、GLB结构/几何/外部引用、实际图片解码转PNG、目录锁/原子检查点/坏目录拒绝、准确计数。14个新增聚焦测试，合并`pnpm assets:audit:test`24项通过；不得把测试图像有效当成真实模型视觉验收。
- 首次真实搜索48项均只有许可标签/ID，未下载；随后读取官方许可详情补版本核对，不把标签推断为CC4.0。许可详情缓存复用，下载签名接口调用也设配额上限。Windows缩略图转换曾因libvips文件锁失败，现由受限下载缓冲解码，反例通过。
- **已完成（下载，不等于入库）**：新增Robot Gripper Mechanism（trinityscsp，277392B）与Industrial Yellow Safety Barrier/Pole（Just8，4155016B），均CC-BY-4.0并有作者/模型页/许可URL；缓存164→166，2次下载调用/0错误。日志`.runtime-logs/codex-source-b-sync.log`，源目录留只读前态备份`catalog.before-codex-20260905-2028.json`。未改admin/admin、.env或存储拓扑，无新依赖/凭据写入。
- **本轮待办**：真实渲染/准确缩略图→source-b目录适配→许可信息随导入/交付保留→导入优化新资源闭环。两项仍review-required；旧164项未自动审核或宣称新增。glTF ZIP、多项资源/模板深度及原SIM/数据/AI/调试/性能待办保留。现有u19d-thumbnail-e2e使用原业务场景并从浏览器读token，不能复用；新验收继续独立数据/正常浏览器操作。
