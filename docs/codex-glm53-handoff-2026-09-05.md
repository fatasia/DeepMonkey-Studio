...
## 20. 素材真实入库、刷新与导入边界（ASSET-001增量）

- **已完成**：2项从缓存推进到统一目录。`平行机械夹爪`（trinityscsp）/`黄黑工业防撞柱`（Just8），CC-BY-4.0；真实渲染确认第二项是单柱而不是护栏，未照搬搜索名称。`source-b-render-GvHJ5v`的GLB哈希/干净PNG与人工视觉核对记录入本地缓存audit；旧164项未放行、未改文件，合计仍166个缓存，新增可用2项。
- **已完成**：`sourceBAssetCatalog`为独立适配器，许可/署名/结构证据/实际字节/模型与渲染PNG哈希一致才公开；坏JSON/null报错，不以空库遮掩。审核更新立即可见，加载失败不永久缓存。模型导入像材质一样核验实际上传副本；待审核409；6并发同内容导入只生成1个模型/转换任务。许可署名随ModelRecord保留，资源卡片折叠展示来源链接和作者。
- **已完成**：真实浏览器发现资源页刷新丢工作区/项目、关闭精选反而排除精选项，均根因修复；manager/optimizer用URL保留项目和工作区，合法标签白名单，迟到导入与刷新不跨项目。优化预览新增真实PNG下载，截取渲染帧且临时隐藏网格/操控器/灯代理，按钮不进入图片，原始模型不改。
- **已完成（配色复核）**：新门禁先测到亮色卡片1.1:1，再扩大到标题/说明/徽标测出3.8–4.43；已换统一主题令牌、徽标使用稳定背景，未重染缩略图。`source-b-library-pUMxr4`四组合全部通过，抽样最低暗5.53/亮4.56；图片/署名/导入→转换/原文件哈希/重复复用/刷新项目与资源页/精选关闭/503重试/迟到导入均是真实浏览器或接口断言。截图人工检查，长署名URL改为具名链接，原始署名文本仍完整保留在数据中。
- **已完成（回归）**：全仓`pnpm test`通过一轮（Web343文件1210项/API115文件465项及其余包）；追加坏null元数据后API466项通过。根typecheck/1826源文件≤800，Web/API生产构建通过，首屏306.7KiB/gzip99.7KiB不变。相邻`script-editor-qUke1Z`与`semantic-editor-M98mgO`各四组通过，正常开发Web的`report-final`也四组通过（旧证据复制到report-final-before-source-b-*保留）。新增预览按钮双主题/双宽度与最终全量复跑结果补在本节后，不将这些结果等同全项目后验收。
- 原服务仍web模式健康PID46300；admin/admin真实登录正常、4100资源查询200，两项确实可查。只更新指定source-b缓存审核与新增缩略图，不改.env、Postgres+MinIO、原场景和用户项目。源缓存/密钥仍gitignored，不加入提交，不push。
- **本轮待办（直接接续）**：优化器已有约800行，下一按状态所有权/Worker生命周期/文件URL/控件分离，先核对再改；已发现优化另存只上传GLB、不继承来源署名，需补导出/另存/发布追溯与新模型回场景。作者脚本运行隔离/范围/真调试、语义消费联动、SIM实际运行/Study、Agent恢复、性能和全站后验收全部继续；不能把“素材2项入库”说成整个模型流水线已完成。
- **已完成（最终复跑）**：`source-b-render-SyNc6Q`两模型×双主题×1440/980共8组通过，下载PNG真实无UI/辅助线，新增按钮文字对比度≥8.21；亮980和暗1440整页截图已人工复查。1条既有ANGLE精度诊断独立记录，不能称驱动警告为零。Web1210项再次通过，根类型检查/1826源文件门禁通过。此处只证明新增按钮和模型渲染；优化器原有绿色状态/长页面交互仍纳入下一批复核。

## 21. 并行收口与用量接手检查点

**新的接手首读：[NEXT-AGENT-START-HERE-2026-09-05.md](NEXT-AGENT-START-HERE-2026-09-05.md)**。用户要求耗尽前保留进度，已先本地提交检查点`84ca27c`；该文件明确未提交源码的所有权、待接线位置、构建/截图与最终测试的区别。只本地提交、不push，原账号/场景/存储不动。

- **已完成（Agent）**：明确数据集候选与可信HTTP决策失败有界手动恢复，`0967ef6`，四组合`agent-recovery-ea2f0X`和真实编排/查询聚焦已验证；详见AI-UP§7。不是在线模型验收，AI-2～5仍待办。
- **已完成（优化器限定批次，源码未提交）**：按任务/源/结果/回执所有权拆分，Worker死亡与迟到文件读取、取消轮询、失败换源保留结果、参数过期禁止导出、重复优化另存与取消后不重复上传；原始GLB不经Viewer重编码，保留动画及来源署名；服务端校验同项目父模型并保留衍生追溯，不污染目录去重身份。资源/PMREM/位图收尾，IBL默认可正确显示金属色。`optimizer-workflow-znjT1z`四组合每组17/17 Worker关闭，暗5.53/亮4.56，截图人工检查。主组件约800→312，烘焙UI与任务hook职责分离；连续RAF与全部格式并未完成性能验收。
- **已完成（作者隔离限定批次，源码未提交）**：`author-script-runtime-LE9JXj`四组合含真实3D青色像素、私有导航、单帧、异常/超时、停止保稿；运行0 PUT/仅显式保存1 PUT，每组6/6 Worker释放。相邻`script-editor-aU1HPs`/`script-playback-81tvnw`各四组；80聚焦。controller563→403；真源码断点仍待办。EX-005已完整回填。
- **本轮待办（语义最后复测）**：消费者真实取数/聚合/AND联动/钻取/级联和revision失效确认已经实现；首轮`semantic-consumers-IwAkFl`四组通过，人工截图抓到参数查询浮层一直加载，追加只读草稿选项求值/父→子批量参数顺序，38聚焦通过但仍须新生产bundle重跑。S1-003记录AskData等未完成边界。
- **本轮待办（正在实施）**：SIM-1a场景实体→既有PlantLite→Study→单一回放器动态覆盖层；公开应用/场景可达的模型署名和优化素材入场景发布；用户新追加ASSET-002素材浏览/优化/场景导入上下文无缝衔接。详细文件和恢复点见接手首读及两组checkpoint/spec，不以代码存在替代浏览器验收。
- **项目级后验收**：23:20全仓测试API492通过，Web1240通过/1失败，失败是优化器GLB读取绕过统一资源传输边界。已改为复用viewerAssetTransport，保持取消与有界超时，不扩大例外；架构+原始GLB+资源传输13项随后通过，**仍待最终Web/全仓重跑**。并行typecheck发现的接口中间态由各组修复，不误报成功。最新结构快照1873文件，56个>500、0个>800，生产TS/JS函数276个>80；代码治理未全完成。
- **明确排除**：GPT接入暂停、两个定时任务PAUSED、§28继承；未兑换额度重置。页面/交互/代码/功能/性能/体验六项门槛都保留，未声称全面超过ThingJS/Unity/Siemens。
