...
独立重跑原3文件30项通过，再补两项机制回归：64次透明倍率更新只在渲染模式变化时增加材质版本；原先关闭的深度写入在恢复和无穷倍率输入后仍关闭。最终3文件32项与Web类型检查通过。

主线查看修复后的 `dark-1920-full-overview.png` 和 `light-1280-pair-overview.png`：蓝绿外壳能看到内部部件，并排两个实例均可见且没有模型裁切。此次是保存截图的复核，不计新增浏览器轮次；灰银差异仍以源数值证据为准。既有驱动warning、未评分维度和插件克隆边界保持不变。
