...
function Get-BimStudioSigningCertificate {
    $subject = "CN=Deep Monkey Studio Internal"
...
foreach ($targetVersion in $versions) {
    ...
    Write-Host "[Deep Monkey Studio] Revit $targetVersion Add-in 已安装：$assemblyPath" -ForegroundColor Green
    Write-Host "[Deep Monkey Studio] Add-in 已使用当前用户的 Deep Monkey Studio Internal 证书签名。" -ForegroundColor Green
}

Write-Host "[Deep Monkey Studio] 常驻 Worker 已发布：$(Join-Path $PublishDirectory 'BimStudio.RevitWorker.exe')" -ForegroundColor Green
Write-Host "[Deep Monkey Studio] Worker 缓存目录：$WorkerCacheRoot" -ForegroundColor Green
Write-Host "[Deep Monkey Studio] 请重启已打开的 Revit，使 Add-in 生效。" -ForegroundColor Yellow
