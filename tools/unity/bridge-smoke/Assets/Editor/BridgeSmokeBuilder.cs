...
public static class BridgeSmokeBuilder
{
    public static void Validate()
    {
        ...
        Debug.Log($"BIM Studio Unity bridge package validation succeeded: {Application.unityVersion}");
    }
    ...
    public static void Build()
    {
        ...
        if (!BimStudio.Bridge.Editor.BimStudioProjectSetup.PrepareForExport(out _, out var preparationError))
            throw new BuildFailedException("BIM Studio automatic project preparation failed: " + preparationError);
        AssertPrepared();
        ...
        if (report.summary.result != UnityEditor.Build.Reporting.BuildResult.Succeeded)
            throw new BuildFailedException($"BIM Studio Unity bridge smoke build failed: {report.summary.result}");
        ...
        Debug.Log($"BIM Studio Unity bridge smoke build succeeded: {report.summary.totalSize} bytes");
    }
    ...
    private static void PrepareProject()
    {
        ...
        if (!BimStudio.Bridge.Editor.BimStudioProjectSetup.EnsureActiveSceneInBuildSettings(out var error))
            throw new BuildFailedException("BIM Studio scene preparation failed: " + error);
        AssertPrepared();
    }
    ...
    private static void AssertPrepared()
    {
        ...
        if (bridgeObject == null || bridgeObject.GetComponent<BimStudioBridge>() == null)
            throw new BuildFailedException("BIM Studio automatic setup did not create the Bridge object.");
        ...
        if (manifest == null) throw new BuildFailedException("BIM Studio automatic setup did not create the Manifest asset.");
        ...
        if (System.Array.Find(manifest.dataLayers, item => item.key == "telemetry") == null ||
            ...
            throw new BuildFailedException("BIM Studio automatic manifest discovery did not find every no-code binding.");
        if (string.IsNullOrWhiteSpace(actionBinding.objectId))
            throw new BuildFailedException("BIM Studio automatic manifest discovery did not assign a stable business object ID.");
        var validation = BimStudio.Bridge.Editor.BimStudioManifestValidator.Validate(manifest);
        if (!validation.IsValid) throw new BuildFailedException("BIM Studio no-code binding validation failed: " + string.Join("; ", validation.errors));
        var preparedScene = System.Array.Find(EditorBuildSettings.scenes, item => item.enabled && item.path == "Assets/BridgeSmoke.unity");
        if (preparedScene == null) throw new BuildFailedException("BIM Studio automatic setup did not enable the active scene in Build Settings.");
    }
}
