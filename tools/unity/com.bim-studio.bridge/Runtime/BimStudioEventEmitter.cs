...
{
    /// <summary>
    /// No-code Unity-to-Deep Monkey Studio event bridge. Add this component to a button,
    ...
    public sealed class BimStudioEventEmitter : MonoBehaviour, IBimStudioEventContractBinding
    {
        ...
        [Tooltip("发送给 Deep Monkey Studio 的 JSON；可以是对象、数组、数字、布尔值、字符串或 null。")]
        ...
        public void Emit()
        {
            ...
            if (name.Length == 0)
            {
                BimStudioEvents.Error($"{gameObject.name} 的 Deep Monkey Studio 事件名称为空。");
                return;
            }
        ...
        {
            var name = (eventName ?? string.Empty).Trim();
            if (name.Length == 0) { BimStudioEvents.Error($"{gameObject.name} 的 Deep Monkey Studio 事件名称为空。"); return; }
