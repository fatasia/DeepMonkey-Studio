...
namespace BimStudio.Bridge.Editor
{
    ...
    {
        [MenuItem("Deep Monkey Studio/一键构建并导出 WebGL ZIP", priority = 2)]
        public static void BuildAndExport()
        {
            if (!BimStudioProjectSetup.PrepareForExport(out var manifest, out var preparationError))
            {
                EditorUtility.DisplayDialog("Deep Monkey Studio", preparationError, "确定");
                return;
            }
            ...
            if (!validation.IsValid)
            {
                ...
                EditorUtility.DisplayDialog("Deep Monkey Studio", "请先修复集成清单中的问题：\n\n" + string.Join("\n", validation.errors), "打开集成清单");
                return;
            }
            ...
            {
                EditorUtility.DisplayDialog("Deep Monkey Studio", "没有可导出的已保存场景。", "确定");
                return;
            }

            var defaultName = string.IsNullOrWhiteSpace(PlayerSettings.productName) ? "UnityWebGL" : SafeFileName(PlayerSettings.productName);
            var zipPath = EditorUtility.SaveFilePanel("导出 Deep Monkey Studio Unity WebGL ZIP", LastExportDirectory(), defaultName + ".zip", "zip");
            ...
            try
            {
                ...
                EditorUtility.DisplayDialog("Deep Monkey Studio", "WebGL ZIP 已生成，可直接拖入 Deep Monkey Studio：\n\n" + zipPath, "完成");
            }
            catch (Exception exception)
            {
                Debug.LogException(exception);
                EditorUtility.DisplayDialog("Deep Monkey Studio 导出失败", exception.Message, "打开控制台");
                throw;
            }
            finally
            {
        ...
        public static void BuildToZip(string zipPath, string[] scenes)
        {
            ...
            try
            {
                ...
                EditorUtility.DisplayProgressBar("Deep Monkey Studio", "正在构建 Unity WebGL…", 0.2f);
                ...
                EditorUtility.DisplayProgressBar("Deep Monkey Studio", "正在打包可直接导入的 ZIP…", 0.9f);
