...
namespace BimStudio.Bridge.Editor
{
    ...
    public sealed class BimStudioManifestAssetEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField("Deep Monkey Studio · Unity WebGL 集成清单", EditorStyles.boldLabel);
        ...
        private static void DrawValidation(BimStudioManifestValidation validation)
        {
            ...
            if (validation.errors.Count == 0 && validation.warnings.Count == 0) EditorGUILayout.HelpBox("集成清单已通过 Deep Monkey Studio 发布校验。", MessageType.Info);
        }

        private static void LogValidation(BimStudioManifestValidation validation)
        {
            foreach (var error in validation.errors) Debug.LogError($"[Deep Monkey Studio Manifest] {error}");
            foreach (var warning in validation.warnings) Debug.LogWarning($"[Deep Monkey Studio Manifest] {warning}");
            if (validation.IsValid) Debug.Log("[Deep Monkey Studio 集成清单] 校验通过。");
        }
    ...
    public static class BimStudioProjectSetup
    {
        ...
        [MenuItem("Deep Monkey Studio/准备项目（通信桥与集成清单）", priority = 1)]
        ...
        public static void EnsureBridgeObject()
        {
            ...
            if (bridge == null)
            {
                ...
                Undo.RegisterCreatedObjectUndo(gameObject, "Create Deep Monkey Studio Bridge");
                EditorSceneManager.MarkSceneDirty(gameObject.scene);
            }
            else if (bridge.gameObject.name != "BimStudioBridge")
            {
                Undo.RecordObject(bridge.gameObject, "Rename Deep Monkey Studio Bridge");
