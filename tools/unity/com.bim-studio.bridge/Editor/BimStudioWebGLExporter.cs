...
namespace BimStudio.Bridge.Editor
{
    public static class BimStudioWebGLExporter
    {
        ...
        public static void ExportManifest(BuildTarget target, string outputPath)
        {
            ...
            if (settings == null) throw new BuildFailedException("缺少 Deep Monkey Studio 集成清单，请先运行“Deep Monkey Studio/准备项目（通信桥与集成清单）”。");
            var validation = BimStudioManifestValidator.Validate(settings);
            if (!validation.IsValid) throw new BuildFailedException("Deep Monkey Studio 集成清单校验失败：\n" + string.Join("\n", validation.errors));
        ...
        private static void InstallBrowserBridge(string outputPath)
        {
            ...
            if (package == null) throw new BuildFailedException("无法定位 Deep Monkey Studio Bridge 插件目录。");
            ...
            if (!File.Exists(source) || !File.Exists(indexPath))
                throw new BuildFailedException("Deep Monkey Studio Bridge 找不到 unity-bridge.js 或构建生成的 index.html。");
            ...
            if (loaderIndex < 0)
                throw new BuildFailedException("Deep Monkey Studio Bridge 无法在 index.html 中定位 Unity 播放器加载器。");
            ...
            if (promiseIndex < 0)
                throw new BuildFailedException("Deep Monkey Studio Bridge 无法在 index.html 中定位 Unity 播放器创建回调。");
