...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/polynomial-bezier-2026-09-17-v1/nonrect');
...
test('four actual nonrectangular cylinder trims preserve source PointAt and GLB identity',async()=>{
  ...
  const completed=completeBrepParts(noCache.objects.find((o:any)=>o.cadIr),source.metersPerUnit);assert.equal(completed.parts.length,33);
