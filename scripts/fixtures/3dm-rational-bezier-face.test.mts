...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/rational-bezier-2026-09-17-v1');
...
test('twelve real rational faces preserve original PointAt, source knots, boundary topology and whole-model preview',async()=>{
  ...
  const parts=completeBrepParts(noCache.objects.find((o:any)=>o.cadIr),.001);assert.equal(parts.parts.length,29);
