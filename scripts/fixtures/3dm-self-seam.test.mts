...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/plane-isocurve-2026-09-17-v1/self-seam');
...
test('seven real periodic seams have independent source identities and matching oriented mesh sides',async()=>{
  ...
  assert.equal(audit.shared.filter(e=>!e.conforming).length,60);assert.equal(audit.allFacesPresent,true);
  ...
  assert.equal(sha(glb.bytes),'46a61f9c6cbf074babfa96d05841140118488435d1de739a5038807e5625a507');
  ...
  const evidence={sourceSha256,sourceUrl:'https://github.com/mcneel/opennurbs/blob/v8.35.26251.13001/example_files/V4/v4_MechPartA.3dm',archiveVersion:source.archiveVersion,
    ...
    seams:audit.seams,unverified:audit.unverified,nonconformingShared:60,glbSha256:sha(glb.bytes),status:glb.sidecar.status};
