...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/common-knot-2026-09-17-v1/self-seam');
...
test('seven real periodic seams have independent source identities and matching oriented mesh sides',async()=>{
  ...
  assert.equal(audit.shared.filter(e=>!e.conforming).length,53);assert.equal(audit.allFacesPresent,true);
  ...
  assert.equal(sha(glb.bytes),'fd72669a9cd020ea95323a54310d445576ff2e8d42175fdc7e0ee3574c74823f');
  ...
  const evidence={sourceSha256,sourceUrl:'https://github.com/mcneel/opennurbs/blob/v8.35.26251.13001/example_files/V4/v4_MechPartA.3dm',archiveVersion:source.archiveVersion,
    ...
    seams:audit.seams,unverified:audit.unverified,nonconformingShared:53,glbSha256:sha(glb.bytes),status:glb.sidecar.status};
