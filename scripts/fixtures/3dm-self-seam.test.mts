...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/bezier-interval-2026-09-17-v1/self-seam');
...
test('seven real periodic seams have independent source identities and matching oriented mesh sides',async()=>{
  ...
  assert.equal(audit.shared.filter(e=>!e.conforming).length,54);assert.equal(audit.allFacesPresent,true);
  ...
  assert.equal(sha(glb.bytes),'86cc0d49977f93c9d35704bd7088aea0dfb0b81389f9bb9364e355840817c9f2');
  ...
  const evidence={sourceSha256,sourceUrl:'https://github.com/mcneel/opennurbs/blob/v8.35.26251.13001/example_files/V4/v4_MechPartA.3dm',archiveVersion:source.archiveVersion,
    ...
    seams:audit.seams,unverified:audit.unverified,nonconformingShared:54,glbSha256:sha(glb.bytes),status:glb.sidecar.status};
