...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/self-seam-2026-09-17-v1');
...
test('seven real periodic seams have independent source identities and matching oriented mesh sides',async()=>{
  ...
  assert.equal(audit.shared.filter(e=>!e.conforming).length,65);assert.equal(audit.allFacesPresent,true);
  ...
  assert.equal(sha(glb.bytes),'0a4dfc1fec93cfbc2a8e2c73faf637acde533c510225a0827681801774eeae1c');
  ...
  const evidence={sourceSha256,sourceUrl:'https://github.com/mcneel/opennurbs/blob/v8.35.26251.13001/example_files/V4/v4_MechPartA.3dm',archiveVersion:source.archiveVersion,
    ...
    seams:audit.seams,unverified:audit.unverified,nonconformingShared:65,glbSha256:sha(glb.bytes),status:glb.sidecar.status};
