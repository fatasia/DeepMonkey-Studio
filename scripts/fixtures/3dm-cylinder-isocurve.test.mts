...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/cylinder-isocurve-2026-09-17-v1');
...
test('three actual cylinder-plane curves share source chains without changing old points or normals',async()=>{
  const before=JSON.stringify(ir),result=completeBrepParts(object,.001),records=result.sourceEdgeSynchronizations.filter(r=>[1,37,42].includes(r.edge));
  assert.deepEqual(records.map(r=>r.edge),[1,37,42]);assert.equal(result.parts.length,41);assert.equal(result.boundaryAudit.shared.filter(e=>!e.conforming).length,56);
  ...
  assert.equal(JSON.stringify(ir),before);assert(references>=114&&maxReferenceError<1e-10);
