...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/bezier-interval-2026-09-17-v1');
...
test('five actual full and partial cylinder-plane curves share source chains without changing old points or normals',async()=>{
  ...
  assert.deepEqual(records.map(r=>r.edge),[1,34,37,39,42]);assert.equal(result.parts.length,41);assert.equal(result.boundaryAudit.shared.filter(e=>!e.conforming).length,54);
