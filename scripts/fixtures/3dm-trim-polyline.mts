...
const point=(p:number[])=>p.slice(0,2).map(x=>x/p[2]);
...
function segmentDistance(p:number[],a:number[],b:number[]) {
  const u=b.map((x,i)=>x-a[i]),d=u[0]*u[0]+u[1]*u[1];
  const t=d?Math.max(0,Math.min(1,((p[0]-a[0])*u[0]+(p[1]-a[1])*u[1])/d)):0;
...
export function trimPolyline(curve:any,domain:number[],reversed:boolean,tolerance:number) {
  check(curve?.dimension===2 && Number.isInteger(curve.degree) && curve.degree>=1 && curve.degree<=8
  ...
  check(points.length>p && knots.length===points.length+p+1 && knots.every((k:number,i:number)=>Number.isFinite(k)&&(i===0||k>=knots[i-1]))
    && points.every((cv:number[])=>cv.length===3&&cv.every(Number.isFinite)&&cv[2]>0),'invalid-trim-nurbs');
  ...
  const result:number[][]=[]; let maxBound=0;
  const append=(control:number[][],depth:number)=>{
    ...
    if(bound<=tolerance) {
      ...
      if(!result.length) result.push(a); result.push(b); maxBound=Math.max(maxBound,bound); return;
    }
    check(depth<MAX_DEPTH,'trim-subdivision-budget');
    const [left,right]=split(control,.5); append(left,depth+1); append(right,depth+1);
  };
