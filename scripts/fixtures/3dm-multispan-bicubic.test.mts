...
import { tessellateMultispanBicubicFace } from './3dm-bicubic-face.mts';
...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/multispan-bicubic-2026-09-17-v1');
...
test('actual multispan bicubic face preserves original PointAt, topology, orientation and GLB',async()=>{
  const part=tessellateMultispanBicubicFace(ir,1,.001),mesh=part.mesh,surface=ir.surfaces[ir.faces[1].surface];
  ...
  assert(count>=49);assert(error<=part.audit.physicalBoundMm);
  ...
  for(let i=0;i<triangles.length;i++)for(const vertex of mesh.triangles[i])assert(triangles[i].getNormal(new Vector3()).dot(new Vector3(...mesh.normals[vertex]))>0);
  for(const t of mesh.triangles)for(let axis=0;axis<2;axis++)for(const knot of surface.knots[axis]){
    const values=t.map(i=>part.audit.uv[i][axis]);assert(!(Math.min(...values)<knot-1e-11&&Math.max(...values)>knot+1e-11));}
  assert(part.audit.windingRefinement.inserted>0);
  ...
  const glb=await export3dmGlb(noCache,sha(readFileSync(path)));assert(glb.bytes);assert.equal(glb.sidecar.status,'partial-geometry-preview');mkdirSync(out,{recursive:true});
  ...
  assert.equal(json.meshes.flatMap((m:any)=>m.primitives).find((p:any)=>p.extras.brepFaceIndex===1).extras.geometrySource,'cad-ir-multispan-bicubic');
  const evidence={sourceSha256:sha(readFileSync(path)),sourceUrl:'https://github.com/mcneel/opennurbs/blob/v8.35.26251.13001/example_files/V4/v4_MechPartB.3dm',
    ...
    face:1,vertices:mesh.positions.length,triangles:triangles.length,referenceCount:count,maxPointAtDistanceMm:error,
    physicalBoundMm:part.audit.physicalBoundMm,derivativeBounds:part.audit.derivativeBounds,divisions:part.audit.divisions,windingRefinement:part.audit.windingRefinement,audit,glbSha256:sha(glb.bytes),status:glb.sidecar.status};
...
test('controlled hole stays empty and face reversal flips exact source normals',()=>{
  const copy=structuredClone(ir),face=copy.faces[1],surface=copy.surfaces[face.surface],center=surface.domain.map((d:number[])=>(d[0]+d[1])/2),r=.01;
  ...
  copy.loops.push({face:1,type:2,trims});face.loops.push(loop);const part=tessellateMultispanBicubicFace(copy,1,.001);
  assert.equal(part.audit.holeCount,1);assert.equal(contains(part,center),false);
  const before=tessellateMultispanBicubicFace(ir,1,.001);const reversed=structuredClone(ir);reversed.faces[1].reversed=!reversed.faces[1].reversed;
  const after=tessellateMultispanBicubicFace(reversed,1,.001);
  let matched=0;
  for(let i=0;i<after.mesh.positions.length;i++){const p=after.mesh.positions[i],index=before.mesh.positions.findIndex(q=>Math.hypot(...p.map((x,j)=>x-q[j]))<1e-9);
    if(index>=0){matched++;assert(after.mesh.normals[i].every((x,j)=>Math.abs(x+before.mesh.normals[index][j])<1e-10));}}
  assert(matched>=3100);
});
test('single-span, rational, invalid knots, singular normal and unbounded precision refuse',()=>{
  assert.throws(()=>tessellateMultispanBicubicFace(ir,3,.001),/unsupported-single-bicubic/);
  assert.throws(()=>tessellateMultispanBicubicFace(ir,1,0),/physical-unit/);
  assert.throws(()=>tessellateMultispanBicubicFace(ir,1,10000),/budget/);
  for(const change of [(s:any)=>s.rational=true,(s:any)=>s.knots[0][2]+=.01,(s:any)=>s.knots[0][5]=s.knots[0][4],
    (s:any)=>s.controlPoints[0][0]=NaN,(s:any)=>s.controlPoints=s.controlPoints.map(()=>[0,0,0])]){
    const copy=structuredClone(ir);change(copy.surfaces[copy.faces[1].surface]);assert.throws(()=>tessellateMultispanBicubicFace(copy,1,.001));}
});
