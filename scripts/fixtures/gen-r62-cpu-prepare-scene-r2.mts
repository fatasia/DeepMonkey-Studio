...
// R6-2 r2 变体:输出目录与实例数均可由环境变量覆盖,默认写 r2 目录。
const outDir = resolve(repo, process.env.R62_OUT ?? "test-output/r6-2-cpu-prepare-20260919-r2");
...
`);
// esbuild 在本仓是 vite 的传递依赖(.pnpm 存储),不一定有顶层链接;先按
// 常规路径解析,失败时回退 .pnpm 目录(readdirSync 取实际版本,不猜版本号)。
let esbuildRequire;
try {
  esbuildRequire = createRequire(resolve(repo, "apps/web/package.json"));
  esbuildRequire("esbuild");
} catch {
  const { readdirSync } = await import("node:fs");
  const esbuildDir = readdirSync(resolve(repo, "node_modules/.pnpm")).find((entry) =>
    entry.startsWith("esbuild@"),
  );
  if (!esbuildDir) throw new Error("esbuild not found in node_modules/.pnpm");
  esbuildRequire = createRequire(resolve(repo, "node_modules/.pnpm", esbuildDir, "node_modules/esbuild/package.json"));
}
