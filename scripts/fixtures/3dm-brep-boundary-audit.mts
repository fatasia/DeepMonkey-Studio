...
export function auditBrepBoundaries(ir:any,parts:any[],tolerance=1e-8){
  ...
  const shared:any[]=[],unverified:any[]=[];
  for(let edge=0;edge<uses.length;edge++){
    ...
    if(faces[0]===faces[1]){unverified.push({edge,reason:'self-seam-not-audited',faces});continue;}
    ...
    shared.push({edge,faces,segmentCounts:pair.map(s=>s.segments.length),maxSampledDeviation,unmatchedSegments,
      orientationErrors,invalidBoundarySegments,conforming:!unmatchedSegments&&!orientationErrors&&!invalidBoundarySegments});
  }
  const allFacesPresent=ir.faces.length===byFace.size&&ir.faces.every((_:any,i:number)=>byFace.has(i));
  return {tolerance,allFacesPresent,shared,unverified,
    status:allFacesPresent&&shared.length>0&&!unverified.length&&shared.every(s=>s.conforming)?'conforming-two-sided-boundary':'unverified-or-nonconforming-boundary'};
}
