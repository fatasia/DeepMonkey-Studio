...
门禁保留外部消费者与归档，便于另一位开发者检查 `package.json`、安装产物、默认出口和编译输出。精确目录、命令日志、声明解析路径、浏览器输入图、运行结果与截图记录在 `test-output/codex-2026-09-06/sdk-consumer-*/report.json`。不自动清理用户目录；这些临时消费者不应提交。
