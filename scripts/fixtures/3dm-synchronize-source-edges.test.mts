...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/cylinder-isocurve-2026-09-17-v1/isocurve');
...
test('four actual zero-tolerance source curves synchronize without moving existing vertices or losing any face',async()=>{
  ...
  assert.equal(initial.shared.filter(e=>!e.conforming).length,60);
  assert.equal(complete.boundaryAudit.shared.filter(e=>!e.conforming).length,56);assert.equal(complete.boundaryAudit.unverified.length,0);
  assert.equal(complete.boundaryAudit.seams.length,7);assert(complete.boundaryAudit.seams.every(e=>e.conforming));let references=0,maxReferenceError=0;
  for(const record of complete.sourceEdgeSynchronizations.filter(r=>repairedEdges.includes(r.edge))){const c=ir.curves3d[ir.edges[record.edge].curve3d];
