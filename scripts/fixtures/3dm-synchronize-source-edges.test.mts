...
const root=resolve(import.meta.dirname,'../..'),out=resolve(root,'test-output/industrial-3dm/bezier-interval-2026-09-17-v1/isocurve');
...
test('four actual zero-tolerance source curves synchronize without moving existing vertices or losing any face',async()=>{
  ...
  assert.equal(initial.shared.filter(e=>!e.conforming).length,58);
  assert.equal(complete.boundaryAudit.shared.filter(e=>!e.conforming).length,54);assert.equal(complete.boundaryAudit.unverified.length,0);
