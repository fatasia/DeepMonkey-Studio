...
async function main() {
  ...
  if (sampleChart) {
    ...
    page.nodes = [{ id: "portable-author-bar", kind: "data-widget", zIndex: 0,
      ...
      widget: { title: "Authored output", key: "author.value", unit: headingChart ? "MW" : "", type: "bar", field: "value",
        ...(headingChart ? { fontSize: 18 } : {}),
  ...
  try {
    ...
    if (sampleChart) {
      ...
      if (headingChart) {
        assert.equal(record.candidate.freezeManifest.resources.filter(resource => resource.kind === "font").length, 2);
