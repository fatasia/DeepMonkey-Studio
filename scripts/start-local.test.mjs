...
test("help is read-only and rejects unknown options", async () => {
  ...
  assert.match(help.stdout, /Industrial Studio 本地启动器/);
...
test("refuses an occupied API port when the listener is not Industrial Studio", async () => {
  ...
  try {
    ...
    assert.match(result.stderr, new RegExp(`API 端口 ${address.port} 已被非 Industrial Studio 服务占用`));
  } finally {
    listener.close();
...
test("check mode returns a non-zero health result without starting services", async () => {
  const listener = createServer((_request, response) => {
    ...
    response.end("not Industrial Studio");
  });
  listener.listen(0, "127.0.0.1");
  await once(listener, "listening");
  const address = listener.address();
...
async function run(args, apiPort = 41999) {
  try {
