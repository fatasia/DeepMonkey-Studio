...
- Switch the project license to standard MIT text plus ethical restrictions (`DMS-MIT-ER-1.0`, 2026-09-25 owner decision): rewrite `LICENSE` and `LICENSE-RESTRICTIONS.md` (Hippocratic-style human-rights carve-out), refresh `LICENSE.zh-CN.md`, `LICENSING.md`, README wording, the `AGENTS.md` governance section, contribution terms, Cargo `license-file` declarations, and governance assertions; the predecessor `DMCSL-1.0` commercial-use/restricted-organization clauses are retired.
- Ship a root-level `docker-compose.yml` for the fixed PostgreSQL + MinIO storage topology with pinned upstream images, explicitly named persistent volumes (`bim_studio_postgres_data` / `bim_studio_minio_data`), `pg_isready` and `mc ready` health checks, and required-credential injection through `.env` only (extend `.env.example` with `MINIO_ROOT_*`, port and volume entries); add `docs/deployment.md` covering one-command startup, initialization, health checks, backup/restore and upgrade cautions, and reference it from a new "Quick deploy" section in both READMEs. The application itself still runs bare-metal through `pnpm studio` (no application image is included).
- Freeze the exact Windows player bytes used for Native scene window verification into each publication, so later service upgrades cannot break historical downloads or silently substitute another engine build.
...
- Stop cancelled Dashboard downloads after manifest lookup, before allocating and serializing offline archives.
...
- Replace X packages by dropping them into an explicitly opened X window; reuse the existing background loader and GPU candidate transaction while keeping ordinary viewers isolated.
