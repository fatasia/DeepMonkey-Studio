...
mod local_api;
mod scene_viewer_launcher;

use local_api::{LocalApiManager, start_local_api};
...
pub fn run() {
    if scene_viewer_launcher::handle_inspection_cli() {
        return;
    }
    tauri::Builder::default()
        .register_uri_scheme_protocol("scene-viewer", scene_viewer_launcher::protocol_response)
        .manage(LocalApiManager::default())
        .setup(|app| {
            let mut main_config = app
                ...
                .clone();
            let scene_viewer_payload = scene_viewer_launcher::embedded_payload()
                .map_err(|error| format!("Three WebView 启动器负载无效：{error}"))?;
            let read_only = scene_viewer_payload.is_some();
            if let Some(payload) = scene_viewer_payload {
                main_config.label = "scene-viewer".to_owned();
                main_config.url = WebviewUrl::CustomProtocol(
                    Url::parse("scene-viewer://localhost/index.html")
                        .expect("valid scene-viewer URL"),
                );
                main_config.title = payload.product_name().to_owned();
                main_config.decorations = true;
                main_config.theme = Some(tauri::Theme::Dark);
            }
            ...
            WebviewWindowBuilder::from_config(app, &main_config)?
                ...
                .on_new_window(move |url, features| {
                    ...
                    // Keep the native WebView surface dark before the editor document paints.
                    // This mirrors apps/web/src/styles/base.css --bg-0.
                    .background_color(tauri::webview::Color(11, 17, 20, 255))
