...
async function installInputBinding(page: Page, width: number, height: number): Promise<void> {
...
async function createProducer(page: Page, framesPerSecond: number, codecPreferences: Codec[], iceServers: unknown[], iceTimeoutMs: number): Promise<RTCSessionDescriptionInit> {
...
function parseInput(raw: unknown): InputEvent | undefined {
