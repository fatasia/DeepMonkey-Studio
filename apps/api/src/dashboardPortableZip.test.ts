...
describe("Dashboard Windows portable ZIP", () => {
  ...
  it("writes a real deterministic ZIP with identical verified bytes and complete hashes", async () => {
    ...
    // 许可证法定名称按 AGENTS.md 钉死为 Deep Monkey Studio License — MIT with Ethical Restrictions，
    // 产品显示名改 DeepMonkey Studio 不改变该名称，断言对齐 LICENSE 原文。
    expect(await zip.file("LICENSE")!.async("text")).toContain("Deep Monkey Studio License — MIT with Ethical Restrictions");
