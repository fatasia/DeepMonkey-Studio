...
export async function registerThreeSceneViewerExecutableRoutes(app: FastifyInstance, dependencies: { store: MetadataStore; builderScript: string }) {
  app.post<{ Params: { projectId: string; sceneId: string; version: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const executable = await buildThreeSceneViewerExecutable(dependencies.builderScript, bytes, signal);
