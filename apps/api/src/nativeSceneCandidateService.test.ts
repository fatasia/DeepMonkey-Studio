...
async function fixture() {
 ...
 return { sourceSha256: artifact, scope: "native-window", nonce: "test", executableSha256: digest, verifiedAt: at, report: { gpuErrorsClean: true } } as NativeSceneWindowEvidence; });
 const service = createNativeSceneCandidateService({ store, objects: new LocalObjectStore(dataDir), dataDir, prepare, assess, verifyWindow });
 return { dataDir, store, scene, compiled, report, capture, prepare, assess, verifyWindow, service, artifact };
}
