...
// 用法:node scripts/build-android-template.mjs --out <apk路径> [--abi aarch64-v8a,x86_64]
...
for (const abi of abis) {
  const so = path.join(shellDir, "target", jniLibAbi[abi], "release", soName);
  if (!existsSync(so)) throw new Error(`缺少 ${so};先跑 scripts/android-so-build 段的 cargo build --release --target ${jniLibAbi[abi]}`);
}
...
try {
  ...
  const zip = new JSZip();
  const base = new JSZip().loadAsync(readFileSync(baseApk));
  const loaded = await base;
  ...
  for (const abi of abis) {
    const so = readFileSync(path.join(shellDir, "target", jniLibAbi[abi], "release", soName));
    zip.file(`lib/${abi}/${soName}`, so, { date: new Date("2000-01-01T00:00:00Z"), createFolders: false, compression: "STORE" });
  }
