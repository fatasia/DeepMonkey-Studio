// one-off: serve probe9 page and print results
...
const server = createServer((request, response) => {
  ...
  if (pathname === "/") { filePath = resolve(probeDir, "probe9.html"); contentType = "text/html; charset=utf-8"; }
  else if (pathname === "/probe9.js") { filePath = resolve(probeDir, "probe9.js"); contentType = "text/javascript; charset=utf-8"; }
...
try {
  ...
  try {
    ...
    throw new Error("probe9 timeout");
  }
  const results = await page.evaluate(() => window.__probeResults);
  writeFileSync("test-output/a01x-babylon-capture-probe/probe9-results.json", JSON.stringify({ results, errors }, null, 2), "utf8");
