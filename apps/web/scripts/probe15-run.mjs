// one-off: serve probe15 page and print results
...
const server = createServer((request, response) => {
  ...
  if (pathname === "/") { filePath = resolve(probeDir, "probe15.html"); contentType = "text/html; charset=utf-8"; }
  else if (pathname === "/probe15.js") { filePath = resolve(probeDir, "probe15.js"); contentType = "text/javascript; charset=utf-8"; }
...
try {
  ...
  await page.waitForFunction(() => window.__probeDone === true, undefined, { timeout: 540_000 });
  const results = await page.evaluate(() => window.__probeResults);
  const fs = await import("node:fs");
  fs.writeFileSync(resolve(probeDir, "probe15-results.json"),
