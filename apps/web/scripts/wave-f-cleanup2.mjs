// 清理 v4:删除取证残留 → 轮询等待自动保存落盘(状态栏"所有修改已保存")→ 复核后退出。
...
// 等自动保存落盘:状态栏出现"所有修改已保存"且连续稳定。
await page.waitForFunction(() => document.body.textContent?.includes("所有修改已保存"), undefined, { timeout: 30000 }).catch(() => {});
