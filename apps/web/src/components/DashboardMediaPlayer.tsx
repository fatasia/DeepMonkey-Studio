...
import { advancePlaybackWatchdog, browserMediaUrl, effectivePlaybackOptions, initialPlaybackWatchdogState, isHlsUrl, isPlaybackWaiting, liveSourceUrl, mediaReconnectDelay, mediaReferenceFromWidget, mediaUrlErrorMessage, shouldRequestAutoplay } from "./dashboardMedia";
...
export function DashboardImage({ widget, locale }: { widget: DashboardDataWidgetConfig; locale: AppLocale }) {
  ...
  if (failed) return <MediaMessage locale={locale} icon="image" message={tr(locale, "图片加载失败，请检查地址、权限或 CORS。", "Image loading failed. Check the URL, permissions, or CORS.")} onRetry={() => { setFailed(false); setAttempt((value) => value + 1); }} />;
...
export function DashboardVideo({ widget, locale, compact }: { widget: DashboardDataWidgetConfig; locale: AppLocale; compact: boolean }) {
  ...
  return reference?.playbackUrl
    ? <StreamVideo src={reference.playbackUrl} fit={widget.videoFit ?? "contain"} autoplay={widget.videoAutoplay !== false && !compact} muted={widget.videoMuted !== false} controls={!compact} locale={locale} />
...
export function DashboardMonitor({ widget, locale, compact }: { widget: DashboardDataWidgetConfig; locale: AppLocale; compact: boolean }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void import("../api").then(({ api }) => api.resolveLiveMonitor(source.url, reference.protocol ?? "hls"))
      .then((resolved) => { if (!cancelled) setResolvedUrl(reference.protocol === "webrtc" ? resolved.webRtcUrl : resolved.hlsUrl); })
      .catch((error) => { if (!cancelled) setFailure(error instanceof Error ? error.message : tr(locale, "监控流解析失败", "Monitor stream resolution failed")); });
  ...
  if (!resolvedUrl) return <MediaMessage locale={locale} icon="monitor" message={reference?.sourceUrl ? tr(locale, "正在连接媒体网关…", "Connecting to the media gateway…") : tr(locale, "填写监控源地址并解析", "Enter and resolve a monitor source URL")} />;
  if (reference?.protocol === "webrtc") {
    ...
    return <><iframe src={result.url} title={widget.title} allow="autoplay; fullscreen" sandbox="allow-scripts allow-same-origin allow-forms" referrerPolicy="no-referrer" />{compact && <i>{tr(locale, "编辑时已暂停画面交互", "Interaction paused while editing")}</i>}</>;
  }
  return <StreamVideo src={resolvedUrl} fit={widget.videoFit ?? "cover"} autoplay={widget.videoAutoplay !== false && !compact} muted={widget.videoMuted !== false} controls={!compact} locale={locale} />;
}

export function StreamVideo({ src, fit, autoplay, muted, controls, locale }: StreamVideoProps) {
  ...
  function retryPlayback() {
    ...
    if (failure?.kind === "autoplay" && video) {
      video.muted = true;
      void video.play().then(() => setFailure(undefined)).catch(() => setAttempt((value) => value + 1));
      return;
    }
    setFailure(undefined);
    setStalled(false);
  ...
  useEffect(() => {
    ...
    const inspectPlayback = () => {
      ...
      watchdog = advancePlaybackWatchdog(watchdog, { currentTime: video.currentTime, paused: video.paused, waiting: isPlaybackWaiting(waiting, video.readyState), hidden: document.hidden }, Date.now());
      if (!disposed) setStalled(watchdog.stalled);
      if (!wasStalled && watchdog.stalled) scheduleReconnect();
    };
    const markWaiting = () => { waiting = true; inspectPlayback(); };
    const markPlaying = () => { waiting = false; inspectPlayback(); };
    const markProgress = () => {
    ...
    const fail = () => {
      if (!disposed) {
        setStalled(false);
        setFailure({ kind: "stream", text: tr(locale, "播放失败或流已中断。请检查流服务、鉴权、CORS 后重试。", "Playback failed or the stream was interrupted. Check the service, authentication, and CORS, then retry.") });
        scheduleReconnect();
      }
    };
    const beginPlayback = () => {
      ...
      void video.play()
        .then(() => { if (!disposed) setFailure(undefined); })
        .catch(() => { if (!disposed) setFailure({ kind: "autoplay", text: tr(locale, "画面已就绪，浏览器需要一次点击才能开始播放。", "The stream is ready; the browser requires one click to start playback.") }); })
        .finally(() => { playRequestPending = false; });
    };
    const clearFailure = () => { if (!disposed) setFailure(undefined); };
    const mediaReady = () => { if (disposed) return; clearFailure(); setStalled(false); beginPlayback(); };
  ...
  return <><video ref={ref} style={{ objectFit: fit }} autoPlay={playback.autoplay} muted={playback.muted} controls={controls} loop playsInline preload="metadata" />{failure && !reconnectNotice && <MediaMessage locale={locale} icon={failure.kind === "stream" ? "warning" : "video"} message={failure.text} onRetry={retryPlayback} overlay={failure.kind === "stream"} />}{reconnectNotice && <MediaMessage locale={locale} icon="video" message={tr(locale, `信号中断，正在自动重连（第 ${reconnectNotice} 次）…`, `Signal interrupted. Reconnecting automatically (attempt ${reconnectNotice})…`)} overlay />}{stalled && !failure && !reconnectNotice && <MediaMessage locale={locale} icon="video" message={tr(locale, "信号中断，正在自动重连…", "Signal interrupted. Reconnecting automatically…")} overlay />}</>;
}

function MediaMessage({ locale, icon, message, onRetry, overlay = false }: { locale: AppLocale; icon: "image" | "video" | "monitor" | "warning"; message: string; onRetry?: () => void; overlay?: boolean }) {
  const Icon = icon === "image" ? ImageIcon : icon === "video" ? Video : icon === "monitor" ? Cctv : AlertTriangle;
  return <div role={icon === "warning" ? "alert" : "status"} style={overlay ? { position: "absolute", inset: 0, padding: 12, textAlign: "center", background: "rgba(7,11,13,.88)", zIndex: 2 } : undefined}><Icon size={22} /><span>{message}</span>{onRetry && <button type="button" onClick={onRetry}><RefreshCw size={12} />{tr(locale, "重试", "Retry")}</button>}</div>;
}
