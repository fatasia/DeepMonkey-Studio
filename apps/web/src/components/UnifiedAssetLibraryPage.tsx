...
export function UnifiedAssetLibraryPage({ controller }: { controller: SceneManagerController }) {
  ...
  return (
    <section className="manager-page-panel asset-library-page unified-assets-page" aria-label={tr(locale, "素材中心", "Asset center")}>
      <header className="model-library-head unified-assets-head">
        <div>
          <span className="eyebrow">UNIFIED ASSET CENTER</span>
          <h2>{tr(locale, "素材中心", "Asset center")}</h2>
          <p>{tr(locale, "一个入口管理二维、三维与项目素材；编辑器会自动呈现当前场景最相关的内容。", "One place for 2D, 3D and project assets, filtered automatically by editing context.")}</p>
        </div>
        <div className="unified-assets-scope" role="tablist" aria-label={tr(locale, "素材范围", "Asset scope")}>
          <button role="tab" aria-selected={scope === "library"} className={scope === "library" ? "active" : ""} onClick={() => setScope("library")}><Boxes size={15} />{tr(locale, "公共素材", "Library")}</button>
          <button role="tab" aria-selected={scope === "project"} className={scope === "project" ? "active" : ""} onClick={() => setScope("project")}><FolderOpen size={15} />{tr(locale, `项目素材 ${project?.models.length ?? 0}`, `Project assets ${project?.models.length ?? 0}`)}</button>
        </div>
      </header>
      {scope === "library" && (
        <nav className="unified-assets-kinds" aria-label={tr(locale, "素材类型", "Asset types")}>
          ...
          <KindButton active={kind === "2d"} icon={<ChartNoAxesCombined size={15} />} label={tr(locale, `二维组件 ${DASHBOARD_COMPONENT_PRESETS.length}`, `2D components ${DASHBOARD_COMPONENT_PRESETS.length}`)} onClick={() => setKind("2d")} />
