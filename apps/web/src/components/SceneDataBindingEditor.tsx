...
export function SceneDataBindingEditor({
  ...
  useEffect(() => {
    ...
    const referenced = [
      ...new Map(
        targetBindings
          .filter((binding) => !binding.directBinding)
          .map((binding) => {
            const product = dataBindingProduct(binding);
            return [productKey(product.kind, product.id), product] as const;
          }),
      ).values(),
    ].filter((product) => !previews[productKey(product.kind, product.id)]);
    if (referenced.length === 0) return;
    void Promise.all(
      referenced.map(async (product) => {
        try {
  ...
  function addDirectBinding() {
    onChange([
      ...
      {
        id: crypto.randomUUID(),
        name: `${targetName} · ${tr(locale, "直接接口", "Direct interface")}`,
        enabled: true,
  ...
  async function testBinding(binding: SceneDataBindingState) {
    ...
    try {
      ...
      const key = binding.directBinding
        ? `direct:${binding.id}`
        : (() => {
            const product = dataBindingProduct(binding);
            return productKey(product.kind, product.id);
          })();
  ...
  return (
    <details className="scene-data-binding-editor" open>
      <summary>
        <span>{tr(locale, "数据绑定", "Data bindings")}</span>
        <small>{targetBindings.filter((binding) => binding.enabled).length}</small>
      </summary>
      <div className="scene-data-binding-body">
        ...
        {catalogStatus === "loading" && (
          <div className="scene-data-binding-empty">
            <LoaderCircle className="spin" size={15} />
            {tr(locale, "正在读取数据产品…", "Loading data products…")}
          </div>
        )}
        {catalogStatus === "error" && (
          <div className="scene-data-binding-empty error">
            {tr(locale, "数据中台暂不可用，直接 HTTP / WebSocket 仍可使用。", "The data platform is unavailable; direct HTTP / WebSocket remains available.")}
          </div>
        )}
        {catalogStatus === "ready" && products.length === 0 && (
          <div className="scene-data-binding-empty">
            <span>{tr(locale, "还没有可绑定的数据产品", "No data products are available")}</span>
            <button onClick={onOpenData}>{tr(locale, "创建数据管道", "Create a data pipeline")}</button>
          </div>
        )}
        {targetBindings.map((binding) => {
          ...
          return (
            <article className="scene-data-binding-card" key={binding.id}>
              <header>
                <label>
                  <input disabled={disabled} type="checkbox" checked={binding.enabled} onChange={(event) => updateBinding(binding.id, { enabled: event.target.checked })} />
                  <input
                    disabled={disabled}
                    value={binding.name}
                    aria-label={tr(locale, "绑定名称", "Binding name")}
                ...
                <button
                  disabled={disabled}
                  className="danger"
                  title={tr(locale, "删除绑定", "Delete binding")}
                  onClick={() => onChange(bindings.filter((candidate) => candidate.id !== binding.id))}
                >
                  <Trash2 size={12} />
                </button>
              </header>
              <label>
                ...
                <select
                  disabled={disabled}
                  value={binding.directBinding ? "direct" : "platform"}
                  onChange={(event) => {
              ...
              {binding.directBinding ? (
                <DirectBindingEditor
                  locale={locale}
                  value={binding.directBinding}
                  disabled={disabled}
                  onChange={(directBinding: DirectBindingSpec) => updateBinding(binding.id, { directBinding })}
                />
              ) : (
                <>
                  <label>
                    <span>{tr(locale, "数据产品", "Data product")}</span>
                    <select disabled={disabled} value={`${product!.kind}:${product!.id}`} onChange={(event) => void selectProduct(binding, event.target.value)}>
                      {pipelines.length > 0 && (
              ...
              <div className="scene-data-binding-grid">
                <label>
                  <span>{tr(locale, "字段", "Field")}</span>
                  {binding.directBinding ? (
                    <input disabled={disabled} value={binding.field} onChange={(event) => updateBinding(binding.id, { field: event.target.value })} />
                  ) : (
                    <select
                      disabled={disabled}
                      value={binding.field}
                    ...
                    >
                      {fields.length === 0 && <option value={binding.field}>{binding.field}</option>}
                      {fields.map((field) => (
              ...
              {!binding.directBinding && (
                <label>
                  ...
                  <div className="scene-data-binding-refresh">
                    <input
                      disabled={disabled}
                      type="range"
                      min="2"
                      max="60"
                      step="1"
                      value={binding.refreshSeconds}
                      onChange={(event) => updateBinding(binding.id, { refreshSeconds: Number(event.target.value) })}
                    />
                    <output>{binding.refreshSeconds}s</output>
                  </div>
                </label>
              )}
              <footer>
                <div className={`scene-data-binding-status ${runtime?.status ?? "idle"}`}>
                  <i />
                  <span>
                    {error ? error : runtime?.status === "ready" ? `${tr(locale, "当前值", "Current")}: ${formatValue(runtime.value)}` : tr(locale, "等待数据", "Waiting for data")}
                  </span>
                </div>
                <button disabled={disabled || testingId === binding.id} onClick={() => void testBinding(binding)}>
                  {testingId === binding.id ? <LoaderCircle className="spin" size={12} /> : <Play size={12} />}
                  {tr(locale, "测试", "Test")}
                </button>
              </footer>
            </article>
        ...
        <div>
          <button className="scene-data-binding-add" disabled={disabled || products.length === 0 || catalogStatus !== "ready"} onClick={() => void addBinding()}>
            <Plus size={13} />
            {tr(locale, "添加中台绑定", "Add platform binding")}
          </button>
          <button className="scene-data-binding-add" disabled={disabled} onClick={addDirectBinding}>
            <Plus size={13} />
...
function suggestedAction(field: DataDatasetField): DataEventAction {
  ...
  if (field.type === "json" && /material|pbr|roughness|metalness/i.test(key)) return "material";
...
function productKey(kind: "dataset" | "pipeline", id: string): string {
  return `${kind}:${id}`;
}
function errorMessage(reason: unknown): string {
  return reason instanceof Error ? reason.message : "数据预览失败";
}
