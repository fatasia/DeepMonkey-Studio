...
export function estimateRobotCycleBudget(input: RobotWorkcellAssistantInput): RobotCycleBudgetResult {
  ...
  for (const target of activeTargets) {
    ...
    if (moveSeconds !== undefined) {
      ...
      lines.push({
        ...
        basis: joint !== undefined && joint >= (cartesian ?? 0) ? "最大关节角差 ÷ 已确认关节速度" : "TCP 直线距离 ÷ 已确认 TCP 速度",
      });
    }
  ...
  if (overhead > 0) lines.push({ id: "controller-overhead", label: "控制器与通信固定开销", kind: "controller", seconds: round(overhead), basis: "本次已确认的规划预算" });
  ...
  const hasEstimate = activeTargets.length > 0 && estimatedMoves > 0;
...
function addTargetTimes(lines: RobotCycleBudgetLine[], target: RobotAssistantTargetInput, toolActionSec: number): void {
  ...
  if (toolActionSec > 0) lines.push({ id: `tool:${target.id}`, label: `${target.name} 工具动作`, kind: "tool", seconds: round(toolActionSec), basis: "本次已确认的工具动作预算" });
}
