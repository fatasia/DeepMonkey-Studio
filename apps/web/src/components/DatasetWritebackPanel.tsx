...
export function DatasetWritebackPanel({ locale, projectId, userId, dataset, canWrite, onSaved, fixedRecordId }: {
  ...
  }, cached), [userId, projectId, dataset.id, dataset.writeback, cached]);
  ...
  return <section className="dataset-writeback" aria-label={tr(locale, "业务填报", "Record entry")}>
    ...
    {viewRefresh.status === "loading" && <p role="status">{tr(locale, "正在刷新视图…", "Refreshing view…")}</p>}
    {viewRefresh.status === "error" && <div className="dataset-writeback-notice" role="alert">
      <span>{tr(locale, "记录已写入，视图刷新失败。", "Record saved, but refreshing the view failed.")}</span>
      <button type="button" onClick={() => void viewRefresh.refresh()}>{tr(locale, "刷新视图", "Refresh view")}</button>
    </div>}
