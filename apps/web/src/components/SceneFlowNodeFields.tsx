...
export function SceneFlowNodeFields({ entity, onChange }: { entity: SceneFlowNode; onChange: (next: SceneFlowNode) => void }) {
  ...
  const updateDistribution = (value: PlantLiteDistribution) => {
    ...
    if (node.kind === "transport") onChange({ ...entity, node: { ...node, travelTime: value } });
  };
  return <div className="scene-flow-node-fields">
    ...
    {distribution && <>
      <label><span>{node.kind === "source" ? "到料间隔（分）" : node.kind === "transport" ? "搬运时间（分）" : "处理时间（分）"}</span><input type="number" min={0.01} step={0.1} value={distributionTypicalValue(distribution)} onChange={event => { if (Number.isFinite(event.target.valueAsNumber) && event.target.valueAsNumber > 0) updateDistribution(setDistributionTypicalValue(distribution, event.target.valueAsNumber)); }} /></label>
    ...
    {(node.kind === "station" || node.kind === "transport") && <NumberField label="等待队列容量" value={node.queueCapacity ?? 20} integer onChange={queueCapacity => onChange({ ...entity, node: { ...node, queueCapacity } })} />}
  </div>;
}
