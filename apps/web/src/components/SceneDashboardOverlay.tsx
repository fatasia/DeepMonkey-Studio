...
import { Activity, Database, ExternalLink, GripVertical, PanelLeft, PanelRight, Pause, Play, Plus, Settings2, Trash2, X } from "lucide-react";
...
export function SceneDashboardOverlay({ locale, sceneId, state, readOnly = false, onChange, onClose }: {
  ...
  function addWidget(type: SceneDashboardWidgetType = selectedType) {
    ...
    const chart = type === "line" || type === "bar";
  ...
  return <section className={`scene-dashboard-overlay ${state.side} ${readOnly ? "read-only" : ""}`} style={{ width: state.width }} aria-label={tr(locale, "场景二维看板", "Scene dashboard") }>
    <header>
      ...
      <nav>
        ...
        {!readOnly && <a href="/node-red" target="_blank" rel="noreferrer" title={tr(locale, "高级数据源", "Advanced data sources")}><Database size={14} /></a>}
        {!readOnly && <button title={tr(locale, "关闭", "Close")} onClick={onClose}><X size={15} /></button>}
      </nav>
    </header>
    <div className={`dashboard-canvas ${editing ? "editing" : ""}`}>
      {editing && <div className="dashboard-quick-add"><label><span>{tr(locale, "当前数据", "Current data")}</span><input list={`dashboard-keys-${sceneId}`} value={selectedKey} onChange={(event) => setSelectedKey(event.target.value)} placeholder="source.key" /><datalist id={`dashboard-keys-${sceneId}`}>{availableKeys.map((key) => <option key={key} value={key} />)}</datalist></label><div className="dashboard-component-strip">{dashboardWidgetTypes(locale).map((item) => <button key={item.type} className={selectedType === item.type ? "active" : ""} title={tr(locale, `添加${item.label}`, `Add ${item.label}`)} onClick={() => addWidget(item.type)}><Plus size={10} />{item.label}</button>)}</div></div>}
