...
export function createPlantLiteModelExchange(model: PlantLiteModel): PlantLiteModelExchangeFile {
  ...
  return {
    ...
    source: { application: "DeepMonkey Studio", kind: "authoring-draft" },
...
function exchangeEnvelopeIssues(input: Record<string, unknown>): string[] {
  const issues: string[] = [];
  if (input.schema !== PLANT_LITE_MODEL_EXCHANGE_SCHEMA) issues.push("文件类型不受支持，请选择 DeepMonkey Studio Plant Lite 模型文件");
