...
export function DirectBindingEditor({
  locale,
  value,
  disabled = false,
  onChange,
  ...
  async function runTest() {
    ...
    try {
      const result = onTest ? await onTest(value) : await (await import("../directBindingRuntime")).testDirectBinding(value);
  ...
  return (
    <div className="direct-binding-editor">
      <div className="dashboard-frame-grid scene-data-binding-grid">
        <label>
          <span>{tr(locale, "接口协议", "Protocol")}</span>
          <select disabled={disabled} value={value.transport} onChange={(event) => switchTransport(event.target.value as DirectBindingSpec["transport"])}>
        ...
        {value.transport === "http" && (
          <label>
            ...
            <select
              disabled={disabled}
              value={value.http?.method ?? "GET"}
              onChange={(event) => onChange({ ...value, http: { ...value.http!, method: event.target.value as DirectBindingHttpMethod } })}
            >
              {(["GET", "POST", "PUT", "PATCH", "DELETE"] as DirectBindingHttpMethod[]).map((method) => (
      ...
      <label>
        <span>{tr(locale, "上游地址", "Upstream URL")}</span>
        <input
          disabled={disabled}
          value={value.endpoint}
          onChange={(event) => onChange({ ...value, endpoint: event.target.value })}
          placeholder={value.transport === "http" ? "https://api.example.com/telemetry" : "wss://events.example.com/telemetry"}
        />
      </label>
      <label>
        <span>{tr(locale, "服务端凭据引用", "Server credential reference")}</span>
        <input disabled={disabled} value={value.credentialRef ?? ""} onChange={(event) => updateCredentialRef(event.target.value)} placeholder="plant-api-readonly" />
        <small>{tr(locale, "这里只保存引用，密钥由服务器解析。", "Only the reference is saved; the server resolves the secret.")}</small>
      </label>
      <div className="dashboard-frame-grid scene-data-binding-grid">
        <label>
          <span>JSONPath</span>
          <input
            disabled={disabled}
            value={value.selection?.jsonPath ?? ""}
            onChange={(event) => updateSelection({ jsonPath: event.target.value })}
            placeholder="$.data.items[0]"
          />
        </label>
        <label>
          <span>{tr(locale, "字段", "Field")}</span>
          <input disabled={disabled} value={value.selection?.field ?? ""} onChange={(event) => updateSelection({ field: event.target.value })} placeholder="temperature" />
        </label>
      </div>
      {value.transport === "http" && value.http && (
        <>
          <label>
            <span>{tr(locale, "查询参数模板（JSON）", "Query parameter template (JSON)")}</span>
            <textarea
              disabled={disabled}
              value={paramsDraft}
              onChange={(event) => setParamsDraft(event.target.value)}
              onBlur={() =>
                commitJson<Record<string, DirectBindingTemplateScalar>>(
                  paramsDraft,
                  (params) => onChange({ ...value, http: { ...value.http!, params } }),
                  tr(locale, "查询参数", "Query parameters"),
                )
              }
            />
          </label>
          <label>
            <span>{tr(locale, "请求体模板（JSON）", "Body template (JSON)")}</span>
            <textarea
              disabled={disabled}
              value={bodyDraft}
              onChange={(event) => setBodyDraft(event.target.value)}
              onBlur={() =>
                commitJson<DirectBindingTemplateValue>(
                  bodyDraft,
                  (bodyTemplate) => onChange({ ...value, http: { ...value.http!, bodyTemplate } }),
                  tr(locale, "请求体", "Request body"),
                )
              }
            />
          </label>
          <label>
            <span>{tr(locale, "刷新周期（毫秒）", "Refresh interval (ms)")}</span>
            <input
              disabled={disabled}
              type="number"
              min="250"
              value={value.http.refresh.intervalMs}
              onChange={(event) => onChange({ ...value, http: { ...value.http!, refresh: { ...value.http!.refresh, intervalMs: Math.max(250, Number(event.target.value)) } } })}
            />
          </label>
        </>
      )}
      {value.transport === "websocket" && value.websocket && (
        <>
          <label>
            ...
            <input
              disabled={disabled}
              value={(value.websocket.protocols ?? []).join(", ")}
              onChange={(event) =>
                onChange({
                  ...value,
                  websocket: {
              ...
              placeholder="telemetry.v1"
            />
          </label>
          <label>
            <span>{tr(locale, "订阅消息模板（JSON）", "Subscribe message template (JSON)")}</span>
            <textarea
              disabled={disabled}
              value={subscribeDraft}
              onChange={(event) => setSubscribeDraft(event.target.value)}
              onBlur={() =>
                commitJson<DirectBindingTemplateValue>(
                  subscribeDraft,
                  (subscribeMessageTemplate) => onChange({ ...value, websocket: { ...value.websocket!, subscribeMessageTemplate } }),
                  tr(locale, "订阅消息", "Subscribe message"),
                )
              }
            />
          </label>
          <div className="dashboard-frame-grid scene-data-binding-grid">
            <label>
              <span>{tr(locale, "初始重连（毫秒）", "Initial reconnect (ms)")}</span>
              <input
                disabled={disabled}
                type="number"
                min="0"
                value={value.websocket.reconnect.initialDelayMs}
                onChange={(event) =>
                  onChange({ ...value, websocket: { ...value.websocket!, reconnect: { ...value.websocket!.reconnect, initialDelayMs: Math.max(0, Number(event.target.value)) } } })
                }
              />
            </label>
            <label>
              <span>{tr(locale, "最大重连（毫秒）", "Maximum reconnect (ms)")}</span>
              <input
                disabled={disabled}
                type="number"
                min="0"
                value={value.websocket.reconnect.maxDelayMs}
                onChange={(event) =>
                  onChange({ ...value, websocket: { ...value.websocket!, reconnect: { ...value.websocket!.reconnect, maxDelayMs: Math.max(0, Number(event.target.value)) } } })
                }
              />
            </label>
          </div>
          <label>
            <span>{tr(locale, "自动重连", "Automatic reconnect")}</span>
            <input
              disabled={disabled}
              type="checkbox"
              checked={value.websocket.reconnect.enabled}
              onChange={(event) => onChange({ ...value, websocket: { ...value.websocket!, reconnect: { ...value.websocket!.reconnect, enabled: event.target.checked } } })}
            />
          </label>
        </>
      )}
      {draftError && <small className="dashboard-data-binding-state error">{draftError}</small>}
      <button type="button" disabled={disabled || !value.endpoint.trim() || testState?.status === "testing"} onClick={() => void runTest()}>
        ...
        {testState?.status === "testing" ? tr(locale, "正在测试…", "Testing…") : tr(locale, "测试连接", "Test connection")}
      </button>
