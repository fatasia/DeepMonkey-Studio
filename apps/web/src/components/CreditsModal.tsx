...
const projects = [
  ...
  ["GNU LibreDWG", "GPL-3.0-or-later", "https://github.com/LibreDWG/libredwg"],
  ["Rapier", "Apache-2.0", "https://github.com/dimforge/rapier.js"],
...
export function CreditsModal({ locale, onClose }: { locale: AppLocale; onClose: () => void }) {
  return <div className="modal-backdrop credits-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="credits-modal" role="dialog" aria-modal="true" aria-label={tr(locale, "开源项目致谢", "Open-source acknowledgements")}>
      <header><span><HeartHandshake size={18} /></span><div><strong>{tr(locale, "开源项目致谢", "Open-source acknowledgements")}</strong><small>{tr(locale, "感谢这些项目让 iTwin Studio 成为可能", "Thanks to the projects that make iTwin Studio possible")}</small></div><button title={tr(locale, "关闭", "Close")} onClick={onClose}><X size={16} /></button></header>
