...
export function ModelOptimizerHeader({ locale, onBack, onViewAssets, onImport, onDownload, onSave, canExport, canSave, busy, saved = false }: {
  locale: AppLocale; onBack: () => void; onImport: () => void; onDownload: () => void; onSave: () => void;
  onViewAssets?: () => void;
  canExport: boolean; canSave: boolean; busy: boolean; saved?: boolean;
}) {
  return <header className="optimizer-header">
    ...
    <div className="optimizer-header-actions">
      ...
      <button className="primary" disabled={!canSave && !saved} title={!canSave && !saved ? tr(locale, "导入模型并完成处理后可保存到项目素材", "Import and process a model before saving to project assets") : undefined} onClick={saved ? onViewAssets ?? onBack : onSave}><Database size={15} />{saved ? tr(locale, "查看项目素材", "View project assets") : tr(locale, "保存到项目素材", "Save to project assets")}</button>
    </div>
  </header>;
}
