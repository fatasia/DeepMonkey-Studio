...
export class ViewerEngine {
  ...
  private bimPlacementPreview: THREE.Object3D | undefined;
  ...
  private interactionScripts: SceneInteractionScriptState[] = [];
  private hoverInteractionTargets: SceneInteractionTarget[] = [];
  private pointerMoveSequence = 0;
  private lastInteractionHoverCheck = 0;
  ...
  setInteractionScripts(scripts: SceneInteractionScriptState[]): void {
    this.interactionScripts = scripts.map((script) => ({ ...script, target: { ...script.target }, ...(script.actions ? { actions: script.actions.map((action) => ({ ...action })) } : {}) }));
  }

  getInteractionScripts(): SceneInteractionScriptState[] {
    return this.interactionScripts.map((script) => ({ ...script, target: { ...script.target }, ...(script.actions ? { actions: script.actions.map((action) => ({ ...action })) } : {}) }));
  }

  dispatchInteraction(trigger: SceneInteractionTrigger, target: SceneInteractionTarget, detail: InteractionEventDetail = {}): void {
    const targets = target.kind === "object" && target.layerId
      ? [{ kind: "object", modelId: target.modelId } as SceneInteractionTarget, target]
      : [target];
  ...
  private dispatchObjectLifecycle(trigger: "load" | "animationStart" | "animationEnd", modelId: string): void {
  ...
  private async runInteractionAction(target: SceneInteractionTarget, action: SceneInteractionActionState): Promise<void> {
    ...
    if (action.type === "visibility") {
      const current = objectTarget.layerId
        ? this.fragmentLayers.get(objectTarget.modelId)?.get(objectTarget.layerId)?.node.visible ?? this.layerObjects.get(objectTarget.modelId)?.get(objectTarget.layerId)?.visible ?? true
        : this.models.get(objectTarget.modelId)?.visible ?? true;
      const visible = action.value === "show" ? true : action.value === "hide" ? false : !current;
      this.applySceneDataMessage({ source: "interaction", key: "visibility", value: visible, timestamp: new Date().toISOString(), target: objectTarget, action: "visibility" });
      return;
    }
  ...
  private interactionTargetContext(target: SceneInteractionTarget, detail: InteractionEventDetail): Record<string, unknown> {
    if (target.kind === "widget") return { ...target, widget: detail.payload };
    const model = this.models.get(target.modelId);
    const object = target.layerId
      ? detail.object ?? this.layerObjects.get(target.modelId)?.get(target.layerId) ?? model?.object
      : model?.object;
    return {
      ...target,
      model,
  ...
  private clearBimPlacementPreview(): void {
    if (!this.bimPlacementPreview) return;
    this.disposeObject(this.bimPlacementPreview);
    this.bimPlacementPreview = undefined;
  }

  private async componentBounds(record: ComponentRecord): Promise<BimBoundsValue | undefined> {
    const fragment = this.fragmentLayers.get(record.modelId)?.get(record.id);
    const fragmentModel = this.fragmentModels.get(record.modelId);
  ...
  private interactionTargetFromHit(hit: PointerSceneHit | undefined): SceneInteractionTarget | undefined {
  ...
  private interactionTargetHierarchy(target: SceneInteractionTarget | undefined): SceneInteractionTarget[] {
  ...
  private updateHoverInteraction(hit: PointerSceneHit | undefined, event: PointerEvent): void {
...
function sameRuntimeInteractionTarget(left: SceneInteractionTarget, right: SceneInteractionTarget): boolean {
...
function uniqueComponentRecords(records: ComponentRecord[]): ComponentRecord[] {
