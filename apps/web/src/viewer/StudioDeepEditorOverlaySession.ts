...
export class StudioDeepEditorOverlaySession {
  ...
  private revision = 0;
  read(roots: readonly THREE.Object3D[], camera: THREE.Camera, width: number, height: number, pixelRatio: number): EditorOverlaySnapshot {
    const vertices = projectStudioEditorOverlay(roots, camera, width * pixelRatio, height * pixelRatio, pixelRatio);
