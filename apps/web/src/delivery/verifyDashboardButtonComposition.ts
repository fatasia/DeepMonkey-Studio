...
export function verifyDashboardButtonComposition(evidence: DashboardRasterEvidence, input: DashboardRasterCompileInput,
  ...
  if (value.sourceWidth !== Math.ceil(box.rect[2]) || value.sourceHeight !== Math.ceil(box.rect[3]))
  ...
  const output = compositeDashboardButtonGroup(value.recipe.group, { rect: value.recipe.textRect,
