 ...
 */
export function lowerDashboardWidget(node: DashboardDataWidgetNode, bindingId: string, revision: number): WidgetContentLowering {
  const widget = node.widget;
  const resources: Deep2dResource[] = [], commands: Deep2dCommand[] = [];
  ...
  if (node.visible !== false) {
    ...
    contentCompiled = lowerWidgetContent(node, bindingId, revision, draw, compiledFields, reasons);
  }
  // 已知未编译域统一登记:文字排版、数据绑定、阴影与运行时行为属于后续切片。
  reasons.push("组件文字排版、数据绑定内容、容器阴影与运行时行为尚未编译");
  return { resources, commands, compiledFields, reasons, contentCompiled };
}
