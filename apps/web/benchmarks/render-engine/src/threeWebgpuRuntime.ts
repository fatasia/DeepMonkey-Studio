...
export async function createThreeWebgpuRuntime(canvas: HTMLCanvasElement, objectCount: number, startedAt: number, workload: BenchmarkWorkload): Promise<RenderBenchmarkRuntime> {
  const renderer = new WebGPURenderer({ canvas, antialias: true, powerPreference: "high-performance", trackTimestamp: true });
  await renderer.init();
  ...
  const timestampAvailable = Boolean((renderer.backend as { trackTimestamp?: boolean }).trackTimestamp);
  let timestampResolutionPending = false;
  let renderedFrames = 0;
  let lastRebuildMs = 0;
  let firstFrameMs = 0;
  rebuild(0);
  const initializedMs = performance.now() - startedAt;
  let animationFrame = requestAnimationFrame(renderFrame);
  ...
  function renderFrame(time: number): void {
    ...
    renderedFrames += 1;
    if (timestampAvailable && renderedFrames % 30 === 0 && !timestampResolutionPending) {
      timestampResolutionPending = true;
      void renderer.resolveTimestampsAsync("render")
        .then((duration) => gpuSampler.record(Number(duration)))
        .finally(() => { timestampResolutionPending = false; });
    }
  ...
  return {
    ...
    snapshot() {
      const info = renderer.info as unknown as WebGpuRendererInfo;
      return {
        engine: "three-webgpu",
        workload,
        objectCount,
