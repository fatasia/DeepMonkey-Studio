...
// RayBackend WGSL 软件执行器真机对拍 runner（波次4；模式沿用 r2ShaderIrGpuTest.mjs）。
// headless Chrome + WebGPU 走完整 API 路径（执行器打包 → dispatch → 读回），与 CPU 参考
// 实现（rayTrace.traceClosest，同 bundle 单一来源）逐射线对拍：命中/缺席一致、
// primitiveIndex 精确相等、t 相对容差 1e-5。证据写入 test-output/ray-trace-gpu-20260920-r1/。

const packageRoot = fileURLToPath(new URL("../", import.meta.url));
const repoRoot = path.resolve(packageRoot, "../..");
const outputDirectory = path.join(repoRoot, "test-output", "ray-trace-gpu-20260920-r1");
...
async function main() {
  ...
  const caseResults = [];
  let gate = Boolean(probe.cases) && Object.keys(probe.cases).length === casesSpec.length;
  for (const spec of casesSpec) {
    ...
    const caseResult = {
      name: spec.name, note: spec.note, rayCount: spec.rays.tMax.length, gpuHitCount: hitCount,
      inputSha256: {
  ...
  const evidence = {
    ...
    lane: "wave4-software-ray-tracing", createdAt: new Date().toISOString(),
    ...
    verdict: {
      ...
      notes: [
        "CPU 参考与浏览器腿共用同一 esbuild bundle（buildTracedScene/traceClosest/执行器单一来源，防口径分叉）。",
        "对拍合同：命中/缺席逐射线一致 + primitiveIndex 精确相等 + t 相对容差 1e-5；另记录 16B HitRecord 缓冲逐位比较作证据（非门槛）。",
        "单 BLAS 切片；TLAS 实例层与 native wgpu RT 腿属后续波次。",
        "栈深上限 32 fail-closed：stackOverflows 哨兵非零即整批拒绝，本门槛要求全案例为 0。",
      ],
    },
  };
  await writeFile(path.join(outputDirectory, "evidence.json"), JSON.stringify(evidence, null, 2));
  const summary = caseResults.map((entry) => `${entry.name}: rays=${entry.rayCount} hits=${entry.gpuHitCount} ` +
