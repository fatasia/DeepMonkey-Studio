...
const babylonPairingResult = await build({ absWorkingDir: root, entryPoints: ["lab/babylonPairingMain.ts"], bundle: true, format: "esm", target: "es2022", minify: true,
  outfile: path.join(output, "babylonPairing.js"), conditions: ["development"], metafile: true, legalComments: "eof" });
...
for (const file of ["babylonPairing.html"]) await copyFile(path.join(root, "lab", file), path.join(output, file));
...
for (const file of ["lab.js", "switch.js", "benchmark.js", "babylonPairing.js", "index.html", "lab.css",
  "switch.html", "switch.css", "benchmark.html", "benchmark.css", "babylonPairing.html", "tokens.css",
  "LICENSE", "LICENSE.zh-CN.md", "THIRD_PARTY_NOTICES.md",
...
const babylonPairingInputs = {};
for (const name of Object.keys(babylonPairingResult.metafile.inputs).sort()) {
  babylonPairingInputs[name] = createHash("sha256").update(await readFile(path.resolve(root, name))).digest("hex");
}
