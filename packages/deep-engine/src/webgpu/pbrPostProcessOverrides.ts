...
export function validatePbrPostProcessOverrides(value: PbrPostProcessOverrides | undefined): void {
  ...
  for (const [key, candidate] of Object.entries(value)) {
    ...
    if (key !== "ambientOcclusion" && key !== "bloom") throw new TypeError(`Unknown PBR postProcess option: ${key}.`);
...
export function resolvePbrPostProcessOverrides(value: PbrPostProcessOverrides | undefined,
  features: Pick<PbrRendererFeatures, "ambientOcclusion" | "bloom">): Readonly<{
    ambientOcclusion: boolean; bloom: boolean; authorBloom?: PbrAuthorBloomOptions }> {
  validatePbrPostProcessOverrides(value);
  for (const key of ["ambientOcclusion", "bloom"] as const) {
  ...
  return { ambientOcclusion: value?.ambientOcclusion ?? features.ambientOcclusion, bloom,
