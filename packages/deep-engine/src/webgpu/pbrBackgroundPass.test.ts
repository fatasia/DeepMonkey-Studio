...
describe("panorama GPU draw", () => {
  it.each([false, true])("matches MRT=%s while leaving geometric depth untouched", mrt => {
    ...
    const displayDescriptor = createRenderPipeline.mock.calls[1]![0] as GPURenderPipelineDescriptor;
    expect(descriptor.depthStencil).toMatchObject({ depthWriteEnabled: false, depthCompare: "always" });
    expect(displayDescriptor.depthStencil).toMatchObject({ depthWriteEnabled: false, depthCompare: "equal" });
    expect(displayDescriptor.fragment).toMatchObject({ entryPoint: "display", targets: [{ format: undefined }] });
