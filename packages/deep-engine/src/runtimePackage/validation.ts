...
import { DEEP_RUNTIME_PACKAGE_BUDGETS as LIMITS, DEEP_RUNTIME_PACKAGE_SCHEMA, DEEP_RUNTIME_PACKAGE_SCHEMA_VERSION,
  DEEP_RUNTIME_PACKAGE_SHADER_BINDINGS_VERSION, type DeepRuntimePackage, type RuntimePackageValidation, type RuntimeResourceKind } from "./types.js";
...
function validate(input: unknown): DeepRuntimePackage {
  ...
  const hasBindings = value.schemaVersion === DEEP_RUNTIME_PACKAGE_SHADER_BINDINGS_VERSION;
  ...
  for (const [position, candidate] of entries.entries()) {
    ...
    requireValue(["render-packet", "deep2d-runtime", "ibl-environment", "shader-package"].includes(string(entry.kind, `${path}.kind`)), path, "Unknown resource kind.");
  ...
  fields(entry, ["renderPacket", "deep2d", "environment", "shaderPackages"], [], "$.entrypoints");
  ...
  if (hasBindings) {
    ...
    validateRuntimeMaterialBindings(value.materialBindings, packet.materials, shaders);
  }
