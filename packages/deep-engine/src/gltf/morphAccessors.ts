...
interface MorphBufferView { readonly data: DataView; readonly offset: number; readonly length: number; readonly stride?: number; readonly target?: number }
...
export class MorphAccessorReader {
  ...
  private layout(value: unknown, type: "SCALAR" | "VEC3" | "VEC4", width: number, maximum: number,
    ...
    if (accessor.sparse !== undefined) unsupported(`${location}.sparse`, "sparse morph accessors");
    ...
    if (usage === "vertex" && view.target !== undefined && view.target !== 34962) invalid(location, "Buffer target conflicts with morph vertex usage.");
    if (usage === "animation" && view.target !== undefined) invalid(location, "Morph animation buffer views cannot declare a GPU target.");
    ...
    if (offset % 4 !== 0 || view.offset % 4 !== 0 || stride < elementBytes || stride % 4 !== 0) invalid(location, "Morph accessor alignment or stride is invalid.");
    if (offset + (count - 1) * stride + elementBytes > view.length) invalid(location, "Morph accessor exceeds its buffer view.");
    this.usage.set(view, usage);
    return { accessor, location, view, count, offset, stride };
  }
  ...
  private scan(layout: MorphAccessorLayout, width: number, output?: Float32Array<ArrayBuffer>): void {
    for (let item = 0; item < layout.count; item += 1) for (let component = 0; component < width; component += 1) {
      const value = layout.view.data.getFloat32(layout.view.offset + layout.offset + item * layout.stride + component * 4, true);
