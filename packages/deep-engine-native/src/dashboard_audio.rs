...
impl DashboardAudioTrack {
    ...
    pub fn output_devices() -> Result<Vec<String>, String> {
        ...
        if let Some(name) = default { names.push(name); }
        let devices = host.output_devices().map_err(|error| format!("enumerate audio outputs: {error}"))?;
        for device in devices {
            if let Ok(description) = device.description() {
                let name = description.name().to_string();
                if !names.iter().any(|existing| existing == &name) { names.push(name); }
            }
        }
        Ok(names)
    }
    ...
    pub fn play(&self) { self.player.play(); }
