...
fn two_level_records(
    ...
    records.push(
        coarse
            .encode_with_base(2 + fine.probe_count as usize + 1)
            .unwrap(),
    );
...
fn two_level_cascade_blends_by_web_smoothstep() {
    ...
    for axis in 0..3 {
        assert!(
            (deep[axis] - FINE_VALUE[axis]).abs() < 1e-6,
            "deep[{axis}]={}",
            deep[axis]
        );
    }
    ...
    for axis in 0..3 {
        assert!(
            (edge[axis] - COARSE_VALUE[axis]).abs() < 1e-6,
            "edge[{axis}]={}",
            edge[axis]
        );
    }
    ...
    for axis in 0..3 {
        ...
        assert!(
            (blended[axis] - expected).abs() < 1e-6,
            "blended[{axis}]={}",
            blended[axis]
        );
    }
...
fn coarse_fallback_when_fine_weight_is_insufficient() {
    ...
    for axis in 0..3 {
        assert!(
            (value[axis] - COARSE_VALUE[axis]).abs() < 1e-6,
            "value[{axis}]={}",
            value[axis]
        );
    }
    ...
    for axis in 0..3 {
        assert!(
            (value[axis] - FINE_VALUE[axis]).abs() < 1e-6,
            "value[{axis}]={}",
            value[axis]
        );
    }
...
fn cascade_record_budget_is_fail_closed() {
    ...
    let mut records = vec![layout.encode().unwrap()];
    let big = ProbeGiGridHeader {
        origin: [0.0; 3],
        spacing: 1.0,
        grid_size: [40, 40, 40],
        probe_count: 64_000,
    };
    let small = ProbeGiGridHeader {
        origin: [0.0; 3],
        spacing: 2.0,
        grid_size: [12, 12, 12],
        probe_count: 1_728,
