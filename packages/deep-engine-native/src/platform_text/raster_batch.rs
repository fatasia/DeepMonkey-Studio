...
pub fn rasterize_text_batch_json(bytes: &[u8]) -> Result<Vec<u8>, String> {
    if bytes.len() > super::TEXT_RASTER_REQUEST_MAX_BYTES {
        return Err("text batch exceeds 90 MiB".into());
    }
    let batch: Batch = serde_json::from_slice(bytes).map_err(|e| e.to_string())?;
    if batch.schema != "deep-engine.text-raster-batch"
        || batch.schema_version != 1
        || batch.requests.is_empty()
        || batch.requests.len() > 512
    {
    ...
    if shared.schema != "deep-engine.text-raster-request"
        || shared.schema_version != 1
        || shared.fonts.is_empty()
        || shared.fonts.len() > 32
    {
        ...
        pixels = pixels
            .checked_add(
                (request.width as usize)
                    ...
                    .ok_or("text batch extent overflow")?,
            )
            .ok_or("text batch budget overflow")?;
        if pixels > 64 * 1024 * 1024 {
            return Err("text batch pixels exceed 64 MiB".into());
        }
    ...
    for font in shared.fonts {
        total = total
            .checked_add(crate::deep2d::runtime_base64::decoded_len(
                &font.data_base64,
            )?)
            .ok_or("font byte overflow")?;
        if total > super::FONT_BYTES_MAX {
    ...
    for (request, wire) in requests.into_iter().zip(batch.requests) {
        ...
        identity.update(wire.as_bytes());
        identity.update(b"}");
        results.push(result_value(
            rasterizer.rasterize_styled(request)?,
            identity.finish(),
        ));
    }
...
mod tests {
    ...
    fn header() -> (String, serde_json::Value) {
        ...
        let face = system
            ...
            .find(|face| {
                face.weight.0 == 400
                    && face.style == cosmic_text::Style::Normal
                    && face.families.iter().any(|(name, _)| {
                        ["Arial", "DejaVu Sans", "Liberation Sans"].contains(&name.as_str())
                    })
            })
        ...
        let font = system
            ...
            .unwrap();
        let reference =
            serde_json::json!({ "sha256": font["sha256"], "faceIndex": font["faceIndex"] });
        (
            serde_json::json!({ "schema": "deep-engine.text-raster-request", "schemaVersion": 1,
            "locale": "en-US", "fonts": [font] })
            .to_string(),
            reference,
        )
    }
    ...
    fn batch_matches_single_pixels_source_identity_and_font_receipts() {
        ...
        let requests: Vec<String> = ["Alpha", "A B", ""]
            .into_iter()
            .map(|text| {
                serde_json::json!({
            "text": text, "font": font, "weight": 400, "style": "normal", "align": "left",
            "verticalAlign": "top", "wrap": "none", "fontSize": 16, "lineHeight": 20,
            "width": 128, "height": 32, "color": [255,255,255,255] })
                .to_string()
            })
            .collect();
        let wire =
            serde_json::json!({ "schema": "deep-engine.text-raster-batch", "schemaVersion": 1,
            "shared": shared, "requests": requests })
            .to_string();
        let actual: serde_json::Value =
            serde_json::from_slice(&rasterize_text_batch_json(wire.as_bytes()).unwrap()).unwrap();
        for (index, request) in requests.iter().enumerate() {
            let single = format!(
                "{},\"request\":{}}}",
                shared.strip_suffix('}').unwrap(),
                request
            );
            let expected: serde_json::Value = serde_json::from_slice(
                &super::super::rasterize_text_json(single.as_bytes()).unwrap(),
            )
            .unwrap();
    ...
    fn batch_rejects_unknown_fields_empty_and_oversized_jobs() {
        for value in [
            serde_json::json!({ "schema":"deep-engine.text-raster-batch", "schemaVersion":1,"shared":"{}","requests":[] }),
