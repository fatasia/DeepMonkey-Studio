...
use cosmic_text::{Align, Attrs, Buffer, Color, Family, Metrics, Shaping, Style, Weight, Wrap};
...
impl FrozenTextRasterizer {
    pub fn rasterize_styled(
        ...
        let needed: HashSet<_> = buffer
            .layout_runs()
            .flat_map(|run| {
                run.glyphs
                    .iter()
                    .map(move |glyph| glyph.physical((0., run.line_y), 1.).cache_key)
            })
            .collect();
        self.inner
            .cache
        ...
        for run in buffer.layout_runs() {
            for glyph in run.glyphs {
                    ...
                    .get_image(&mut self.inner.fonts, physical.cache_key)
                {
                    ...
                    if !cached {
                        resident_bytes = resident_bytes.saturating_add(image.data.len());
                    }
                    if resident_bytes > 64 * 1024 * 1024
                        || raster_bytes > 64 * 1024 * 1024
                        || sample_budget > 64 * 1024 * 1024
                    {
