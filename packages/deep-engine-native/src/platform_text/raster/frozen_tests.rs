...
fn batched_labels_reuse_current_glyph_allocations_without_retaining_old_glyphs() {
    ...
    let prior: Vec<_> = painter
        .inner
        .cache
        .image_cache
        .iter()
        .filter_map(|(key, image)| image.as_ref().map(|image| (*key, image.data.as_ptr())))
        .collect();
    assert!(!prior.is_empty());
    painter.rasterize_styled(request("AAA")).unwrap();
    for (key, pointer) in &prior {
        assert_eq!(
            painter.inner.cache.image_cache[key]
                .as_ref()
                .unwrap()
                .data
                .as_ptr(),
    ...
    assert!(
        prior
            .iter()
            .all(|(key, _)| !painter.inner.cache.image_cache.contains_key(key))
    );
}
