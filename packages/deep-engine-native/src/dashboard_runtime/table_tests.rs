...
#[test]
fn table_controls_join_forward_and_reverse_page_focus() {
    let mut runtime = fixture();
    assert!(runtime.has_focus_controls());
    runtime.focus_step(false).unwrap();
    assert_eq!(runtime.focused_table_action().unwrap().action, "next");
    let action = runtime.focused_table_action().unwrap();
    runtime.table_action(&action).unwrap();
    runtime.focus_step(false).unwrap();
    assert!(runtime.focused_control().is_none());
    runtime.focus_step(true).unwrap();
    assert_eq!(runtime.focused_table_action().unwrap().action, "next");
    runtime.blur_controls().unwrap();
    assert!(runtime.focused_control().is_none());
}
...
fn table_sort_page_and_export_share_the_frozen_order() {
    let mut runtime = fixture();
    assert_eq!(
        runtime.table_export(&action("csv", None)).unwrap(),
        ("report_test.csv".into(), b"a".to_vec())
    );
    assert!(runtime.table_action(&action("next", None)).unwrap());
    assert!(!runtime.table_action(&action("next", None)).unwrap());
    assert!(
        runtime
            .table_action(&action("sort", Some("value")))
            .unwrap()
    );
...
fn table_rejected_action_keeps_content_order_and_revision() {
    ...
    assert!(
        runtime
            .table_action(&action("sort", Some("missing")))
            .is_err()
    );
